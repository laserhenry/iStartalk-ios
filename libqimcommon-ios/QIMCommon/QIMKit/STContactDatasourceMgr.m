...
@interface STContactDatasourceMgr ()

@property (nonatomic, retain, readwrite) NSMutableArray * mergedRootBranch;

@end

static STContactDatasourceMgr *_globalDataController = nil;

@implementation STContactDatasourceMgr
...
+ (STContactDatasourceMgr *)getInstance {
    ...
    dispatch_once(&onceToken, ^{
        _globalDataController = [[STContactDatasourceMgr alloc] init];
    });
    return _globalDataController;
}

-(void)expandBranchAtIndex:(NSInteger)index {
    STDatasourceItem *childNode = [self.mergedRootBranch objectAtIndex:index];
...
-(void)recursiveLoadUnMergeData:(NSDictionary *)recursiveDict byNode:(STDatasourceItem *)parentNode nLevel:(NSInteger)nLevel {
    /*
...
-(int) removeChildren:(STDatasourceItem *)parentNode  atIndex:(NSUInteger)index removeParentNode:(BOOL)removeParentNode{
