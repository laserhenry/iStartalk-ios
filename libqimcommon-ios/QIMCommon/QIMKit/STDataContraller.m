...
static STDataContraller *__globalDataController = nil;
@implementation STDataContraller

+ (STDataContraller *)getInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __globalDataController = [[STDataContraller alloc] init];
    });
...
- (long long) sizeofImagePath {
    ...
    return [STUtility sizeofPath:cachePath] + [STUtility sizeofPath:logDirectory];
}
...
- (void)addResource:(id)resource withKey:(NSString *)key {
    ...
    [STUtility performInBackground:block];
}
