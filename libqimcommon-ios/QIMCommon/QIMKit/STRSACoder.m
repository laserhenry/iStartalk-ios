...
+ (NSString *) RSAForPassword:(NSString *)password
{
    ...
    NSString *md5Key = [STRSACoder md5:password];
    
    //
    // step2 rsa
    
    NSString *rsaKey = [STRSACoder encryptByRsa:md5Key publicKeyFileName:[QIMNavConfigManager sharedInstance].pubkey];
    
    //
    // step3 url encode key
    
    NSString *key = [STRSACoder URLEncodedString:rsaKey];
    return key;
}
