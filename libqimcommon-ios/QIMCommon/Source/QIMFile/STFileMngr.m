...
- (void)qim_sendImageMessageWithImageUrl:(NSString *)imageUrl forMessage:(STMsgModel *)message withImageWidth:(CGFloat)width withImageHeight:(CGFloat)height {
    ...
    if (QIMMessageType_Text == msgType || QIMMessageType_Image == msgType || QIMMessageType_ImageNew == msgType) {
        ...
        if ([imageUrl qim_hasPrefixHttpHeader]) {
            ...
            messageNewBody = [NSString stringWithFormat:@"[obj type=\"image\" value=\"%@/%@\" width=%f height=%f]", [[STKit sharedInstance] qimNav_InnerFileHttpHost], imageUrl, width, height];
        }
    ...
    if ((videoConfigUseAble == YES) && (videoDuration <= videoMaxTimeLen)) {
        ...
        [self qim_newCheckVideo:videoMd5 withCallBack:^(NSDictionary *remoteVideoDic) {
            if (remoteVideoDic.count) {
                ...
                //限制视频时长
                NSInteger videoTimeLen = [[[STUserCacheManager sharedInstance] userObjectForKey:@"videoTimeLen"] integerValue] / 1000;
                NSInteger videoDuration = [[videoExt objectForKey:@"Duration"] integerValue];
...
- (void)qim_sendFileMessageWithFileUrl:(NSString *)fileUrl forMessage:(STMsgModel *)message {
