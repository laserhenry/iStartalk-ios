...
- (instancetype)init {
    ...
    if (self) {
        ...
        } else if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQChat) {
        ...
        NSString *navConfigStr = [[STUserCacheManager sharedInstance] userObjectForKey:@"NavConfig"];
        ...
        NSMutableDictionary *oldNavConfigUrlDict = [[STUserCacheManager sharedInstance] userObjectForKey:@"QC_CurrentNavDict"];
        ...
        NSDictionary *advertConfig = [[STUserCacheManager sharedInstance] userObjectForKey:@"AdvertConfig"];
        NSArray *adList = [advertConfig objectForKey:@"adlist"];
        if (adList.count > 0) {
            NSMutableArray *itemArray = [NSMutableArray array];
            for (NSDictionary *adDic in adList) {
                STAdvertItem *advertItem = [[STAdvertItem alloc] init];
                ...
                if (advertItem.adType == AdvertType_Image) {
                    NSString *filePath = [[STDataContraller getInstance] getSourcePath:advertItem.adImgUrl];
                    if (filePath == nil) {
                        continue;
                    }
                }
...
- (BOOL)debug {
    // TODO: 这里也要改
    NSNumber *debugNum = [[STUserCacheManager sharedInstance] userObjectForKey:@"QC_Debug"];
    if (debugNum == nil) {
        debugNum = @(NO);
        [[STUserCacheManager sharedInstance] setUserObject:debugNum forKey:@"QC_Debug"];
    }
...
- (NSString *)navUrl {
    NSString *appNavUrl = @"";
    NSDictionary *navUrlDict = [[STUserCacheManager sharedInstance] userObjectForKey:@"QC_CurrentNavDict"];
    if (navUrlDict) {
        appNavUrl = navUrlDict[QIMNavUrlKey];
    } else {
        if ([[STAppInfo sharedInstance] appType] != QIMProjectTypeQChat) {
            appNavUrl = @"https://www.qtalk.app:8080/newapi/nck/qtalk_nav.qunar";
        } else {
            appNavUrl = @"https://www.qtalk.app:8080/newapi/nck/qtalk_nav.qunar";
        }
    }
...
- (NSString *)navTitle {
    ...
    if (navUrlDict) {
        ...
        appNavTitle = [NSString stringWithFormat:@"%@导航", [[STAppInfo sharedInstance] appName]];
    }
    return appNavTitle;
}
...
- (NSMutableArray *)localNavConfigs {
    if (!_localNavConfigs) {
        NSMutableArray *clientNavServerConfigs = [NSMutableArray arrayWithArray:[[STUserCacheManager sharedInstance] userObjectForKey:@"QC_NavAllDicts"]];
        if (!clientNavServerConfigs.count) {

            clientNavServerConfigs = [NSMutableArray arrayWithCapacity:5];
            NSString *tempNavName = [NSString stringWithFormat:@"%@导航", [[STAppInfo sharedInstance] appName]];
            NSDictionary *qtalkNav = @{QIMNavNameKey: tempNavName, QIMNavUrlKey: @"https://www.qtalk.app:8080/newapi/nck/qtalk_nav.qunar"};
            NSDictionary *publicQTalkNav = @{QIMNavNameKey: @"Qunar公共域导航", QIMNavUrlKey: @"https://i.startalk.im/newapi/nck/qtalk_nav.qunar?c=startalk.im"};
            NSDictionary *qchatNav = @{QIMNavNameKey: @"QChat导航", QIMNavUrlKey: @"https://i.startalk.im/newapi/nck/qtalk_nav.qunar?c=startalk.im"};
            if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQTalk) {
...
//    _loginType = pwdLogin ? QTLoginTypePwd : QTLoginTypeSms;
    [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"QC_UserWillSaveNavDict"];
    ...
    [[QIMManager sharedInstance] sendTPGetRequestWithUrl:navConfigUrl withSuccessCallBack:^(NSData *responseData) {
        ...
        if (navConfig.count > 0) {
            ...
            if (!navConfigUrl || (range.location != NSNotFound && range.length > 0)) {
                [[STUserCacheManager sharedInstance] setUserObject:@(YES) forKey:@"isQunarQTalk"];
            }
            [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"QCNavFailed"];
            NSString *navConfigStr = [[QIMJSONSerializer sharedInstance] serializeObject:navConfig];
            [[STUserCacheManager sharedInstance] setUserObject:navConfigStr forKey:@"NavConfig"];
            [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"currentLoginUserName"];
            if (callBack) {
                callBack(YES);
            }
        } else {
            [[STUserCacheManager sharedInstance] setUserObject:@(NO) forKey:@"QCNavFailed"];
            [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"currentLoginUserName"];
            if (callBack) {
                callBack(NO);
            }
...
- (void)qimNav_updateNavigationConfigWithDomain:(NSString *)domain WithUserName:(NSString *)userName withCallBack:(QIMKitGetNavConfigCallBack)callback {
    if (domain) {
        ...
        [[STUserCacheManager sharedInstance] setUserObject:currentNav forKey:@"QC_CurrentNavDict"];
...
- (void)qimNav_updateNavigationConfigWithNavDict:(NSDictionary *)navDict WithUserName:(NSString *)userName Check:(BOOL)check WithForcedUpdate:(BOOL)forcedUpdate withCallBack:(QIMKitGetNavConfigCallBack)callback {
    ...
    long long navConfigUpdateTime = [[[STUserCacheManager sharedInstance] userObjectForKey:@"NavConfigUpdateTime"] longLongValue];
    long long currentTime = [[NSDate date] timeIntervalSince1970];
    BOOL qcNavFailed = [[[STUserCacheManager sharedInstance] userObjectForKey:@"QCNavFailed"] boolValue];
    if (check || currentTime - navConfigUpdateTime > 2 * 60 * 60 || [self debug] == 1 || forcedUpdate == YES || qcNavFailed == YES) {
        ...
        if (![realNavUrl containsString:@"https://"]) {
            if (![realNavUrl containsString:@"http://"]) {
                realNavUrl = [NSString stringWithFormat:@"https://i.startalk.im/newapi/nck/qtalk_nav.qunar?c=%@", realNavUrl];
            }
        }
        ...
        [self qimNav_updateNavigationConfigWithNavDict:navDict NavStr:navConfigUrl Check:check withCallBack:^(BOOL res) {
            ...
            if (resultSuccess) {
                ...
                [[STUserCacheManager sharedInstance] setUserObject:_localNavConfigs forKey:@"QC_NavAllDicts"];
            ...
            [[STUserCacheManager sharedInstance] setUserObject:@(currentTime) forKey:@"NavConfigUpdateTime"];
            [[STUserCacheManager sharedInstance] setUserObject:@(_loginType) forKey:@"LoginType"];
            if (navDict.count > 0) {
                [[STUserCacheManager sharedInstance] setUserObject:navDict forKey:@"QC_CurrentNavDict"];
            } else {
                NSDictionary *emptyNavDict = @{QIMNavNameKey: [self navTitle], QIMNavUrlKey: [self navUrl]};
                [[STUserCacheManager sharedInstance] setUserObject:emptyNavDict forKey:@"QC_CurrentNavDict"];
            }
            //[[QIMManager sharedInstance] updateNavigationConfig];
            [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"currentLoginUserName"];
            if (callback) {
...
- (void)qimNav_updateNavigationConfigWithCheck:(BOOL)check withCallBack:(QIMKitGetNavConfigCallBack)callback {
    ...
    NSDictionary *currentLoginNav = [[STUserCacheManager sharedInstance] userObjectForKey:@"QC_UserWillSaveNavDict"];
    NSString *userName = [[STUserCacheManager sharedInstance] userObjectForKey:@"currentLoginUserName"];
    if (currentLoginNav.count) {
        ...
        [self qimNav_updateNavigationConfigWithNavDict:[[STUserCacheManager sharedInstance] userObjectForKey:@"QC_CurrentNavDict"] WithUserName:userName Check:check WithForcedUpdate:NO withCallBack:^(BOOL success) {
            if (callback) {
                callback(success);
            }
        }];
...
- (void)qimNav_updateAdvertConfigWithCheck:(BOOL)check {
    NSDictionary *infoDictionary = [[NSBundle mainBundle] infoDictionary];
    NSString *appVersion = [[infoDictionary objectForKey:@"CFBundleVersion"] description];
    long long navConfigUpdateTime = [[[STUserCacheManager sharedInstance] userObjectForKey:@"AdvertConfigUpdateTime"] longLongValue];
    long long currentTime = [[NSDate date] timeIntervalSince1970];
    if (check || currentTime - navConfigUpdateTime > 24 * 60 * 60 || [self debug] == 1) {
        ...
        NSString *user = [[[STUserCacheManager sharedInstance] userObjectForKey:@"kLastUserId"] lowercaseString];
        NSURL *url = [NSURL URLWithString:[self navUrl]];
        NSString *host = [url host];
        NSString *advertConfigUrl = [NSString stringWithFormat:@"https://qim.qunar.com/advert/%@/advert.php?v=%@&p=%@&u=%@&debug=%@&ver=%d&nav=%@&mv=%@", [[STAppInfo sharedInstance] appName], appVersion, @"iphone", [user ? user : @"unknow" stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding], [self debug] ? @"true" : @"false", oldVersion, host ? host : @"unkown", @"v2"];
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    ...
                    if (version > oldVersion || [self debug] == 1) {
                        ...
                        if (adList.count > 0) {
                            ...
                            if ([self debug] != 1) {
                                [[STUserCacheManager sharedInstance] setUserObject:advertConfig forKey:@"AdvertConfig"];
                                long long currentTime = [[NSDate date] timeIntervalSince1970];
                                [[STUserCacheManager sharedInstance] setUserObject:@(currentTime) forKey:@"AdvertConfigUpdateTime"];
                            }
                        }
                    }
                });
