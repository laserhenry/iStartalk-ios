...
static STAppSetting *_appSetting = nil;
static NSString *MAPAPIKEY = nil;

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _appSetting = [[STAppSetting alloc] init];
    });
    return _appSetting;
}
 ...
 */
- (BOOL)isFirstLauched {
    NSString *AppBuildVersion = [[STUserCacheManager sharedInstance] userObjectForKey:@"AppBuildVersion"];
...
//        [[QTalkUserCacheManager shareInstance] removeUserObjectForKey:@"NavConfig"];
    ...
    if (![[STUserCacheManager sharedInstance] userObjectForKey:@"everLaunched"] || !AppBuildVersion || [AppBuildVersion compare:newAppBuildVersion options:NSNumericSearch] == NSOrderedAscending || ![AppBuildVersion isEqualToString:newAppBuildVersion]) {
        ...
        */
        ...
        //清空本地缓存的广告配置
        [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"AdvertConfig"];
        [[STUserCacheManager sharedInstance] clearUserCache];
...
//        [[QTalkUserCacheManager shareInstance] removeUserObjectForKey:@"QC_CurrentNavDict"];
        [[STUserCacheManager sharedInstance] setUserObject:@(YES) forKey:@"everLaunched"];
        [[STUserCacheManager sharedInstance] setUserObject:newAppBuildVersion forKey:@"AppBuildVersion"];
        [[STUserCacheManager sharedInstance] setUserObject:@(YES) forKey:@"firstLaunch"];
        if ([[STAppInfo sharedInstance] appType] != QIMProjectTypeQChat) {
            [[STUserCacheManager sharedInstance] setUserObject:@(YES) forKey:@"notesFirstNotice"];
        }
        return YES;
    } else {
        QIMVerboseLog(@"用户非第一次安装");
        if ([[STAppInfo sharedInstance] appType] != QIMProjectTypeQChat) {
            [[STUserCacheManager sharedInstance] setUserObject:@(NO) forKey:@"notesFirstNotice"];
        }
        [[STUserCacheManager sharedInstance] setUserObject:@(NO) forKey:@"firstLaunch"];
        return NO;
    }
    return NO;
}
 ...
 */
- (BOOL)debugMode {
    BOOL debug = [[[STUserCacheManager sharedInstance] userObjectForKey:@"QC_Debug"] boolValue];
    if (debug) {
        return YES;
    } else {
        return NO;
