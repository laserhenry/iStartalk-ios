...
@implementation STManager (Calendar)

- (NSArray *)selectTripByYearMonth:(NSString *)date {
    return [[STDataMgr qimDB_SharedInstance] qimDB_SelectTripByYearMonth:date];
}
...
- (void)getRemoteUserTripList {
    ...
    [bodyProperties setQIMSafeObject:[[STManager sharedInstance] getLastJid] forKey:@"userName"];
    ...
    [STHTTPClient sendRequest:request complete:^(QIMHTTPResponse *response) {
        if (response.code == 200) {
            ...
            if ([[result objectForKey:@"ret"] boolValue]) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertTrips:tripsList];

                NSString *updateTime = [tripsData objectForKey:@"updateTime"];
                NSString *jid = [[STManager sharedInstance] getLastJid];
                NSArray *configArray = @[@{@"subkey":jid?jid:@"", @"configinfo":updateTime}];
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertConfigArrayWithConfigKey:[strongSelf transformClientConfigKeyWithType:QIMClientConfigTypeKLocalTripUpdateTime] WithConfigVersion:0 ConfigArray:configArray];
            }
        }
...
- (NSArray *)getLocalAreaList {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getLocalArea];
}
    ...
    [STHTTPClient sendRequest:request complete:^(QIMHTTPResponse *response) {
        if (response.code == 200) {
            ...
            if ([[result objectForKey:@"ret"] boolValue]) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertArea:areaList];
            }
        }
    } failure:^(NSError *error) {
