...
- (int)getDealIdState:(NSString *)dealId {
    NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:@"kDealInfoDic"];
...
- (void)setDealId:(NSString *)dealId ForState:(int)state {
    ...
    [[STUserCacheManager sharedInstance] setUserObject:dealDic forKey:@"kDealInfoDic"];
}
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:data withSuccessCallBack:^(NSData *responseData) {
        ...
        if (value.count > 0) {
            ...
            if (errorCode == 0) {
                ...
                if (flag) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertPublicNumbers:cardList];
                }
                if (callback) {
                    callback(cardList);
                }
...
- (void)updateAllPublicNumberCard {
    NSArray *list = [[STDataMgr qimDB_SharedInstance] qimDB_getPublicNumberVersionList];
        ...
        if (response.code == 200) {
            ...
            if (value.count > 0) {
                ...
                if (errorCode == 0) {
                    ...
                    [[STDataMgr qimDB_SharedInstance] qimDB_checkPublicNumbers:pubList];
...
- (NSArray *)getPublicNumberMsgListById:(NSString *)publicNumberId WithLimit:(int)limit WithOffset:(int)offset {
    ...
    NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgListByPublicNumberId:publicNumberId WithLimit:limit WithOffset:offset WithFilterType:@[@(PublicNumberMsgType_Action), @(PublicNumberMsgType_PostBackCookie), @(PublicNumberMsgType_ClientCookie)]];
    if (array.count > 0) {
...
- (void)clearNotReadMsgByPublicNumberId:(NSString *)jid {
    ...
    //Comment
    [[STUserCacheManager sharedInstance] setUserObject:self.notReadMsgDic forKey:kNotReadPublicNumberMsgCount];
