...
- (NSString *)getUserMarkupNameWithUserId:(NSString *)userId {
    ...
    dispatch_block_t block = ^{
        ...
        if (!strongSelf.userMarkupNameDic.count) {
            NSDictionary *userMarkNameDics = [[STDataMgr qimDB_SharedInstance] qimDB_getUserMarkNameDic];
...
- (NSDictionary *)getUserInfoByUserId:(NSString *)myId {
    ...
    dispatch_block_t block = ^{
        ...
        if (!tempDic) {
            tempDic = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserByJID:myId];
        }
        result = tempDic;
...
- (NSString *)getUserBigHeaderImageUrlWithUserId:(NSString *)userId {
    ...
    if (!infoDic) {
        ...
        infoDic = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserByJID:userId];
    }
...
- (void)updateUserBigHeaderImageUrl:(NSString *)url WithUserMood:(NSString *)mood WithVersion:(NSString *)version ForUserId:(NSString *)userId {
    if (userId.length > 0 && url.length > 0) {
        [[STDataMgr qimDB_SharedInstance] qimDB_updateUser:userId WithMood:mood WithHeaderSrc:url WithVersion:version];
    }
}
...
- (void)updateUserCard:(NSArray *)xmppIds {
    ...
    dispatch_async(self.update_chat_card, ^{
        NSDictionary *usersDic = [[STDataMgr qimDB_SharedInstance] qimDB_selectUsersDicByXmppIds:xmppIds];
            ...
            if (response.code == 200) {
                ...
                if (ret) {
                    ...
                    for (NSDictionary *dataDic in list) {
                        ...
                        for (NSDictionary *userDic in userList) {
                            ...
                            if ([myUserID isEqualToString:userId]) {
                                if (isToCManager!=nil) {
                                     [[STUserCacheManager sharedInstance] setUserObject:isToCManager forKey:@"isToCManager"];
                                }
                            }
                    ...
                    if (dataList.count > 0) {
                        ...
                        [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateUserCards:dataList];
                        dispatch_async(dispatch_get_main_queue(), ^{
...
- (NSDictionary *)getUserWorkInfoByUserId:(NSString *)userId {
    __block NSDictionary *result = nil;
    dispatch_block_t block = ^{
        NSDictionary *tempDic = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserBackInfoByXmppId:userId];
        if (tempDic) {
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:requestData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (errcode == 0) {
            ...
            if ([resultData isKindOfClass:[NSDictionary class]]) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateUserBackInfo:userBackInfo WithXmppId:userId];
...
- (NSArray *)searchUserListBySearchStr:(NSString *)searchStr {
    ...
    dispatch_block_t block = ^{
        ...
        array = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserListBySearchStr:searchStr];
...
- (NSInteger)searchUserListTotalCountBySearchStr:(NSString *)searchStr {
    ...
    dispatch_block_t block = ^{
        totalCount = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserListTotalCountBySearchStr:searchStr];
    };
    if (dispatch_get_specific(self.cacheTag))
        block();
    else
...
- (NSArray *)selectUserListExMySelfBySearchStr:(NSString *)searchStr WithLimit:(NSInteger)limit WithOffset:(NSInteger)offset {
    ...
    dispatch_block_t block = ^{
        ...
        array = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserListExMySelfBySearchStr:searchStr WithLimit:limit WithOffset:offset];
    };
...
- (NSArray *)searchUserListBySearchStr:(NSString *)searchStr WithLimit:(NSInteger)limit WithOffset:(NSInteger)offset {
    ...
    dispatch_block_t block = ^{
        ...
        array = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserListBySearchStr:searchStr WithLimit:limit WithOffset:offset];
    };
