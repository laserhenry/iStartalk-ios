...
- (void)initManager {
    ...
    [[XmppImManager sharedInstance] setProductType:[[STAppInfo sharedInstance] appType]];
    [[XmppImManager sharedInstance] setAppVersion:[[STAppInfo sharedInstance] AppBuildVersion]];
    [[XmppImManager sharedInstance] setSystemVersion:[[STAppInfo sharedInstance] SystemVersion]];
    [[XmppImManager sharedInstance] setPlatform:[[STAppInfo sharedInstance] Platform]];
    [[XmppImManager sharedInstance] setDeviceName:[[STAppInfo sharedInstance] deviceName]];
...
- (void)initUserDicts {
    
    {
        ...
        NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:kStickJidDic];
        if (dic) {
    ...
    
    {
        ...
        NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:kConversationParamDic];
        if (dic) {
    ...
    {
        NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:kUserBigHeaderDic];
        if (dic) {
    ...
    {
        NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:kNotSendTextDic];
        [_notSendTextDic setDictionary:dic];
    }
    
    self.clientConfigDic = [NSMutableDictionary dictionary];
    {
        NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:kNewClinetConfigDic];
        if (dic) {
    ...
    {
        NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:kChannelInfoDic];
        if (dic) {
    ...
    if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQChat) {
        ...
        {
            NSDictionary *dic = [[STUserCacheManager sharedInstance] userObjectForKey:kAppendInfoDic];
            if (dic) {
...
- (void)loginComplateOperation {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        [[STUserCacheManager sharedInstance] setUserObject:userFullJid forKey:@"kFullUserJid"];
        QIMVerboseLog(@"LoginComplate 之后的userFullJid : %@", userFullJid);
        NSString *tempUserToken = [[STUserCacheManager sharedInstance] userObjectForKey:@"kTempUserToken"];
...
#warning NavDict NavConfig
        NSDictionary *currentLoginNavConfig = [[STUserCacheManager sharedInstance] userObjectForKey:@"NavConfig"];
        NSDictionary *currentNavDict = [[STUserCacheManager sharedInstance] userObjectForKey:@"QC_CurrentNavDict"];
    ...
    [[STDataMgr qimDB_SharedInstance] qimDB_updateMessageFromState:QIMMessageSendState_Waiting ToState:QIMMessageSendState_Faild];
    ...
    [self sendPushTokenWithMyToken:[[STAppInfo sharedInstance] getPushToken] WithDeleteFlag:NO withCallback:nil];
    ...
    switch (type) {
        ...
        case QIMClientConfigTypeKCollectionCacheKey: {
            ...
            for (NSDictionary *dic in valueArray) {
                ...
                NSString *subKey = [[STFileManager sharedInstance] md5fromUrl:configValue];
                NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithCapacity:3];
    ...
    [[QIMManager sharedInstance] sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:versionData withSuccessCallBack:^(NSData *responseData) {
        ...
        if ([responseDic isKindOfClass:[NSDictionary class]]) {
            ...
            if (ret && errcode == 0) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_UpdateUserCacheDataWithKey:kGetUpdateUsersV2Version withType:7 withValue:@"新版组织架构时间戳" withValueInt:maxVersion];
                NSArray *updateUserList = [dataDic objectForKey:@"update"];
                NSArray *userList = [dataDic objectForKey:@"update"];
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertOrgansUserInfos:userList];
                
                NSArray *deleteUserList = [dataDic objectForKey:@"delete"];
                [[STDataMgr qimDB_SharedInstance] qimDB_clearUserListForList:deleteUserList];
            } else {
...
- (void)setIsMerchant:(BOOL)isMerchant {
    _isLoadMerchant = YES;
    [[STUserCacheManager sharedInstance] setUserObject:@(isMerchant) forKey:@"isMerchant"];
...
- (BOOL)isMerchant {
    if (YES == _isLoadMerchant) {
        ...
        NSNumber *isMerchantCacheNumber = [[STUserCacheManager sharedInstance] userObjectForKey:@"isMerchant"];
...
+ (NSString *)getLastUserName {
    return [[[STUserCacheManager sharedInstance] userObjectForKey:kLastUserId] lowercaseString];
}

- (void)updateLastTempUserToken:(NSString *)token {
    [[STUserCacheManager sharedInstance] setUserObject:token forKey:@"kTempUserToken"];
}

- (NSString *)getLastTempUserToken {
    return [[STUserCacheManager sharedInstance] userObjectForKey:@"kTempUserToken"];
}

- (void)updateLastUserToken:(NSString *)tempUserToken {
    [[STUserCacheManager sharedInstance] setUserObject:tempUserToken?tempUserToken:@"" forKey:@"userToken"];
}

- (NSString *)getLastUserToken {
    return [[STUserCacheManager sharedInstance] userObjectForKey:@"userToken"];
}
...
- (NSString *)getLastPassword {
    return [[STUserCacheManager sharedInstance] userObjectForKey:kLastPassword];
}
...
- (NSDictionary *)loadWlanCookie {
    ...
    if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQChat) {
        ...
        [qcookieProperties setObject:@".startalk.im" forKey:NSHTTPCookieDomain];
        ...
        [vcookieProperties setObject:@".startalk.im" forKey:NSHTTPCookieDomain];
        ...
        [tcookieProperties setObject:@".startalk.im" forKey:NSHTTPCookieDomain];
        ...
        [qckeyCookieProperties setObject:@".startalk.im" forKey:NSHTTPCookieDomain];
...
- (void)checkClientConfig {
    ...
    if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQChat) {
        title = @"qchat";
    } else if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQTalk) {
        title = @"qtalk";
    } else {
        title = @"startalk";
    }
    ...
    [bodyProperties setObject:[[STAppInfo sharedInstance] AppBuildVersion] forKey:@"v"];
    [bodyProperties setObject:title forKey:@"ver"];
    [bodyProperties setObject:@"ios" forKey:@"p"];
    NSInteger clientVersion = [[[STUserCacheManager sharedInstance] userObjectForKey:kCheckConfigVersion] integerValue];
    [bodyProperties setObject:[NSString stringWithFormat:@"%lld", (clientVersion > 0) ? clientVersion : 0] forKey:@"cv"];
    [bodyProperties setQIMSafeObject:[[STAppSetting sharedInstance] currentLanguage] forKey:@"language"];
            ...
            if (resDic.count > 0) {
                ...
                if (errcode == 0) {
                    ...
                    if (dataDic.count) {
                        ...
                        [[STUserCacheManager sharedInstance] setUserObject:self.clientConfigDic forKey:kNewClinetConfigDic];
                        NSInteger cvVersion = [dataDic objectForKey:@"version"];
                        [[STUserCacheManager sharedInstance] setUserObject:@(cvVersion) forKey:kCheckConfigVersion];
                    }
                }
            }
        }
...
- (BOOL)pickerPixelOriginal {
    NSNumber *flagNum = [[STUserCacheManager sharedInstance] userObjectForKey:@"pickerPixelOriginal"];
    if (flagNum == nil) {
        flagNum = @(YES);
...
- (void)setPickerPixelOriginal:(BOOL)flag {
    [[STUserCacheManager sharedInstance] setUserObject:@(flag) forKey:@"pickerPixelOriginal"];
}

//OPS发现页RN调试
- (NSString *)opsFoundRNDebugUrl {
    NSString *opsFoundRNDebugUrl = [[STUserCacheManager sharedInstance] userObjectForKey:@"opsFoundRNDebugUrl"];
    return opsFoundRNDebugUrl;
}

- (void)setOpsFoundRNDebugUrl:(NSString *)opsFoundRNDebugUrl {
    [[STUserCacheManager sharedInstance] setUserObject:opsFoundRNDebugUrl forKey:@"opsFoundRNDebugUrl"];
}

//qtalk发现页测试地址
- (NSString *)qtalkFoundRNDebugUrl {
    NSString *qtalkFoundRNDebugUrl = [[STUserCacheManager sharedInstance] userObjectForKey:@"qtalkFoundRNDebugUrl"];
    return qtalkFoundRNDebugUrl;
}

- (void)setQtalkFoundRNDebugUrl:(NSString *)qtalkFoundRNDebugUrl {
    [[STUserCacheManager sharedInstance] setUserObject:qtalkFoundRNDebugUrl forKey:@"qtalkFoundRNDebugUrl"];
}

//qtalk搜索测试地址
- (NSString *)qtalkSearchRNDebugUrl {
    NSString *qtalkSearchRNDebugUrl = [[STUserCacheManager sharedInstance] userObjectForKey:@"qtalkSearchRNDebugUrl"];
    return qtalkSearchRNDebugUrl;
}

- (void)setQtalkSearchRNDebugUrl:(NSString *)qtalkSearchRNDebugUrl {
    [[STUserCacheManager sharedInstance] setUserObject:qtalkSearchRNDebugUrl forKey:@"qtalkSearchRNDebugUrl"];
}

//是否优先展示对方个性签名
- (BOOL)moodshow {
    NSNumber *flagNum = [[STUserCacheManager sharedInstance] userObjectForKey:@"moodshow"];
    if (flagNum == nil) {
        flagNum = @(YES);
...
- (void)setMoodshow:(BOOL)flag {
    [[STUserCacheManager sharedInstance] setUserObject:@(flag) forKey:@"moodshow"];
}

//是否展示水印
- (BOOL)waterMarkState {
    NSNumber *flagNum = [[STUserCacheManager sharedInstance] userObjectForKey:@"waterMarkState"];
    if (flagNum == nil) {
        flagNum = @(YES);
...
- (void)setWaterMarkState:(BOOL)flag {
    [[STUserCacheManager sharedInstance] setUserObject:@(flag) forKey:@"waterMarkState"];
}
...
- (NSArray *)getHasAtMeByJid:(NSString *)jid {
    ...
    dispatch_block_t block = ^{
        if (!_hasAtMeDic) {
            _hasAtMeDic = [[STDataMgr qimDB_SharedInstance] qimDB_getTotalAtMessageDic];
        }
...
//        [_hasAtMeDic removeObjectForKey:groupId];
        [[STDataMgr qimDB_SharedInstance] qimDB_UpdateAtMessageReadStateWithGroupId:groupId withMsgIds:msgIds withReadState:readState];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)clearAtMeMessageWithJid:(NSString *)groupId {
    dispatch_block_t block = ^{
        [_hasAtMeDic removeObjectForKey:groupId];
        [[STDataMgr qimDB_SharedInstance] qimDB_UpdateAtMessageReadStateWithGroupId:groupId withReadState:QIMAtMsgHasReadState];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)addOfflineAtMeMessageByJid:(NSString *)groupId withType:(QIMAtType)atType withMsgId:(NSString *)msgId withMsgTime:(long long)msgTime {
    dispatch_block_t block = ^{
        _hasAtMeDic = nil;
        [[STDataMgr qimDB_SharedInstance] qimDB_insertAtMessageWithGroupId:groupId withType:atType withMsgId:msgId withMsgTime:msgTime];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)addAtMeMessageByJid:(NSString *)groupId withType:(QIMAtType)atType withMsgId:(NSString *)msgId withMsgTime:(long long)msgTime {
    dispatch_block_t block = ^{
        ...
        if (arr == nil) {
            ...
            arr = [NSMutableArray arrayWithArray:[[STDataMgr qimDB_SharedInstance] qimDB_getAtMessageWithGroupId:groupId]];
...
- (void)setNotSendText:(NSString *)text inputItems:(NSArray *)inputItems ForJid:(NSString *)jid {
    ...
    if (text.length > 0) {
        ...
        [[STUserCacheManager sharedInstance] setUserObject:_notSendTextDic forKey:kNotSendTextDic];
    } else {
...
- (void)getQChatTokenWithBusinessLineName:(NSString *)businessLineName withCallBack:(QIMKitGetQChatTokenSuccessBlock)callback {
    ...
    NSDictionary *params = @{@"macCode": [[STAppInfo sharedInstance] macAddress], @"plat": (businessLineName.length > 0) ? businessLineName : @"app"};
...
- (NSDictionary *)getQVTForQChat {
    ...
    NSDictionary *qvtInfoDic = [[STUserCacheManager sharedInstance] userObjectForKey:kQVTCookie];
    return qvtInfoDic;
}

- (void)removeQVTForQChat {
    
    [[STUserCacheManager sharedInstance] removeUserObjectForKey:kQVTCookie];
}
...
*/
    ...
    [self sendTPGetRequestWithUrl:str withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            NSInteger localPushFlag = [[[STUserCacheManager sharedInstance] userObjectForKey:@"MsgSettings"] integerValue];
            localPushFlag = localPushFlag ^ setting;
            [[STUserCacheManager sharedInstance] setUserObject:@(localPushFlag) forKey:@"MsgSettings"];
            if (callback) {
                callback(YES);
            }
        } else {
...
- (BOOL)getLocalMsgNotifySettingWithIndex:(QIMMSGSETTING)setting {
    ...
    if (![[STUserCacheManager sharedInstance] containsObjectForKey:@"MsgSettings"]) {
        localPushFlag = QIMMSGSETTINGPUSH_SWITCH;
    } else {
        localPushFlag = [[[STUserCacheManager sharedInstance] userObjectForKey:@"MsgSettings"] integerValue];
    }
        ...
        if (response.code == 200) {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                if (ret) {
                    ...
                    [[STUserCacheManager sharedInstance] setUserObject:@(pushFlag) forKey:@"MsgSettings"];
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)sendNoPush {
    QIMErrorLog(@"注销Token");
    [[STAppInfo sharedInstance] setPushToken:nil];
}
...
- (void)sendServer:(NSString *)notificationToken withUsername:(NSString *)username withParamU:(NSString *)paramU withParamK:(NSString *)paramK WithDelete:(BOOL)deleteFlag withCallback:(QIMKitRegisterPushTokenSuccessBlock)callback {
    ...
    if ([[QIMNavConfigManager sharedInstance] newPush] == NO) {
        url = [NSString stringWithFormat:@"%@/push/qtapi/token/setpersonmackey.qunar?username=%@&domain=%@&mac_key=%@&platname=%@_%@_%@&pkgname=%@&os=%@&version=%@&show_content=%@", [[QIMNavConfigManager sharedInstance] javaurl], [userInfo objectAtIndex:0], [userInfo objectAtIndex:1], notificationToken, [[[STAppInfo sharedInstance] deviceName] stringByReplacingOccurrencesOfString:@" " withString:@""], [[[NSLocale preferredLanguages][0] componentsSeparatedByString:@"-"] firstObject], [[NSLocale currentLocale] countryCode], [[NSBundle mainBundle] bundleIdentifier], @"ios", [[STAppInfo sharedInstance] AppBuildVersion], @(YES)];
    } else {
        url = [NSString stringWithFormat:@"%@/qtapi/token/setpersonmackey.qunar?username=%@&domain=%@&mackey=%@&os=%@&version=%@", [[QIMNavConfigManager sharedInstance] javaurl], [userInfo objectAtIndex:0], [userInfo objectAtIndex:1], notificationToken, @"ios", [[STAppInfo sharedInstance] AppBuildVersion]];
    }
    if (deleteFlag) {
...
- (void)checkClearCache {
    NSInteger clearCacheVersion = [[[STUserCacheManager sharedInstance] userObjectForKey:kClearCacheCheck] integerValue];
    if (clearCacheVersion < kClearCacheVersion) {
        QIMErrorLog(@"clearCacheVersion : %lu", clearCacheVersion);
        [self clearcache];
        [[STUserCacheManager sharedInstance] setUserObject:@(kClearCacheVersion) forKey:kClearCacheCheck];
    }
}

- (void)findNewestClient {
    NSInteger updateAppVersion = [[[STUserCacheManager sharedInstance] userObjectForKey:@"updateAppVersion"] integerValue];
    if (updateAppVersion > 0 && updateAppVersion > [[[STAppInfo sharedInstance] AppBuildVersion] integerValue]) {
