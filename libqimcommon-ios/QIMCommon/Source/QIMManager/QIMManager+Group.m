...
- (NSDictionary *)getGroupCardByGroupId:(NSString *)groupId {
    ...
    dispatch_block_t block = ^{
        ...
        groupVCard = [self.groupVCardDict objectForKey:groupId];
        if (!groupVCard.count) {
            groupVCard = [[STDataMgr qimDB_SharedInstance] qimDB_getGroupCardByGroupId:groupId];
        }
...
- (NSArray *)syncgroupMember:(NSString *)groupId {
    ...
    [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertGroupMember:[dic allValues] WithGroupId:groupId];
...
- (NSArray *)getGroupMembersByGroupId:(NSString *)groupId {
    NSArray *result = [[STDataMgr qimDB_SharedInstance] qimDB_getGroupMember:groupId];
    if ([result count] <= 0) {
...
- (BOOL)isGroupMemberByGroupId:(NSString *)groupId {
    NSDictionary *memberInfo = [[STDataMgr qimDB_SharedInstance] qimDB_getGroupMemberInfoByJid:[self getLastJid] WithGroupId:groupId];
    return memberInfo ? YES : NO;
}

- (BOOL)isGroupMemberByUserId:(NSString *)userId ByGroupId:(NSString *)groupId {
    NSDictionary *memberInfo = [[STDataMgr qimDB_SharedInstance] qimDB_getGroupMemberInfoByJid:userId WithGroupId:groupId];
    return memberInfo ? YES : NO;
}
...
+ (UIImage *)defaultGroupHeaderImage {
    ...
    dispatch_once(&onceToken, ^{
        if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeStartalk) {
...
- (void)dealWithSetUpdateMucVcard:(NSArray *)updateList {
    if ([updateList isKindOfClass:[NSArray class]]) {
        for (NSDictionary *groupInfo in updateList) {
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_updateGroup:groupId WithNickName:show_name WithTopic:muc_title WithDesc:muc_desc WithHeaderSrc:nil WithVersion:version];
...
- (void)updateGroupCard:(NSArray *)groupIds {
    ...
    dispatch_async(self.update_group_card, ^{
        NSArray *groupCardList = [[STDataMgr qimDB_SharedInstance] qimDB_getGroupVCardByGroupIds:groupIds];
            ...
            if (response.code == 200) {
                ...
                if (ret) {
                    ...
                    if (dataList.count > 0) {
                        ...
                        [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateGroupCards:dataList];
                        dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (response.code == 200) {
            ...
            if (ret) {
                ...
                if ([dataList isKindOfClass:[NSArray class]]) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateIncrementGroupCards:dataList];
                }
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (BOOL)isGroupOwner:(NSString *)groupId {
    ...
    NSDictionary *myInfoDic = [[STDataMgr qimDB_SharedInstance] qimDB_getGroupMemberInfoByJid:[self getLastJid] WithGroupId:groupId];
...
- (QIMGroupIdentity)GroupIdentityForUser:(NSString *)userId byGroup:(NSString *)groupId {
    ...
    NSDictionary *myInfoDic = [[STDataMgr qimDB_SharedInstance] qimDB_getGroupMemberInfoByJid:userId WithGroupId:groupId];
    NSString *affiliation = [myInfoDic objectForKey:@"affiliation"];
...
- (NSInteger)searchGroupTotalCountBySearchStr:(NSString *)searchStr {
    ...
    dispatch_block_t block = ^{
        totalCount = [[STDataMgr qimDB_SharedInstance] qimDB_getLocalGroupTotalCountByUserIds:@[searchStr]];
    };
    if (dispatch_get_specific(self.cacheTag))
        block();
    else
...
- (NSArray *)searchGroupBySearchStr:(NSString *)searchStr  WithLimit:(NSInteger)limit WithOffset:(NSInteger)offset {
    ...
    dispatch_block_t block = ^{
        array = [[STDataMgr qimDB_SharedInstance] qimDB_searchGroupByUserIds:@[searchStr] WithLimit:limit WithOffset:offset];
    };
    if (dispatch_get_specific(self.cacheTag))
        block();
    else
...
- (NSArray *)searchUserBySearchStr:(NSString *)searchStr notInGroup:(NSString *)groupId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_searchUserBySearchStr:searchStr notInGroup:groupId];
}
...
- (void)quickJoinAllGroup {
    if ([[STAppInfo sharedInstance] appType] != QIMProjectTypeQChat) {
        self.lastMaxGroupVersion = [[STDataMgr qimDB_SharedInstance] qimDB_getUserCacheDataWithKey:kGetIncrementMucListVersion withType:11];
...
- (void)checkGroupList:(NSArray *)groupList {
    ...
    for (NSDictionary *myGroup in oldGroupList) {
        ...
        if (![[groupList valueForKey:@"groupId"] containsObject:groupName]) {
            [[STDataMgr qimDB_SharedInstance] qimDB_deleteGroup:groupName];
            [self removeSessionById:groupName];
            [[STDataMgr qimDB_SharedInstance] qimDB_deleteGroupMemberWithGroupId:groupName];
        }
    }
}
...
- (NSDictionary *)getUserInfoByGroupName:(NSString *)groupName {
    ...
    dispatch_block_t block = ^{
        NSDictionary *tempDic = [[STDataMgr qimDB_SharedInstance] qimDB_selectUserByIndex:groupName];
        if (tempDic) {
...
- (void)dealWithIncrement_mucs:(NSArray *)newGroupList {
    ...
    if (updateGroupList.count > 0) {
        //更新群组
        [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertGroups:updateGroupList];
    }
    if (deleteGroupList.count > 0) {
        //删除群组
        [[STDataMgr qimDB_SharedInstance] qimDB_bulkDeleteGroups:deleteGroupList];
    }
    //删除session
    [[STDataMgr qimDB_SharedInstance] qimDB_deleteSessionList:deleteGroupList];
    
    [[STDataMgr qimDB_SharedInstance] qimDB_UpdateUserCacheDataWithKey:kGetIncrementMucListVersion withType:11 withValue:@"群列表时间戳" withValueInt:self.lastMaxGroupVersion];
}
