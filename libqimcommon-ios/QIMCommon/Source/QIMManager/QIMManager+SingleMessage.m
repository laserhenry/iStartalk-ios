...
- (void)updateLastMsgTime {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (errorTime > 0) {
            ...
            self.lastSingleMsgTime = [[STDataMgr qimDB_SharedInstance] qimDB_lastestMessageTime];
        }
        ...
        [[STUserCacheManager sharedInstance] setUserObject:@(self.lastSingleMsgTime) forKey:kGetNewSingleHistoryMsgError];
        QIMVerboseLog(@"强制塞本地单人消息时间戳到为 kGetNewSingleHistoryMsgError : %f完成", self.lastSingleMsgTime);
        
        QIMVerboseLog(@"强制塞本地单人消息消息时间戳完成之后再取一下本地错误时间戳 : %lld", [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewSingleHistoryMsgError] longLongValue]);
        
        long long defaultTime2 = ([[NSDate date] timeIntervalSince1970] - self.serverTimeDiff - 3600 * 24 * 3) * 1000;
        long long errorTime2 = [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewSingleReadFlagError] longLongValue];
        if (errorTime2 > 0) {
...
#warning 这里更新本地数据库本人已发送的消息状态
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateMessageReadStateWithMsg:data];
                QIMVerboseLog(@"移除已读未读状态时间戳");
                [[STUserCacheManager sharedInstance] removeUserObjectForKey:kGetNewSingleReadFlagError];
            } else {
                if (errcode == 5000) {
                ...
                [[STUserCacheManager sharedInstance] setUserObject:@(self.lastSingleReadFlagMsgTime) forKey:kGetNewSingleReadFlagError];
            }
        } else {
...
- (void)sendRecevieMessageState {
    NSArray *msgs = [[STDataMgr qimDB_SharedInstance] qimDB_getReceiveMsgIdListWithMsgReadFlag:QIMMessageRemoteReadStateDidReaded withChatType:ChatType_SingleChat withMsgDirection:QIMMessageDirection_Received];
    if (msgs.count > 0) {
...
- (BOOL)updateOfflineMessagesV2 {
    ...
    do {
        ...
        do {
            ...
            @autoreleasepool {
                ...
                if ([chatlog count] > 0) {
                    @autoreleasepool {
                        long long lastMaxTime = [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertHistoryChatJSONMsg:chatlog];
    ...
    if (!isSuccess) {
        ...
        [[STUserCacheManager sharedInstance] removeUserObjectForKey:kGetNewSingleHistoryMsgError];
    }
    return YES;
}
