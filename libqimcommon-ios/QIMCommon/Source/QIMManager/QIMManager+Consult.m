...
- (STMsgModel *)sendConsultMessageId:(NSString *)msgId WithMessage:(NSString *)msg WithInfo:(NSString *)info toJid:(NSString *)toJid realToJid:(NSString *)realToJid WithChatType:(ChatType)chatType WithMsgType:(int)msgType {
    ...
    if (msgRaw.length > 0) {
        [[STDataMgr qimDB_SharedInstance] qimDB_updateMessageWithMsgId:msgId WithMsgRaw:msgRaw];
    }
    return message;
}
...
- (void)getConsultServerMsgLisByUserId:(NSString *)userId WithVirtualId:(NSString *)virtualId WithLimit:(int)limit WithOffset:(int)offset withLoadMore:(BOOL)loadMore WithComplete:(void (^)(NSArray *))complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMgsListBySessionId:virtualId WithRealJid:userId WithLimit:limit WithOffset:offset];
        if (array.count > 0) {
            ...
            if (list.count < limit && loadMore == YES) {
                ...
                dispatch_async(self.load_history_msg, ^{
                    long long version = [[STDataMgr qimDB_SharedInstance] qimDB_getMinMsgTimeStampByXmppId:virtualId RealJid:userId] - timeChange;
                ...
                dispatch_async(self.load_history_msg, ^{
                    ...
                    [self getConsultServerlogWithFrom:[self getLastJid] virtualId:virtualId to:userId version:version count:limit direction:QIMMessageDirection_Sent withCallBack:^(NSArray *resultList) {
                        if (resultList.count > 0) {
                            ...
                            NSArray *datas = [[STDataMgr qimDB_SharedInstance] qimDB_getMgsListBySessionId:virtualId WithRealJid:userId WithLimit:limit WithOffset:offset];
