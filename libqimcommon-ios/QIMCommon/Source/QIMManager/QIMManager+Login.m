...
- (void)loginWithUserName:(NSString *)userName WithPassWord:(NSString *)pwd {
    ...
    [[STUserCacheManager sharedInstance] setCacheName:userFullJid];
    
    [[STUserCacheManager sharedInstance] setUserObject:[userName lowercaseString] forKey:kLastUserId];
    [[STUserCacheManager sharedInstance] setUserObject:pwd forKey:kTempPassword];
    QIMVerboseLog(@"登陆之前当前CacheName : %@", [[STUserCacheManager sharedInstance] cacheName]);
    ...
    NSDictionary *navConfig = [[STUserCacheManager sharedInstance] userObjectForKey:@"NavConfig"];
...
- (void)addUserCacheWithUserId:(NSString *)userId WithUserFullJid:(NSString *)userFullJid WithNavDict:(NSDictionary *)navDict {
    ...
    NSDictionary *userDict = [[STUserCacheManager sharedInstance] userObjectForKey:@"Users"];
    if ([userDict isKindOfClass:[NSDictionary class]]) {
        ...
        if (user) {
            ...
            [[STUserCacheManager sharedInstance] setUserObject:dict forKey:@"Users"];
        }
        QIMVerboseLog(@"已登录用户 : %@",[[STUserCacheManager sharedInstance] userObjectForKey:@"Users"]);
    } else {
...
- (void)clearUserToken {
    [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"kTempUserToken"];
    [[STUserCacheManager sharedInstance] removeUserObjectForKey:@"userToken"];
}
...
- (void)saveUserInfoWithName:(NSString *)userName passWord:(NSString *)pwd {
    ...
    [[STUserCacheManager sharedInstance] setUserObject:[userName lowercaseString] forKey:kLastUserId];
    [[STUserCacheManager sharedInstance] setUserObject:pwd forKey:kLastPassword];
}
...
- (void)QChatLoginWithUserId:(NSString *)userId rsaPassword:(NSString *)password type:(NSString *)type withCallback:(QIMKitGetQChatBetaLoginTokenDic)callback {
    ...
    [paramDic setObject:[[STAppInfo sharedInstance] deviceName] forKey:@"devicename"];
    [paramDic setObject:[[STAppInfo sharedInstance] appAID] forKey:@"usermac"];
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/get_power?v=%@&p=iphone", [[QIMNavConfigManager sharedInstance] httpHost], [[STAppInfo sharedInstance] AppBuildVersion]]];
...
- (void)getNewUserTokenWithUserName:(NSString *)userName WithPassword:(NSString *)password withCallback:(QIMKitGetUserNewTokenSuccessBlock)callback {
    ...
    NSString *rsaPwd = [STRSACoder encryptByRsa:password];
