    ...
    [[QIMManager sharedInstance] sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:[[QIMJSONSerializer sharedInstance] serializeObject:bodyProperties error:nil] withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode==0) {
            ...
            if ([data isKindOfClass:[NSArray class]]) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_insertFoundListWithAppVersion:[[STAppInfo sharedInstance] AppBuildVersion] withFoundList:rndataStr];
            }
        }
...
- (NSString *)getLocalFoundNavigation {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getFoundListWithAppVersion:[[STAppInfo sharedInstance] AppBuildVersion]];
}
