    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:bodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode==0) {
            ...
            if ([data isKindOfClass:[NSArray class]]) {
                NSLog(@"str : %@", data);
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertUserMedalsWithData:data];
...
- (void)userMedalStatusModifyWithStatus:(NSInteger)status withMedalId:(NSInteger)medalId withCallBack:(QIMKitUpdateMedalStatusCallBack)callback {
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:bodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_updateUserMedalStatus:data];
            if (callback) {
                callback(YES, nil);
            }
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)getRemoteUserMedalListWithUserId:(NSString *)userId {
    ...
    NSDictionary *bodyDic = @{/*@"userId":[[userId componentsSeparatedByString:@"@"] firstObject], @"host" : [[userId componentsSeparatedByString:@"@"] lastObject],*/ @"version":@([[STDataMgr qimDB_SharedInstance] qimDB_selectUserMedalStatusVersion])};
    NSData *bodyData = [[QIMJSONSerializer sharedInstance] serializeObject:bodyDic error:nil];
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:bodyData withSuccessCallBack:^(NSData *responseData) {
        NSDictionary *responseDic = [[QIMJSONSerializer sharedInstance] deserializeObject:responseData error:nil];
        BOOL ret = [[responseDic objectForKey:@"ret"] boolValue];
        ...
        if (ret && errcode == 0) {
            ...
            if ([userMedals isKindOfClass:[NSArray class]]) {
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertUserMedalList:userMedals];
            }
            long long version = [[data objectForKey:@"version"] longLongValue];
            [[STDataMgr qimDB_SharedInstance] qimDB_updateUserMedalStatusVersion:version];
        }
...
- (void)getRemoteMedalList {
    ...
    NSDictionary *bodyDic = @{@"version":@([[STDataMgr qimDB_SharedInstance] qimDB_selectMedalListVersion])};
    NSData *bodyData = [[QIMJSONSerializer sharedInstance] serializeObject:bodyDic error:nil];
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:bodyData withSuccessCallBack:^(NSData *responseData) {
        NSDictionary *responseDic = [[QIMJSONSerializer sharedInstance] deserializeObject:responseData error:nil];
        BOOL ret = [[responseDic objectForKey:@"ret"] boolValue];
        ...
        if (ret && errcode == 0) {
            ...
            if ([medalList isKindOfClass:[NSArray class]]) {
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertMedalList:medalList];
            }
            NSInteger version = [[data objectForKey:@"version"] integerValue];
            [[STDataMgr qimDB_SharedInstance] qimDB_updateMedalListVersion:version];
        }
        QIMVerboseLog(@"responseDic : %@", responseDic);
    } withFailedCallBack:^(NSError *error) {
...
- (NSDictionary *)getUserMedalWithMedalId:(NSInteger)medalId withUserId:(NSString *)userId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getUserMedalWithMedalId:medalId withUserId:userId];
}

- (NSArray *)getUserWearMedalStatusByUserid:(NSString *)userId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_selectUserWearMedalStatusByUserid:userId];
}

- (NSArray *)getUsersInMedal:(NSInteger)medalId withLimit:(NSInteger)limit withOffset:(NSInteger)offset {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getUsersInMedal:medalId withLimit:limit withOffset:offset];
}
