...
- (NSArray *)getSessionList {
    ...
    NSArray *sessionList = [[STDataMgr qimDB_SharedInstance] qimDB_getSessionListWithSingleChatType:ChatType_SingleChat];
    return sessionList;
}

- (NSArray *)getNotReadSessionList {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getNotReadSessionList];
}

- (NSArray *)getFullSessionList {
    NSArray *sessionList = [[STDataMgr qimDB_SharedInstance] qimDB_getFullSessionListWithSingleChatType:ChatType_SingleChat];
    return sessionList;
}
...
//    [self clearNotReadMsgByJid:sid];
    [[STDataMgr qimDB_SharedInstance] qimDB_deleteSession:sid];
...
- (void)removeConsultSessionById:(NSString *)sid RealId:(NSString *)realJid {
    [self clearNotReadMsgByGroupId:sid];
    [self clearNotReadMsgByJid:sid];
    [[STDataMgr qimDB_SharedInstance] qimDB_deleteSession:sid RealJid:realJid];
    [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationSessionListRemove object:sid];
}
...
- (ChatType)getChatSessionTypeByXmppId:(NSString *)xmppId {
    if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQChat) {
        ...
        if (isConsult == YES) {
            ...
            NSDictionary *chatInfo = [[STDataMgr qimDB_SharedInstance] qimDB_getChatSessionWithUserId:xmppId WithRealJid:xmppId];
            if (chatInfo) {
...
- (ChatType)openChatSessionByUserId:(NSString *)userId {
    if ([[STAppInfo sharedInstance] appType] == QIMProjectTypeQChat) {
        ...
        NSDictionary *sessionDic = [[STDataMgr qimDB_SharedInstance] qimDB_getChatSessionWithUserId:userId];
        if (sessionDic == nil) {
...
- (void)openGroupSessionByGroupId:(NSString *)groupId ByName:(NSString *)name {
    ...
    NSDictionary *sessionDic = [[STDataMgr qimDB_SharedInstance] qimDB_getChatSessionWithUserId:groupId];
    if (sessionDic == nil) {
...
- (void)addConsultSessionById:(NSString *)sessionId ByRealJid:(NSString *)realJid WithUserId:(NSString *)userId ByMsgId:(NSString *)msgId WithOpen:(BOOL)open WithLastUpdateTime:(long long)lastUpdateTime WithChatType:(ChatType)chatType{
    ...
    [[STDataMgr qimDB_SharedInstance] qimDB_insertSessionWithSessionId:sessionId WithUserId:userId WithLastMsgId:msgId WithLastUpdateTime:lastMsgTime ChatType:chatType WithRealJid:realJid];
...
- (void)addSessionByType:(ChatType)type ById:(NSString *)jid ByMsgId:(NSString *)msgId WithMsgTime:(long long)msgTime WithNeedUpdate:(BOOL)needUpdate {
    if (jid.length) {
        if (msgId == nil) {
            msgId = [[STDataMgr qimDB_SharedInstance] qimDB_getLastMsgIdByJid:jid];
        }
        if (msgTime <= 0) {
            msgTime = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgTimeWithMsgId:msgId];
        }
        ...
        [[STDataMgr qimDB_SharedInstance] qimDB_insertSessionWithSessionId:jid WithUserId:[[jid componentsSeparatedByString:@"@"] objectAtIndex:0] WithLastMsgId:msgId WithLastUpdateTime:lastMsgTime ChatType:type WithRealJid:jid];
        if (needUpdate) {
            dispatch_async(dispatch_get_main_queue(), ^{
