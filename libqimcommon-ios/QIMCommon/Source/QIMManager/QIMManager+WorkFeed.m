...
- (void)updateLastWorkFeedMsgTime {
    ...
    long long errorTime = [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewWorkFeedHistoryMessageListError] longLongValue];
    if (errorTime > 0) {
        self.lastWorkFeedMsgMsgTime = errorTime;
        QIMVerboseLog(@"本地驼圈错误时间戳 : %lld", errorTime);
    } else {
        self.lastWorkFeedMsgMsgTime = [[STDataMgr qimDB_SharedInstance] qimDB_getWorkNoticeMessagesMaxTime];
    }
    ...
    [[STUserCacheManager sharedInstance] setUserObject:@(self.lastWorkFeedMsgMsgTime) forKey:kGetNewWorkFeedHistoryMessageListError];
    QIMVerboseLog(@"强制塞本地驼圈消息时间戳到为 kGetNewWorkFeedHistoryMessageListError : %f完成", self.lastWorkFeedMsgMsgTime);
    
    QIMVerboseLog(@"强制塞本地驼圈消息消息时间戳完成之后再取一下本地错误时间戳 : %lld", [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewWorkFeedHistoryMessageListError] longLongValue]);
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:momentBodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            if ([momentDic isKindOfClass:[NSDictionary class]]) {
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertMoments:@[momentDic]];
        ...
        if (ret && errcode == 0) {
            ...
            if ([moments isKindOfClass:[NSDictionary class]]) {
                ...
                if ([deletePosts isKindOfClass:[NSArray class]]) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkdeleteMoments:deletePosts];
                }
                if ([newPosts isKindOfClass:[NSArray class]]) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertMoments:newPosts];
                    dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:momentBodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            if ([moments isKindOfClass:[NSDictionary class]]) {
                ...
                if ([newPosts isKindOfClass:[NSArray class]]) {
                    if (newPosts.count <= 0) {
                        [[STDataMgr qimDB_SharedInstance] qimDB_bulkdeleteMomentsWithXmppId:xmppId];
                    } else {
                        [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertMoments:newPosts];
                    }
                    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (ret && errcode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                if (isDeleteFlag == YES) {
                    ...
                    [[STDataMgr qimDB_SharedInstance] qimDB_deleteMomentWithRId:rId];
                }
            }
        }
    } withFailedCallBack:^(NSError *error) {
...
- (void)likeRemoteMomentWithMomentId:(NSString *)momentId withLikeFlag:(BOOL)likeFlag withCallBack:(QIMKitLikeMomentSuccessedBlock)callback {
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:momentBodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            NSDictionary *data = [result objectForKey:@"data"];
            if ([data isKindOfClass:[NSDictionary class]]) {
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMomentLike:@[data]];
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (callback) {
                        callback(data);
                    }
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:commentBodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMomentWithLikeNum:likeNum WithCommentNum:postCommentNum withPostId:[commentDic objectForKey:@"postUUID"]];
                NSArray *attachCommentList = [data objectForKey:@"attachCommentList"];
                if ([attachCommentList isKindOfClass:[NSArray class]]) {
                ...
                if ([deleteComments isKindOfClass:[NSArray class]]) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkDeleteComments:deleteComments];
                }
                NSArray *newComment = [data objectForKey:@"newComment"];
                if ([newComment isKindOfClass:[NSArray class]]) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertComments:newComment];
                    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (ret && errcode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                if ([deteleComments isKindOfClass:[NSArray class]]) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkDeleteComments:deteleComments];
                }
                NSArray *newComment = [data objectForKey:@"newComment"];
                if ([newComment isKindOfClass:[NSArray class]] && newComment.count > 0) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [[NSNotificationCenter defaultCenter] postNotificationName:kNotifyReloadWorkFeedCommentNum object:postCommentData];
                        [[STDataMgr qimDB_SharedInstance] qimDB_updateMomentWithLikeNum:likeNum WithCommentNum:postCommentNum withPostId:momentId];
                        if (callback) {
                            callback(newComment);
                        }
                    });
                } else {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkDeleteCommentsWithPostId:momentId];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (callback) {
                            callback(@[]);
                        }
        ...
        if (ret && errcode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                if ([newComment isKindOfClass:[NSArray class]] && newComment.count > 0) {
                    ...
                    //返回空，删除之前的历史
                    long long commentCreateTime = [[STDataMgr qimDB_SharedInstance] qimDB_getCommentCreateTimeWithCurCommentId:commentRId];
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkDeleteCommentsWithPostId:momentId withcurCommentCreateTime:commentCreateTime];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (callback) {
                            callback(@[]);
                        }
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:momentBodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                NSInteger postCommentNum = [[data objectForKey:@"postCommentNum"] integerValue];
                NSInteger likeNum = [[data objectForKey:@"postLikeNum"] integerValue];
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMomentWithLikeNum:likeNum WithCommentNum:postCommentNum withPostId:postUUId];
                ...
                if ([deleteCommentData isKindOfClass:[NSDictionary class]]) {
                    ...
                    if (isDeleteFlag == YES) {
                        ...
                        if (superParentStatus == 0) {
                            ...
                            [[STDataMgr qimDB_SharedInstance] qimDB_bulkDeleteComments:@[deleteCommentDic]];
                            dispatch_async(dispatch_get_main_queue(), ^{
                                if (callback) {
                            ...
                            //更新主评论为“该评论已被删除”
                            [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateComments:nil];
                            dispatch_async(dispatch_get_main_queue(), ^{
                                if (callback) {
                                    callback(YES, superParentStatus);
                                }
                            ...
                            [[STDataMgr qimDB_SharedInstance] qimDB_bulkDeleteCommentsAndAllChildComments:nil];
                            dispatch_async(dispatch_get_main_queue(), ^{
                                if (callback) {
                                    callback(YES, superParentStatus);
                                }
        ...
        if (ret && errorCode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                if (deleteComments.count > 0) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_deleteWorkNoticeMessageWithUUid:deleteComments];
                }
                NSArray *newComment = [data objectForKey:@"newComment"];
                if (newComment > 0) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertNoticeMessage:newComment];
                } else {
                    [[STDataMgr qimDB_SharedInstance] qimDB_deleteWorkNoticeMessageWithEventTypes:@[@(QIMWorkFeedNotifyTypeMyComment)]];
                }
                if (complete) {
        ...
        if (ret && errorCode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                if (deleteAtList.count > 0) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_deleteWorkNoticeMessageWithUUid:deleteAtList];
                }
                NSArray *newAtList = [data objectForKey:@"newAtList"];
                if (newAtList.count > 0) {
                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertNoticeMessage:newAtList];
                } else {
                    [[STDataMgr qimDB_SharedInstance] qimDB_deleteWorkNoticeMessageWithEventTypes:@[@(QIMWorkFeedNotifyTypePOSTAt), @(QIMWorkFeedNotifyTypeCommentAt)]];
                }
        ...
        if ([result isKindOfClass:[NSDictionary class]]) {
            ...
            if (ret && errcode == 0) {
                ...
                if ([data isKindOfClass:[NSDictionary class]]) {
                    ...
                    BOOL oldAuthSign = [[[STUserCacheManager sharedInstance] userObjectForKey:@"kUserWorkFeedEntrance"] boolValue];
                    [[STUserCacheManager sharedInstance] setUserObject:@(authSign) forKey:@"kUserWorkFeedEntrance"];
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                [[STUserCacheManager sharedInstance] setUserObject:@(useAble) forKey:@"VideoConfigUseAble"];
                [[STUserCacheManager sharedInstance] setUserObject:@(highDefinition) forKey:@"highDefinition"];
...
//    {@"messageTime":0, "user":"lilulucas.li", "userHost":"ejabhost1", "messageId":"123"}
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:noticeReadStateData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            [[STUserCacheManager sharedInstance] removeUserObjectForKey:kGetNewWorkFeedHistoryMessageListError];
            QIMVerboseLog(@"强制塞本地驼圈消息消息时间戳完成之后再取一下本地错误时间戳 : %lld", [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewWorkFeedHistoryMessageListError] longLongValue]);
        } else {
...
- (BOOL)getLocalWorkMomentNotifyConfig {
    BOOL exist = [[STDataMgr qimDB_SharedInstance] qimDB_checkExistUserCacheDataWithKey:kWorkMomentNotifySwitchConfig withType:12];
    if (exist) {
        return [[STDataMgr qimDB_SharedInstance] qimDB_getUserCacheDataWithKey:kWorkMomentNotifySwitchConfig withType:12];
    } else {
        return YES;
    }
}
    ...
    [self sendTPPOSTRequestWithUrl:destUrl withRequestBodyData:bodyData withSuccessCallBack:^(NSData *responseData) {
        ...
        if (ret && errcode == 0) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_UpdateUserCacheDataWithKey:kWorkMomentNotifySwitchConfig withType:12 withValue:@"驼圈开关" withValueInt:flag];
            }
        }
    } withFailedCallBack:^(NSError *error) {
...
- (NSDictionary *)getLastWorkMoment {
    NSDictionary *result = [[STDataMgr qimDB_SharedInstance] qimDB_getLastWorkMoment];
    return [self getLastWorkMomentWithDic:result];
}

- (NSDictionary *)getWorkMomentWithMomentId:(NSString *)momentId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getWorkMomentWithMomentId:momentId];
}
...
- (void)getWorkMomentWithLastMomentTime:(long long)lastMomentTime withUserXmppId:(NSString *)xmppId WithLimit:(int)limit WithOffset:(int)offset withFirstLocalMoment:(BOOL)firstLocal WithComplete:(void (^)(NSArray *))complete{
    if (firstLocal) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getWorkMomentWithXmppId:xmppId WithLimit:limit WithOffset:offset];
            if (array.count > 0) {
...
- (void)getWorkCommentWithLastCommentRId:(NSInteger)lastCommentRId withMomentId:(NSString *)momentId WithLimit:(int)limit WithOffset:(int)offset withFirstLocalComment:(BOOL)firstLocal WithComplete:(void (^)(NSArray *))complete{
    if (firstLocal) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getWorkCommentsWithMomentId:momentId WithLimit:limit WithOffset:offset];
            if (array.count > 0) {
                __block NSMutableArray *list = [NSMutableArray arrayWithArray:array];
                if (list.count >= limit) {
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (NSArray *)getWorkChildCommentsWithParentCommentUUID:(NSString *)parentCommentUUID {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getWorkChildCommentsWithParentCommentUUID:parentCommentUUID];
}

- (NSInteger)getWorkNoticeMessagesCountWithEventType:(NSArray *)eventTypes {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getWorkNoticeMessagesCountWithEventType:eventTypes];
}

- (BOOL)checkWorkMomentExistWithMomentId:(NSString *)momentId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_checkMomentWithMomentId:momentId];
}
...
- (NSArray *)getWorkNoticeMessagesWithLimit:(int)limit WithOffset:(int)offset eventTypes:(NSArray *)eventTypes readState:(int)readState {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getWorkNoticeMessagesWithLimit:limit WithOffset:offset eventTypes:eventTypes readState:readState];
}

//新加
- (NSArray *)getWorkNoticeMessagesWithLimit:(int)limit WithOffset:(int)offset eventTypes:(NSArray *)eventTypes {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getWorkNoticeMessagesWithLimit:limit WithOffset:offset eventTypes:eventTypes];
}

- (void)updateLocalWorkNoticeMsgReadStateWithTime:(long long)time {
    [[STDataMgr qimDB_SharedInstance] qimDB_updateWorkNoticeMessageReadStateWithTime:time];
}
