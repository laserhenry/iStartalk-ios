...
- (NSDictionary *)getCollectionUserInfoByUserId:(NSString *)myId {
    ...
    dispatch_block_t block = ^{
        ...
        if (!tempDic) {
            tempDic = [[STDataMgr qimDB_SharedInstance] qimDB_selectCollectionUserByJID:myId];
            if (tempDic) {
        ...
        [STHTTPClient sendRequest:request complete:^(QIMHTTPResponse *response) {
            if (response.code == 200) {
                ...
                if (ret) {
                    ...
                    if (list.count > 0) {
                        [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertCollectionUserCards:list];
...
- (void)saveCollectionMessage:(NSDictionary *)collectionMsgDic {
    [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertCollectionMsgWithMsgDics:@[collectionMsgDic]];
}

- (STMsgModel *)getCollectionMessageModelWithByDBMsgDic:(NSDictionary *)dbMsgDic {
    /*
...
- (STMsgModel *)getCollectionMsgListForMsgId:(NSString *)msgId {
    NSDictionary *infoDic = [[STDataMgr qimDB_SharedInstance] qimDB_getCollectionMsgListForMsgId:msgId];
    if (infoDic) {
...
- (NSArray *)getCollectionMsgListForUserId:(NSString *)userId originUserId:(NSString *)originUserId {
    NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getCollectionMsgListWithUserId:userId originUserId:originUserId];
    NSMutableArray *list = [NSMutableArray array];
    if (array.count > 0) {
        for (NSDictionary *infoDic in array) {
            STMsgModel *msg = [self getCollectionMessageModelWithByDBMsgDic:infoDic];
            [list addObject:msg];
        }
    }
    return list;
        ...
        [STHTTPClient sendRequest:request complete:^(QIMHTTPResponse *response) {
            if (response.code == 200) {
                ...
                if (ret) {
                    ...
                    if (data.count > 0) {
                        [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertCollectionAccountList:data];
                        [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertCollectionUserCards:data];
                        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)clearNotReadCollectionMsgByJid:(NSString *)jid {
    [[STDataMgr qimDB_SharedInstance] qimDB_updateCollectionMsgNotReadStateByJid:jid WithReadtate:QIMMessageRemoteReadStateDidReaded];
...
- (void)clearNotReadCollectionMsgByBindId:(NSString *)bindId WithUserId:(NSString *)userId {
    [[STDataMgr qimDB_SharedInstance] qimDB_updateCollectionMsgNotReadStateForBindId:bindId originUserId:userId WithReadState:QIMMessageRemoteReadStateDidReaded];
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *xmppId = [NSString stringWithFormat:@"collection_rbt@%@", [self getDomain]];
        [self.notReadMsgDic removeObjectForKey:xmppId];
        [[NSNotificationCenter defaultCenter] postNotificationName:kMsgNotReadCountChange object:@{@"XmppId":xmppId}];
...
- (NSInteger)getNotReadCollectionMsgCount {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getCollectionMsgNotReadCountByDidReadState:QIMMessageRemoteReadStateDidReaded];
}
...
- (NSInteger)getNotReadCollectionMsgCountByBindId:(NSString *)bindId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getCollectionMsgNotReadCountByDidReadState:QIMMessageRemoteReadStateDidReaded ForBindId:bindId];
}
...
- (NSInteger)getNotReadCollectionMsgCountByBindId:(NSString *)bindId WithUserId:(NSString *)userId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getCollectionMsgNotReadCountgetCollectionMsgNotReadCountByDidReadState:QIMMessageRemoteReadStateDidReaded ForBindId:bindId originUserId:userId];
}

#pragma mark - 代收Group

- (NSDictionary *)getCollectionGroupCardByGroupId:(NSString *)groupId {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getCollectionGroupCardByGroupId:groupId];
}
            ...
            if (response.code) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    ...
                    if (ret) {
                        NSArray *list = [result objectForKey:@"data"];
                        if (list.count > 0) {
                            [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertCollectionGroupCards:list];
                            dispatch_async(dispatch_get_main_queue(), ^{
