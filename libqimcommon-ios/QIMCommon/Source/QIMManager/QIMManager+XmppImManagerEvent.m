...
//                NSNumber *forceOldSearch = [notifyMsg objectForKey:@"forceOldSearch"];
                ...
                if ([[STDataMgr qimDB_SharedInstance] qimDB_getConfigVersion] < version && ![resource isEqualToString:[[XmppImManager sharedInstance] resource]]) {
                    [self getRemoteClientConfig];
                } else if (forceUpdate) {
                    QIMVerboseLog(@"强制更新个人配置信息");
                    [[STDataMgr qimDB_SharedInstance] qimDB_clearClientConfig];
                    [self getRemoteClientConfig];
                } else if (forceQuickReplyUpdate) {
                    QIMVerboseLog(@"强制更新快捷回复");
                    [[STDataMgr qimDB_SharedInstance] qimDB_clearQuickReplyGroup];
                    [[STDataMgr qimDB_SharedInstance] qimDB_clearQuickReplyContents];
                    ...
                    [[STUserCacheManager sharedInstance] setUserObject:forceOldSearchNumber forKey:@"forceOldSearch"];
                } else {
            ...
            case QIMCategoryNotifyMsgTypeTickUserWorkWorldNotice: {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkinsertNoticeMessage:@[onlineDict]];
...
- (void)onRevoke:(NSDictionary *)infoDic {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[STDataMgr qimDB_SharedInstance] qimDB_revokeMessageByMsgId:msgId WithContent:msg WithMsgType:QIMMessageType_Revoke];
    });
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)receiveGroupMsg:(NSDictionary *)msgDic {
    dispatch_async(self.receive_msg_queue, ^{
        ...
        // 消息已存在
        if ([[STDataMgr qimDB_SharedInstance] qimDB_checkMsgId:messageId]) {
            return;
        }
...
- (void)receivePublicNumberMsg:(NSDictionary *)msgDic {
    ...
    NSString *extendInfo = [msgDic objectForKey:@"extendInfo"];
    if ([[STDataMgr qimDB_SharedInstance] qimDB_checkPublicNumberMsgById:msgId]) {
        return;
    }
    BOOL isSystemMsg = NO;
    ...
    if (dic == nil) {
        ...
        [self updatePublicNumberCardByIds:@[@{@"robot_name": enName, @"version": @(0)}] WithNeedUpdate:YES withCallBack:^(NSArray *cardList) {
            if (cardList.count <= 0) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertPublicNumbers:@[dic]];
            }
        }];
        /*
    ...
    STMsgModel *c2bFeedBackMessage = [STMsgModel new];
    STMsgModel *message = [STMsgModel new];
    ...
    if (msgType != PublicNumberMsgType_Action && msgType != PublicNumberMsgType_ClientCookie && msgType != PublicNumberMsgType_PostBackCookie && msgType != QIMMessageType_ConsultResult) {
        ...
        [[STDataMgr qimDB_SharedInstance] qimDB_InsertOrUpdateUserInfos:@[userDic]];
        if (result) {
...
- (void)receiveCollectionOriginMessage:(NSDictionary *)msgDic {
    ...
    dispatch_async(self.receive_msg_queue, ^{
        ...
        BOOL isExistCollectionMsg = [[STDataMgr qimDB_SharedInstance] qimDB_checkCollectionMsgById:msgId];
...
- (void)receiveConsultMessage:(NSDictionary *)msgDic{
    ...
    dispatch_async(self.receive_msg_queue, ^{
        ...
        BOOL flag = [[STDataMgr qimDB_SharedInstance] qimDB_checkMsgId:msgId];
        if (flag) {
            return;
        }
...
- (void)onReadState:(NSDictionary *)infoDic {
    ...
    dispatch_async(self.receive_msgReadState_queue, ^{
        ...
        if (readType == QIMMessageReadFlagClearAllUnRead) {
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_updateAllMsgWithMsgRemoteState:remoteState ByMsgDirection:QIMMessageDirection_Received ByReadMarkT:readMarkT / 1000];
            [[STDataMgr qimDB_SharedInstance] qimDB_clearAtMessage];
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_UpdateUserCacheDataWithKey:kGetGroupReadMarkVersion withType:8 withValue:@"群阅读指针时间戳V2" withValueInt:maxMucReadMarkTime];
...
- (void)onMessageStateUpdate:(NSDictionary *)msgDic {
    dispatch_async(self.receive_msgSendState_queue, ^{
        ...
        BOOL check = [[STDataMgr qimDB_SharedInstance] qimDB_checkMsgId:msgId];
        if (check == YES) {
            ...
            if (msgSuccess) {
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMsgState:QIMMessageSendState_Success WithMsgId:msgId];
                
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMsgDate:receivedTime WithMsgId:msgId];
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMsgWithMsgRemoteState:QIMMessageRemoteReadStateNotSent ByMsgIdList:@[@{@"id":msgId}]];
                dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMsgState:QIMMessageSendState_Faild WithMsgId:msgId];
                [[STDataMgr qimDB_SharedInstance] qimDB_updateMsgWithMsgRemoteState:QIMMessageRemoteReadStateNotSent ByMsgIdList:@[@{@"id":msgId}]];
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)pbChatRoomAddMember:(NSDictionary *)infoDic{
    dispatch_async(self.update_group_member_queue, ^{
        ...
        if ([[STDataMgr qimDB_SharedInstance] qimDB_checkGroup:groupId] == NO) {
        ...
        if (memberJid) {
            [[STDataMgr qimDB_SharedInstance] qimDB_insertGroupMember:memberInfoDic WithGroupId:groupId];
        }
    });
}
...
- (void)pbChatRoomDestory:(NSDictionary *)infoDic{
    dispatch_async(self.receive_msg_queue, ^{
        ...
        [[STDataMgr qimDB_SharedInstance] qimDB_deleteMessageWithXmppId:groupId];
...
- (void)pbChatRoomCardUpdate:(NSDictionary *)infoDic{
    ...
    if (groupInfoDic) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateGroupCards:@[groupInfoDic]];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)friendPresence:(NSDictionary *)presenceDic {
    ...
    if ([result isEqualToString:@"success"]) {
        ...
        [[STDataMgr qimDB_SharedInstance] qimDB_insertFriendWithUserId:[infoDic objectForKey:@"UserId"]
