...
- (void)updateLastGroupMsgTime {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        long long errorTime = [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewGroupHistoryMsgError] longLongValue];
        if (errorTime > 0) {
            self.lastGroupMsgTime = errorTime;
            QIMVerboseLog(@"本地群消息错误时间戳 : %lld", errorTime);
        } else {
            self.lastGroupMsgTime = [[STDataMgr qimDB_SharedInstance] qimDB_lastestGroupMessageTime];
        }
        ...
        [[STUserCacheManager sharedInstance] setUserObject:@(self.lastGroupMsgTime) forKey:kGetNewGroupHistoryMsgError];
        QIMVerboseLog(@"强制塞本地群消息时间戳到为 kGetNewGroupHistoryMsgError : %f完成", self.lastGroupMsgTime);
        
        QIMVerboseLog(@"强制塞本地群消息时间戳完成之后再取一下本地错误时间戳 : %lld", [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewGroupHistoryMsgError] longLongValue]);
...
- (void)updateLastMaxMucReadMarkTime {
    ...
    NSInteger userMaxVersion = [[STDataMgr qimDB_SharedInstance] qimDB_getUserCacheDataWithKey:kGetGroupReadMarkVersion withType:8];
...
- (void)updateOfflineGroupMessages {
    ...
    if (!getMucHistotySuccess) {
        ...
        [[STUserCacheManager sharedInstance] removeUserObjectForKey:kGetNewGroupHistoryMsgError];
    }
}
...
- (void)dealWithGroupMsg:(NSArray * _Nonnull)data successed:(BOOL *)flag {
    ...
    if (data.count > 0) {
        ...
        long long lastTime = [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertIphoneHistoryGroupJSONMsg:data WithAtAllMsgList:&atAllMsgList WithNormaleAtMsgList:&normalMsgList];
        ...
        [STHTTPClient sendRequest:request complete:^(QIMHTTPResponse *response) {
            ...
            [autoManagerObject performSelectorInBackground:@selector(addCATTraceData:) withObject:logDic];
             ...
             if (errcode != 0 && result.count > 0 && response.code == 200) {
                 ...
                 [[STDataMgr qimDB_SharedInstance] qimDB_bulkUpdateGroupMessageReadFlag:mucData];
                 /*
