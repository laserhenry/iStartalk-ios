...
- (NSMutableDictionary *)conversationParamDic {
    ...
    if (!conversationParamDic) {
        conversationParamDic = [NSMutableDictionary dictionaryWithDictionary:[[STUserCacheManager sharedInstance] userObjectForKey:kConversationParamDic]];
    }
...
- (void)setAppendInfo:(NSDictionary *)appendInfoDict ForUserId:(NSString *)userId {
    if (appendInfoDict.count > 0 && userId) {
        ...
        [[STUserCacheManager sharedInstance] setUserObject:self.appendInfoDic forKey:kAppendInfoDic];
    }
}
...
- (void)setChannelInfo:(NSString *)channelId ForUserId:(NSString *)userId {
    if (channelId && userId) {
        ...
        [[STUserCacheManager sharedInstance] setUserObject:self.channelInfoDic forKey:kChannelInfoDic];
    }
}
...
- (void)setConversationParam:(NSDictionary *)param WithJid:(NSString *)jid {
    if (!self.conversationParamDic) {
        self.conversationParamDic = [NSMutableDictionary dictionaryWithDictionary:[[STUserCacheManager sharedInstance] userObjectForKey:kConversationParamDic]];
    }
    if (!jid || !param.count) {
        return;
    } else {
...
- (void)saveChatId:(NSString *)chatId ForUserId:(NSString *)userId {
    ...
    if (chatId.length > 0 && userId.length > 0) {
        ...
        if ([oldChatId isEqualToString:chatId] == NO) {
            ...
            [[STUserCacheManager sharedInstance] setUserObject:self.chatIdInfoDic forKey:kChatIdInfoDic];
        }
    }
}
 ...
 */
- (NSDictionary *)createMessageDictWithMessage:(STMsgModel *)message {
...
- (STMsgModel *)sendMessage:(STMsgModel *)msg ToUserId:(NSString *)userId {
    ...
    if (msgRaw.length > 0) {
        [[STDataMgr qimDB_SharedInstance] qimDB_updateMessageWithMsgId:msg.messageId WithMsgRaw:msgRaw];
    } else {
        ...
        [[STDataMgr qimDB_SharedInstance] qimDB_updateMessageWithMsgId:msg.messageId WithMsgRaw:msgRawStr];
    }
    return msg;
}
...
- (STMsgModel *)sendMessage:(NSString *)msg ToGroupId:(NSString *)groupId {
    ...
    if (msgRaw.length > 0) {
        [[STDataMgr qimDB_SharedInstance] qimDB_updateMessageWithMsgId:mesg.messageId WithMsgRaw:msgRaw];
    }
    return mesg;
}
...
- (NSArray *)getMsgsForMsgType:(QIMMessageType)msgType {
    NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgsByMsgType:msgType];
    NSMutableArray *list = [NSMutableArray array];
    for (NSDictionary *infoDic in array) {
        STMsgModel *msg = [self getMessageModelWithByDBMsgDic:infoDic];
        [list addObject:msg];
    }
    return list;
}
...
- (STMsgModel *)getMsgByMsgId:(NSString *)msgId {
    NSDictionary *infoDic = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgsByMsgId:msgId];
    if (infoDic.count > 0) {
        STMsgModel *msg = [self getMessageModelWithByDBMsgDic:infoDic];
        return msg;
    }
...
- (void)checkMsgTimeWithJid:(NSString *)jid WithRealJid:(NSString *)realJid WithMsgDate:(long long)msgDate WithGroup:(BOOL)flag withFrontInsert:(BOOL)frontInsert {
    ...
    if (msgDate - globalMsgDate.longLongValue >= 2 * 60 * 1000) {
        ...
        [msg setFrom:jid];
        [msg setXmppId:jid];
        [msg setMessageId:[[STDataMgr qimDB_SharedInstance] qimDB_getTimeSmtapMsgIdForDate:date WithUserId:key]];
        ...
        if ([[STDataMgr qimDB_SharedInstance] qimDB_checkMsgId:msg.messageId]) {
            return;
        }
...
- (void)checkMsgTimeWithJid:(NSString *)jid WithMsgDate:(long long)msgDate WithGroup:(BOOL)flag withFrontInsert:(BOOL)frontInsert {
    ...
    if (msgDate - globalMsgDate.longLongValue >= 2 * 60 * 1000) {
        ...
        STMsgModel *msg = [STMsgModel new];
        NSDate *date = [NSDate qim_dateWithTimeIntervalInMilliSecondSince1970:msgDate];
        [msg setMessageId:[[STDataMgr qimDB_SharedInstance] qimDB_getTimeSmtapMsgIdForDate:date WithUserId:jid]];
        [msg setRealJid:jid];
        [msg setXmppId:jid];
        [msg setFrom:jid];
        [msg setMessageType:QIMMessageType_Time];
...
- (NSInteger)getNotReadMsgCountByJid:(NSString *)jid WithRealJid:(NSString *)realJid withChatType:(ChatType)chatType {
    if (jid.length > 0 && realJid.length > 0) {
        NSInteger notReadCount = [[STDataMgr qimDB_SharedInstance] qimDB_getNotReaderMsgCountByJid:jid ByRealJid:realJid withChatType:chatType];
        return notReadCount;
    }
    return 0;
}
...
- (NSInteger)getNotReadMsgCountByJid:(NSString *)jid WithRealJid:(NSString *)realJid {
    if (jid.length > 0 && realJid.length > 0) {
        NSInteger notReadCount = [[STDataMgr qimDB_SharedInstance] qimDB_getNotReaderMsgCountByJid:jid ByRealJid:realJid];
        return notReadCount;
    }
    return 0;
}
...
- (NSInteger)getAppNotReaderCount {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getAppNotReadCount];
}
...
- (NSInteger)getNotRemindNotReaderCount {
    ...
    if (groupIdList.count > 0) {
        count = [[STDataMgr qimDB_SharedInstance] qimDB_getSumNotReaderMsgCountByXmppIds:groupIdList];
    }
...
- (void)clearSystemMsgNotReadWithJid:(NSString *)jid {
    ...
    [[STDataMgr qimDB_SharedInstance] qimDB_updateSystemMsgState:QIMMessageSendState_Success withReadState:QIMMessageRemoteReadStateDidReaded WithXmppId:jid];
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)clearNotReadMsgByJid:(NSString *)jid ByRealJid:(NSString *)realJid{
    ...
    NSArray *notReadMsgList = [[STDataMgr qimDB_SharedInstance] qimDB_getNotReadMsgListForUserId:jid ForRealJid:realJid];
...
- (void)clearNotReadMsgByJid:(NSString *)jid {
    ...
    NSArray *msgList = [[STDataMgr qimDB_SharedInstance] qimDB_getNotReadMsgListForUserId:jid];
    if (msgList.count > 0) {
...
- (void)clearNotReadMsgByGroupId:(NSString *)groupId {
    ...
    long long groupLastTime = [[STDataMgr qimDB_SharedInstance] qimDB_getMaxMsgTimeStampByXmppId:groupId ByRealJid:groupId];
...
- (void)updateAllLocalMsgWithMsgRemoteState:(NSInteger)remoteState ByReadMarkT:(long long)readMarkT {
    [[STDataMgr qimDB_SharedInstance] qimDB_updateAllMsgWithMsgRemoteState:remoteState ByMsgDirection:1 ByReadMarkT:readMarkT];
}
...
- (void)updateLocalGroupMessageRemoteState:(NSInteger)remoteState withXmppId:(NSString *)xmppId ByReadList:(NSArray *)readList {
    ...
    [[STDataMgr qimDB_SharedInstance] qimDB_clearAtMessageWithGroupId:xmppId];
    [[STDataMgr qimDB_SharedInstance] qimDB_updateGroupMessageRemoteState:remoteState ByGroupReadList:readList];
    dispatch_async(dispatch_get_main_queue(), ^{
//        [[NSNotificationCenter defaultCenter] postNotificationName:kMsgNotReadCountChange object:@{@"ForceRefresh":@(YES)}];
...
- (void)updateLocalMessageRemoteState:(NSInteger)remoteState withXmppId:(NSString *)xmppId withRealJid:(NSString *)realJid ByMsgIdList:(NSArray *)msgIdList {
    [[STDataMgr qimDB_SharedInstance] qimDB_updateMsgWithMsgRemoteState:remoteState ByMsgIdList:msgIdList];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
//存储消息
- (void)saveMsg:(STMsgModel *)msg ByJid:(NSString *)xmppId {
    ...
    if ([msg isKindOfClass:[NSString class]] == NO) {
        ...
        if (msg.chatType == ChatType_PublicNumber) {
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_insetPublicNumberMsgWithMsgId:messageId WithSessionId:xmppId WithFrom:from WithTo:to WithContent:content WithPlatform:0 WithMsgType:msgType WithMsgState:msgState WithMsgDirection:msgDirection WithMsgDate:msgDate WithReadedTag:QIMMessageSendState_Success];
        } else if (msg.chatType == ChatType_CollectionChat) {
            if (msg.messageType == QIMMessageType_Consult || msg.messageType == QIMMessageType_ConsultResult || msg.messageType == QIMMessageType_MicroTourGuide) {
                content = msg.extendInformation.length > 0 ? msg.extendInformation : msg.message;
            }
            NSDictionary *msgDic = [msg yy_modelToJSONObject];
            [[STDataMgr qimDB_SharedInstance] qimDB_insertMessageWithMsgDic:msgDic];
...
- (void)updateMsg:(STMsgModel *)msg ByJid:(NSString *)sid {
    ...
    if ([msg isKindOfClass:[NSString class]] == FALSE) {
        ...
        [[STDataMgr qimDB_SharedInstance] qimDB_updateMessageWithMsgId:messageId WithSessionId:sid WithFrom:from WithTo:to WithContent:content WithExtendInfo:extendInfo WithPlatform:0 WithMsgType:msgType WithMsgState:msgState WithMsgDirection:msgDirection WithMsgDate:msgDate WithReadedTag:0 ExtendedFlag:0 WithMsgRaw:msg.msgRaw];
    }
}

- (void)deleteMsg:(STMsgModel *)msg ByJid:(NSString *)sid {
    
    [[STDataMgr qimDB_SharedInstance] qimDB_deleteMessageByMessageId:msg.messageId ByJid:sid];
}

- (void)setMsgSentFaild{
    NSArray *msgIds = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgIdsForDirection:QIMMessageDirection_Sent WithMsgState:QIMMessageSendState_Waiting];
...
- (void)getMsgListByUserId:(NSString *)userId WithRealJid:(NSString *)realJid FromTimeStamp:(long long)timeStamp WithComplete:(void (^)(NSArray *))complete {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgListByXmppId:userId WithRealJid:realJid FromTimeStamp:timeStamp];
...
- (NSArray *)getNotReadMsgIdListByUserId:(NSString *)userId WithRealJid:(NSString *)realJid {
    NSArray *msgIdList = [[STDataMgr qimDB_SharedInstance] qimDB_getNotReadMsgListForUserId:userId ForRealJid:realJid];
    return msgIdList;
}

- (STMsgModel *)getMessageModelWithByDBMsgDic:(NSDictionary *)dbMsgDic {
    /*
    ...
    STMsgModel *msgModel = [STMsgModel new];
            ...
            if ([userId rangeOfString:@"@conference."].location != NSNotFound) {
                [self getMucMsgListWithGroupId:userId WithDirection:direction WithLimit:(lastUpdateTime < 0) ? (direction == 0 ? 20 : limit) : limit WithVersion:(lastUpdateTime < 0) ? (direction == 0 ? INT64_MAX : 0) : lastUpdateTime include:YES withCallBack:^(NSArray *resultList) {
                    if (resultList.count > 0) {
                        NSArray *datas = [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertIphoneMucPageJSONMsg:resultList withInsertDBFlag:NO];
                        NSMutableArray *list = [NSMutableArray array];
                ...
                [self getUserChatlogWithFrom:userId to:[self getLastJid] version:lastUpdateTime count:limit direction:direction include:YES withCallBack:^(NSArray *result) {
                    if (result.count > 0) {
                        NSArray *datas = [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertPageHistoryChatJSONMsg:result WithXmppId:userId withInsertDBFlag:NO];
                        NSMutableArray *list = [NSMutableArray array];
...
- (void)getMsgListByUserId:(NSString *)userId WithRealJid:(NSString *)realJid WithLimit:(int)limit WithOffset:(int)offset withLoadMore:(BOOL)loadMore WithComplete:(void (^)(NSArray *))complete{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMgsListBySessionId:userId WithRealJid:realJid WithLimit:limit WithOffset:offset];
        if (array.count > 0) {
            NSMutableArray *list = [NSMutableArray array];
            for (NSDictionary *infoDic in array) {
                STMsgModel *msg = [self getMessageModelWithByDBMsgDic:infoDic];
...
#pragma mark - 这里开始拉取群翻页消息
                        if (groupName) {
                            [self getMucMsgListWithGroupId:userId WithDirection:0 WithLimit:limit WithVersion:[[STDataMgr qimDB_SharedInstance] qimDB_getMinMsgTimeStampByXmppId:userId] include:NO withCallBack:^(NSArray *resultList) {
                                if (resultList.count > 0) {
                                    
                                    [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertIphoneMucPageJSONMsg:resultList];
                                }
                            }];
                        } else {
#pragma mark - 这里开始拉取单人翻页消息
                            [self getUserChatlogWithFrom:userId to:[self getLastJid] version:[[STDataMgr qimDB_SharedInstance] qimDB_getMinMsgTimeStampByXmppId:userId] count:limit direction:0 include:NO withCallBack:^(NSArray *result) {
                                if (result.count > 0) {
                                    NSArray *datas = [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertPageHistoryChatJSONMsg:result WithXmppId:userId];
                ...
                dispatch_async(self.load_history_msg, ^{
                    ...
                    if ([userId rangeOfString:@"@conference."].location != NSNotFound) {
                        ...
                        [self getMucMsgListWithGroupId:userId WithDirection:direction WithLimit:version < 0 ? (direction == 0 ? 20 : limit) : limit WithVersion:version < 0 ? (direction == 0 ? INT64_MAX : 0) : version include:NO withCallBack:^(NSArray *resultList) {
                            if (resultList.count > 0) {
                                NSArray *datas = [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertIphoneMucPageJSONMsg:resultList];
                                NSMutableArray *list = [NSMutableArray array];
                                for (NSDictionary *infoDic in datas) {
                                    STMsgModel *msg = [self getMessageModelWithByDBMsgDic:infoDic];
                                    [list addObject:msg];
...
- (NSMutableArray *)searchLocalMessageByKeyword:(NSString *)keyWord XmppId:(NSString *)xmppid RealJid:(NSString *)realJid {
    return [[STDataMgr qimDB_SharedInstance] qimDB_searchLocalMessageByKeyword:keyWord XmppId:xmppid RealJid:realJid];
}

- (NSArray *)getLocalMediasByXmppId:(NSString *)xmppId ByRealJid:(NSString *)realJid {
    NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getLocalMediaByXmppId:xmppId ByReadJid:realJid];
    NSMutableArray *list = [NSMutableArray arrayWithCapacity:2];
    for (NSDictionary *msgInfoDic in array) {
        STMsgModel *msg = [self getMessageModelWithByDBMsgDic:msgInfoDic];
        [list addObject:msg];
    }
    return list;
}
...
- (NSArray *)getMsgsForMsgType:(NSArray *)msgTypes ByXmppId:(NSString *)xmppId ByReadJid:(NSString *)realJid {
    NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgsByMsgType:msgTypes ByXmppId:xmppId ByReadJid:realJid];
    NSMutableArray *list = [NSMutableArray array];
    for (NSDictionary *msgInfoDic in array) {
        STMsgModel *msg = [self getMessageModelWithByDBMsgDic:msgInfoDic];
        [list addObject:msg];
...
- (NSArray *)getMsgsByKeyWord:(NSString *)keyWork ByXmppId:(NSString *)xmppId ByReadJid:(NSString *)realJid {
    NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMsgsByKeyWord:keyWork ByXmppId:xmppId ByReadJid:realJid];
    NSMutableArray *list = [NSMutableArray array];
    for (NSDictionary *msgInfoDic in array) {
        STMsgModel *msg = [self getMessageModelWithByDBMsgDic:msgInfoDic];
        [list addObject:msg];
