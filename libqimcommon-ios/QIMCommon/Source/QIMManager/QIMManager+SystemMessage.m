...
- (void)checkHeadlineMsg {
    long long errorTime = [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewSystemHistoryMsgError] longLongValue];
...
- (void)updateLastSystemMsgTime {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (errorTime > 0) {
            ...
            self.lastSystemMsgTime = [[STDataMgr qimDB_SharedInstance] qimDB_lastestSystemMessageTime];
        }
        ...
        [[STUserCacheManager sharedInstance] setUserObject:@(self.lastSystemMsgTime) forKey:kGetNewSystemHistoryMsgError];
        QIMVerboseLog(@"强制塞本地HeadLine消息时间戳到为 kGetNewSystemHistoryMsgError : %f完成", self.lastSystemMsgTime);
        QIMVerboseLog(@"强制塞本地HeadLine消息消息时间戳完成之后再取一下本地错误时间戳 : %lld", [[[STUserCacheManager sharedInstance] userObjectForKey:kGetNewSystemHistoryMsgError] longLongValue]);
...
- (void)getSystemMsgLisByUserId:(NSString *)userId WithFromHost:(NSString *)fromHost WithLimit:(int)limit WithOffset:(int)offset withLoadMore:(BOOL)loadMore WithComplete:(void (^)(NSArray *))complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *array = [[STDataMgr qimDB_SharedInstance] qimDB_getMgsListBySessionId:userId WithRealJid:userId WithLimit:limit WithOffset:offset];
        if (array.count > 0) {
...
- (void)dealWithSystemMessages:(NSArray *)systemMsgs {
    ...
    long long lastMaxTime = [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertHistoryChatJSONMsg:systemMsgs];
