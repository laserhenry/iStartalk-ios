...
@implementation STManager (ClientConfig)
    ...
    switch (type) {
        case QIMClientConfigTypeKCollectionCacheKey: {
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:kNotifyResetFaceCollectionManager object:[[STManager sharedInstance] getClientConfigValueArrayWithType:QIMClientConfigTypeKCollectionCacheKey]];
            });
        }
            break;
        default: {
...
- (NSInteger)getClientConfigDeleteFlagWithType:(QIMClientConfigType)type WithSubKey:(NSString *)subKey {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigDeleteFlagWithConfigKey:[self transformClientConfigKeyWithType:type] WithSubKey:subKey];
}
...
- (NSString *)getClientConfigInfoWithType:(QIMClientConfigType)type WithSubKey:(NSString *)subKey WithDeleteFlag:(BOOL)deleteFlag {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigInfoWithConfigKey:[self transformClientConfigKeyWithType:type] WithSubKey:subKey WithDeleteFlag:deleteFlag];
}
...
- (NSDictionary *)getClientConfigDicWithType:(QIMClientConfigType)type WithDeleteFlag:(BOOL)deleteFlag {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigDicWithConfigKey:[self transformClientConfigKeyWithType:type] WithDeleteFlag:deleteFlag];
}
...
- (NSArray *)getClientConfigInfoArrayWithType:(QIMClientConfigType *)type WithDeleteFlag:(BOOL)deleteFlag {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigInfoArrayWithConfigKey:[self transformClientConfigKeyWithType:type] WithDeleteFlag:deleteFlag];
}
...
- (NSArray *)getClientConfigValueArrayWithType:(QIMClientConfigType)type WithDeleteFlag:(BOOL)deleteFlag {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigValueArrayWithConfigKey:[self transformClientConfigKeyWithType:type] WithDeleteFlag:deleteFlag];
}
...
#warning 循环中尽可能避免操作db，汇总之后批量插入
    ...
    for (NSDictionary *configInfo in data) {
        ...
        [[STDataMgr qimDB_SharedInstance] qimDB_bulkInsertConfigArrayWithConfigKey:key WithConfigVersion:version ConfigArray:configInfoData];
        if ([key isEqualToString:@"kStickJidDic"]) {
            ...
            [[STUserCacheManager sharedInstance] setUserObject:colorConfigInfoDic forKey:kChatColorInfo];
            ...
            [[STUserCacheManager sharedInstance] setUserObject:fontConfigInfoDic forKey:kCurrentFontInfo];
            dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (NSDictionary *markupInfo in configInfoData) {
                ...
                if ([subkey isEqualToString:@"ios"]) {
                    self.soundName = [[STManager sharedInstance] getClientNotificationSoundName];
                }
            }
        } else {
...
- (void)updateRemoteClientConfigWithType:(QIMClientConfigType)type BatchProcessConfigInfo:(NSArray *)configInfoArray WithDel:(BOOL)delFlag withCallback:(QIMKitUpdateRemoteClientConfig)callback {
    ...
    [bodyProperties setQIMSafeObject:[STManager getLastUserName] forKey:@"username"];
    [bodyProperties setQIMSafeObject:[[STManager sharedInstance] getDomain] forKey:@"host"];
    ...
    [bodyProperties setQIMSafeObject:@([[STDataMgr qimDB_SharedInstance] qimDB_getConfigVersion]) forKey:@"version"];
...
- (NSMutableArray *)selectStarOrBlackContacts:(NSString *)pkey {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigArrayStarOrBlackContacts:pkey];
}

//查询不在星标用户的好友
- (NSMutableArray *)selectFriendsNotInStarContacts {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigArrayFriendsNotInStarContacts];
}

//搜索不在星标里面的用户
- (NSMutableArray *)selectUserNotInStartContacts:(NSString *)key {
    return [[STDataMgr qimDB_SharedInstance] qimDB_getConfigArrayUserNotInStartContacts:key];
}

- (BOOL)isStarOrBlackContact:(NSString *)subkey ConfigKey:(NSString *)pkey {
    if([@"kStarContact" isEqualToString:pkey]){
        return [[STManager sharedInstance] getClientConfigDeleteFlagWithType:QIMClientConfigTypeKStarContact WithSubKey:subkey] == 0;
    }else if([@"kBlackList" isEqualToString:pkey]){
        return [[STManager sharedInstance] getClientConfigDeleteFlagWithType:QIMClientConfigTypeKBlackList WithSubKey:subkey] == 0;
    }else{
        return NO;
    }
}
...
- (void)setStarOrblackContact:(NSString *)subkey ConfigKey:(NSString *)pkey Flag:(BOOL)value withCallback:(QIMKitUpdateRemoteClientConfig)callback {
    if([@"kStarContact" isEqualToString:pkey]){
        [[STManager sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKStarContact WithSubKey:subkey WithConfigValue:value?@"1":@"0" WithDel:!value withCallback:callback];
    } else if([@"kBlackList" isEqualToString:pkey]){
        [[STManager sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKBlackList WithSubKey:subkey WithConfigValue:value?@"1":@"0" WithDel:!value withCallback:callback];
    } else{
...
- (void)setStarOrblackContacts:(NSDictionary *)map ConfigKey:(NSString *)pkey Flag:(BOOL)value withCallback:(QIMKitUpdateRemoteClientConfig)callback {
    ...
    [[STManager sharedInstance] updateRemoteClientConfigWithType:configType BatchProcessConfigInfo:deleteStickList WithDel:!value withCallback:callback];
}

- (NSString *)getClientNotificationSoundName {
    return [[STManager sharedInstance] getClientConfigInfoWithType:QIMClientConfigTypeKNotificationSound WithSubKey:@"ios"];
}

- (void)setClientNotificationSound:(NSString *)soundName withCallback:(QIMKitUpdateRemoteClientConfig)callback {
    [[STManager sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKNotificationSound WithSubKey:@"ios" WithConfigValue:soundName WithDel:NO withCallback:callback];
}
