...
static STDataMgr *__global_data_manager = nil;
@interface STDataMgr()
@end

@implementation STDataMgr{
    NSString *_dbPath;
...
+ (STDataMgr *) qimDB_sharedInstanceWithDBPath:(NSString *)dbPath withDBFullJid:(NSString *)dbOwnerFullJid {
    ...
    dispatch_once(&_onceDBToken, ^{
        __global_data_manager = [[STDataMgr alloc] initWithDBPath:dbPath];
...
- (void)initSQLiteLog {
    STDBLogger *sqliteLogger = [[STDBLogger alloc] initWithLogDirectory:[self sqliteLogFilesDirectory] WithDBOperator:[self dbInstance]];
