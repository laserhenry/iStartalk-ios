...
@implementation STDataMgr (QIMSession)
...
- (long long)qimDB_insertSessionWithMsgList:(NSDictionary *)msgLists {
    ...
    for (NSString *key in [msgLists allKeys]) {
        ...
        if (isConsult) {
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_insertSessionWithSessionId:userId WithUserId:userId WithLastMsgId:userId WithLastUpdateTime:msgTime ChatType:chattype WithRealJid:realJid];
        } else {
            if ([key containsString:@"collection_rbt"]) {
                [[STDataMgr qimDB_SharedInstance] qimDB_insertSessionWithSessionId:key WithUserId:[[key componentsSeparatedByString:@"@"] objectAtIndex:0] WithLastMsgId:msgId WithLastUpdateTime:msgTime ChatType:ChatType_CollectionChat WithRealJid:key];
            } else {
                [[STDataMgr qimDB_SharedInstance] qimDB_insertSessionWithSessionId:key WithUserId:[[key componentsSeparatedByString:@"@"] objectAtIndex:0] WithLastMsgId:msgId WithLastUpdateTime:msgTime ChatType:chattype WithRealJid:key];
            }
        }
...
- (long long)qimDB_insertGroupSessionWithMsgList:(NSDictionary *)tempGroupDic {
    ...
    for (NSString *groupId in tempGroupDic) {
        if (groupId.length > 0) {
            ...
            [[STDataMgr qimDB_SharedInstance] qimDB_insertSessionWithSessionId:groupId WithUserId:[[groupId componentsSeparatedByString:@"@"] objectAtIndex:0] WithLastMsgId:messageId WithLastUpdateTime:msgDate ChatType:ChatType_GroupChat WithRealJid:groupId];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationOfflineMessageUpdate" object:key userInfo:nil];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"kNotificationSessionListUpdate" object:nil];
...
- (NSDictionary *)qimDB_getLastedSingleChatSession {
    ...
    [[self dbInstance] inDatabase:^(QIMDataBase* _Nonnull database) {
        ...
        if ([reader read]) {
            ...
            [STDataMgr safeSaveForDic:result setObject:xmppId forKey:@"XmppId"];
            [STDataMgr safeSaveForDic:result setObject:userId forKey:@"UserId"];
            [STDataMgr safeSaveForDic:result setObject:chatType forKey:@"ChatType"];
        }
        [reader close];
    }];
    QIMVerboseLog(@"");
...
- (NSArray *)qimDB_getFullSessionListWithSingleChatType:(int)singleChatType {
    ...
    [[self dbInstance] inDatabase:^(QIMDataBase* _Nonnull database) {
        ...
        if ([pReader read]) {
            ...
            [STDataMgr safeSaveForDic:dic setObject:[pReader objectForColumnIndex:0] forKey:@"XmppId"];
            [STDataMgr safeSaveForDic:dic setObject:[pReader objectForColumnIndex:1] forKey:@"Name"];
            [STDataMgr safeSaveForDic:dic setObject:[pReader objectForColumnIndex:2] forKey:@"Content"];
            [STDataMgr safeSaveForDic:dic setObject:[pReader objectForColumnIndex:3] forKey:@"MsgType"];
            [STDataMgr safeSaveForDic:dic setObject:[pReader objectForColumnIndex:4] forKey:@"MsgDateTime"];
            [STDataMgr safeSaveForDic:dic setObject:@(ChatType_PublicNumber) forKey:@"ChatType"];
        ...
        while ([reader read]) {
            ...
            if (added == NO && msgDateTime && msgDateTime.longLongValue < pMaxLastTime.longLongValue) {
                ...
                [STDataMgr safeSaveForDic:sessionDic setObject:xmppId forKey:@"XmppId"];
                [STDataMgr safeSaveForDic:sessionDic setObject:userId forKey:@"UserId"];
                [STDataMgr safeSaveForDic:sessionDic setObject:name forKey:@"Name"];
                [STDataMgr safeSaveForDic:sessionDic setObject:headerSrc forKey:@"HeaderSrc"];
                [STDataMgr safeSaveForDic:sessionDic setObject:lastMsgId forKey:@"LastMsgId"];
                [STDataMgr safeSaveForDic:sessionDic setObject:content forKey:@"Content"];
                [STDataMgr safeSaveForDic:sessionDic setObject:msgType forKey:@"MsgType"];
                [STDataMgr safeSaveForDic:sessionDic setObject:msgState forKey:@"MsgState"];
                [STDataMgr safeSaveForDic:sessionDic setObject:msgDirection forKey:@"MsgDirection"];
                [STDataMgr safeSaveForDic:sessionDic setObject:msgDateTime forKey:@"MsgDateTime"];
                [STDataMgr safeSaveForDic:sessionDic setObject:chatType forKey:@"ChatType"];
                [STDataMgr safeSaveForDic:sessionDic setObject:nickName forKey:@"NickName"];
                [STDataMgr safeSaveForDic:sessionDic setObject:realJid forKey:@"RealJid"];
    ...
    [[self dbInstance] inDatabase:^(QIMDataBase* _Nonnull database) {
        ...
        while ([reader read]) {
            ...
            if (added == NO && msgDateTime && msgDateTime.longLongValue < pMaxLastTime.longLongValue) {
                ...
                [STDataMgr safeSaveForDic:sessionDic setObject:realJid forKey:@"RealJid"];
                [STDataMgr safeSaveForDic:sessionDic setObject:msgDateTime forKey:@"MsgDateTime"];
                [STDataMgr safeSaveForDic:sessionDic setObject:stickState forKey:@"StickState"];
                [STDataMgr safeSaveForDic:sessionDic setObject:reminded forKey:@"Reminded"];
                [STDataMgr safeSaveForDic:sessionDic setObject:markUpName forKey:@"MarkUpName"];
                [STDataMgr safeSaveForDic:sessionDic setObject:msgFrom forKey:@"MsgFrom"];
                [STDataMgr safeSaveForDic:sessionDic setObject:unreadCount forKey:@"UnreadCount"];
                [result addObject:sessionDic];
            }
        }
...
- (NSDictionary *)qimDB_getChatSessionWithUserId:(NSString *)userId chatType:(int)chatType{
    ...
    [[self dbInstance] inDatabase:^(QIMDataBase* _Nonnull database) {
        ...
        if ([reader read]) {
            ...
            [STDataMgr safeSaveForDic:chatSession setObject:xmppId forKey:@"XmppId"];
            [STDataMgr safeSaveForDic:chatSession setObject:userId forKey:@"UserId"];
            [STDataMgr safeSaveForDic:chatSession setObject:name forKey:@"Name"];
            [STDataMgr safeSaveForDic:chatSession setObject:headerSrc forKey:@"HeaderSrc"];
            [STDataMgr safeSaveForDic:chatSession setObject:lastMsgId forKey:@"LastMsgId"];
            [STDataMgr safeSaveForDic:chatSession setObject:content forKey:@"Content"];
            [STDataMgr safeSaveForDic:chatSession setObject:msgType forKey:@"MsgType"];
            [STDataMgr safeSaveForDic:chatSession setObject:msgState forKey:@"MsgState"];
            [STDataMgr safeSaveForDic:chatSession setObject:msgDirection forKey:@"MsgDirection"];
            [STDataMgr safeSaveForDic:chatSession setObject:msgDateTime forKey:@"MsgDateTime"];
            [STDataMgr safeSaveForDic:chatSession setObject:chatType forKey:@"ChatType"];
            [STDataMgr safeSaveForDic:chatSession setObject:nickName forKey:@"NickName"];
        }
        [reader close];
    }];
    QIMVerboseLog(@"");
...
- (NSDictionary *)qimDB_getChatSessionWithUserId:(NSString *)userId WithRealJid:(NSString *)realJid {
    ...
    [[self dbInstance] inDatabase:^(QIMDataBase* _Nonnull database) {
        ...
        if ([reader read]) {
            ...
            [STDataMgr safeSaveForDic:chatSession setObject:userId forKey:@"UserId"];
            [STDataMgr safeSaveForDic:chatSession setObject:lastMsgId forKey:@"LastMsgId"];
            [STDataMgr safeSaveForDic:chatSession setObject:lastUpdateTime forKey:@"LastUpdateTime"];
            [STDataMgr safeSaveForDic:chatSession setObject:chatType forKey:@"ChatType"];
            [STDataMgr safeSaveForDic:chatSession setObject:realJid forKey:@"RealJid"];
        }
        [reader close];
    }];
    QIMVerboseLog(@"");
