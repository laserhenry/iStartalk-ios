...
- (void)touchVerification {
    ...
    if (str.length > 0) {
        ...
        [[STKit sharedInstance] setUserObject:@([NSDate timeIntervalSinceReferenceDate]) forKey:timeAkey];
...
- (void)setUpUI {
    ...
    NSString *timeAkey = [NSString stringWithFormat:@"%ld_%@", (long)self.model.q_id, self.model.q_title];
    NSInteger lastVerficationTime = [[[STKit sharedInstance] userObjectForKey:timeAkey] integerValue];
    ...
    if (nowTime - lastVerficationTime > securitySettingTime || !pwd) {
        ...
        [[STKit sharedInstance] removeUserObjectForKey:timeAkey];
