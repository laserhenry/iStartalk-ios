...
- (NSMutableArray *)randomTexts {
    if (!_randomTexts) {
        ...
        for (NSDictionary *dict in randomDicts) {
            ...
            if ([[[STKit sharedInstance] currentLanguage] containsString:@"zh-Hant"]) {
                randomTextStr = [dict objectForKey:@"zh-Hant"];
                [_randomTexts addObject:randomTextStr];
            } else if ([[[STKit sharedInstance] currentLanguage] containsString:@"en"]) {
                randomTextStr = [dict objectForKey:@"en"];
                [_randomTexts addObject:randomTextStr];
            } else if ([[[STKit sharedInstance] currentLanguage] containsString:@"zh-Hans"]) {
...
- (void)setRemindTime:(id)sender {
    ...
    STWSDatePickerView *datePicker = [[STWSDatePickerView alloc] initWithDateStyle:DateStyleShowYearMonthDayHourMinute CompleteBlock:^(NSDate *completeDate) {
