...
- (void)updateUserMedalList:(NSNotification *)notify {
    ...
    NSDictionary *userMedalListDic = @{@"UserId" : [[STKit sharedInstance] getLastJid], @"medalList": userMedallist ? userMedallist : @[]};
...
- (void)updateMyPersonalSignature:(NSNotification *)notify {
    ...
    [[QimRNBModule getStaticCacheBridge].eventDispatcher sendAppEventWithName:@"updatePersonalSignature" body:@{@"UserId":[[STKit sharedInstance] getLastJid], @"PersonalSignature":(personalSignature.length > 0) ? personalSignature : @""}];
}
