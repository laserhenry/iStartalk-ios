...
//
//  CopyRight © 2021Startalk LTD.
//
//

//
// All Entries of 'Me' button here
//
...
RCT_EXPORT_METHOD(getTOTP:(RCTResponseSenderBlock)success) {
    NSString *secret = [NSString stringWithFormat:@"u=%@&k=%@",[[STKit sharedInstance] getLastJid],[[STKit sharedInstance] myRemotelogginKey]];
    ...
    TOTPGenerator *generator = [[TOTPGenerator alloc] initWithSecret:secretData
                                                           algorithm:kOTPGeneratorSHA1Algorithm
                                                              digits:6
                                                              period:30];
    long long time = [[NSDate date] timeIntervalSince1970] - [[STKit sharedInstance] getServerTimeDiff];
...
RCT_EXPORT_METHOD(appConfig:(RCTResponseSenderBlock)success) {
    ...
    NSInteger projectType = ([STKit getQIMProjectType] != QIMProjectTypeQChat) ? 0 : 1;
    NSString *ckey = [[STKit sharedInstance] thirdpartKeywithValue];
    NSString *ip = @"0.0.0.0";
    NSString *userId = [STKit getLastUserName];
    NSString *httpHost = [[STKit sharedInstance] qimNav_Javaurl];
    BOOL WorkFeedEntrance = [[[STKit sharedInstance] userObjectForKey:@"kUserWorkFeedEntrance"] boolValue];
    BOOL isShowRedPackage = YES;
    BOOL isEasyTrip = NO;
    BOOL notNeedShowEmailInfo = ([[STKit sharedInstance] qimNav_Email].length > 0) ? NO : YES;
    BOOL notNeedShowLeaderInfo = ([[STKit sharedInstance] qimNav_LeaderUrl].length > 0) ? NO : YES;
    BOOL notNeedShowMobileInfo = ([[STKit sharedInstance] qimNav_Mobileurl].length > 0) ? NO : YES;
    BOOL notNeedShowCamelNotify = NO;
    BOOL isToCManager = NO;
    if([[STKit sharedInstance] userObjectForKey:@"isToCManager"]){
        isToCManager = [[[STKit sharedInstance] userObjectForKey:@"isToCManager"] boolValue];
    }
    if ([STKit getQIMProjectType] == QIMProjectTypeStartalk) {
        isShowRedPackage = NO;
        isEasyTrip = YES;
        notNeedShowCamelNotify = YES;
    } else {
        isEasyTrip = [[[STKit sharedInstance] getDomain] isEqualToString:@"ejabhost1"] ? NO : YES;
        notNeedShowCamelNotify = [[[STKit sharedInstance] getDomain] isEqualToString:@"ejabhost1"] ? NO : YES;
    }
    BOOL oldAuthSign = [[[STKit sharedInstance] userObjectForKey:@"kUserWorkFeedEntrance"] boolValue];
    BOOL checkAuthSignKey = [[STKit sharedInstance] containsObjectForKey:@"kUserWorkFeedEntrance"];
    if (checkAuthSignKey == NO) {
        oldAuthSign = YES;
    }
    NSArray *appConfig = @[@{@"projectType" : @(projectType), @"isQtalk" : @(!projectType), @"nativeAppType" : @([STKit getQIMProjectType]), @"ckey" : ckey,@"clientIp" : ip,@"userId" : userId,@"domain" : [[STKit sharedInstance] qimNav_Domain]?[[STKit sharedInstance] qimNav_Domain]:@"", @"httpHost" : httpHost, @"RNAboutView" : @(0), @"RNMineView": @([[STKit sharedInstance] qimNav_RNMineView]), @"RNGroupCardView": @([[STKit sharedInstance] qimNav_RNGroupCardView]), @"RNContactView": @([[STKit sharedInstance] qimNav_RNContactView]), @"RNSettingView" : @([[STKit sharedInstance] qimNav_RNSettingView]), @"RNUserCardView" : @([[STKit sharedInstance] qimNav_RNUserCardView]), @"RNGroupListView": @([[STKit sharedInstance] qimNav_RNGroupListView]), @"RNPublicNumberListView" : @([[STKit sharedInstance] qimNav_RNPublicNumberListView]), @"showOrganizational" : @([[STKit sharedInstance] qimNav_ShowOrganizational]), @"showOA" : @([[STKit sharedInstance] qimNav_ShowOA]), @"qcAdminHost": [[STKit sharedInstance] qimNav_QCHost]?[[STKit sharedInstance] qimNav_QCHost]:@"", @"showServiceState": @([[STKit sharedInstance] isMerchant]), @"fileUrl":[[STKit sharedInstance] qimNav_InnerFileHttpHost]?[[STKit sharedInstance] qimNav_InnerFileHttpHost]:@"", @"isShowWorkWorld":@(oldAuthSign), @"isShowGroupQRCode":@(YES), @"isShowLocalQuickSearch":@(YES), @"isShowRedPackage":@(isShowRedPackage), @"isEasyTrip":@(isEasyTrip), @"isiOSIpad":@([[STKit sharedInstance] getIsIpad]), @"ScreenWidth":@([[UIScreen mainScreen] qim_rightWidth]), @"notNeedShowEmailInfo":@(notNeedShowEmailInfo), @"notNeedShowMobileInfo":@(notNeedShowMobileInfo), @"notNeedShowLeaderInfo":@(notNeedShowLeaderInfo), @"notNeedShowCamelNotify":@(notNeedShowCamelNotify),
...
RCT_EXPORT_METHOD(updateRemoteKey:(RCTResponseSenderBlock)success) {
    if ([[STKit sharedInstance] updateRemoteLoginKey]) {
        success(@[@{@"ok":@(YES)}]);
    } else {
        success(@[@{@"ok":@(NO)}]);
    }
    ...
    switch (AppType) {
        case QIMAppExternal: {
            ...
            if (bundleUrl.length > 0) {
                NSString *bundleMd5Name = [[[STKit sharedInstance] qim_cachedFileNameForKey:bundleUrl] stringByAppendingFormat:@".jsbundle"];
        ...
        case QIMAppTypeH5: {
            ...
            if (webUrl.length > 0) {
                dispatch_async(dispatch_get_main_queue(), ^{
                   [STFastEntrance openWebViewForUrl:webUrl showNavBar:shownav];
                });
            }
        }
            break;
...
RCT_EXPORT_METHOD(openNativePage:(NSDictionary *)params){
    ...
    if ([nativeName isEqualToString:MyRedBag]) {
        // 我的红包
        [STFastEntrance openWebViewForUrl:[[STKit sharedInstance] myRedpackageUrl] showNavBar:YES];
    } else if ([nativeName isEqualToString:BalanceInquiry]) {
        // 余额查询
        [STFastEntrance openWebViewForUrl:[[STKit sharedInstance] redPackageBalanceUrl] showNavBar:YES];
        ...
        [STFastEntrance openWebViewForUrl:[[STKit sharedInstance] qimNav_getManagerAppUrl] showNavBar:YES];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [STFastEntrance openRobotChatVC:publicNumberId];
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSString *searchMsgUrl = [NSString stringWithFormat:@"%@/lookback/main_controller.php", [[STKit sharedInstance] qimNav_InnerFileHttpHost]];
            if (searchMsgUrl.length > 0) {
                [STFastEntrance openWebViewForUrl:searchMsgUrl showNavBar:YES];
            }
        });
...
//    return [NSURL URLWithString:@"http://localhost:8081/index.ios.bundle?platform=ios&dev=true"];
    ...
    NSString *qtalkFoundRNDebugUrlStr = [[STKit sharedInstance] userObjectForKey:@"qtalkFoundRNDebugUrl"];
...
+ (UIViewController *)TOTPVC {
    [[STKit sharedInstance] updateRemoteLoginKey];
    QIMRNBaseVc *vc = [[QIMRNBaseVc alloc] init];
    [vc setHiddenNav:YES];
    vc.rnName = @"TOTP";
    NSURL *jsCodeLocation = [QimRNBModule getJsCodeLocation];
...
+(NSString *)getAssetBundleName{
    ...
    } else if ([STKit getQIMProjectType] == QIMProjectTypeQChat) {
...
RCT_EXPORT_METHOD(openUserCard:(NSDictionary *)param) {
    ...
    if (![userId isEqualToString:[[STKit sharedInstance] getLastJid]] && userId.length > 0) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
RCT_EXPORT_METHOD(getUserInfoByUserCard:(NSString *)userId :(RCTResponseSenderBlock)callback) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[STKit sharedInstance] updateUserCard:userId withCache:NO];
    });
...
RCT_EXPORT_METHOD(syncChatStickyState:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    BOOL stickyState = [[STKit sharedInstance] isStickWithCombineJid:combineId];
...
RCT_EXPORT_METHOD(updateUserChatStickyState:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    if ([[STKit sharedInstance] isStickWithCombineJid:combineJid]) {
        [[STKit sharedInstance] removeStickWithCombineJid:combineJid WithChatType:ChatType_SingleChat withCallback:^(BOOL successed) {
           callback(@[@{@"ok" : @(successed)}]);
        }];
    } else {
        [[STKit sharedInstance] setStickWithCombineJid:combineJid WithChatType:ChatType_SingleChat withCallback:^(BOOL success) {
           callback(@[@{@"ok" : @(success)}]);
        }];
    }
}
...
RCT_EXPORT_METHOD(getFriend:(NSString *)userId :(RCTResponseSenderBlock)callback) {
    if (userId) {
        NSDictionary *friendDic = [[STKit sharedInstance] selectFriendInfoWithUserId:userId];
...
RCT_EXPORT_METHOD(getUserMedal:(NSString *)userId :(RCTResponseSenderBlock)callback) {
    if (userId.length > 0) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            [[STKit sharedInstance] getRemoteUserMedalWithXmppJid:userId];
        });
...
RCT_EXPORT_METHOD(getUserLead:(NSString *)userId :(RCTResponseSenderBlock)callback) {
    ...
    if (userId.length > 0) {
        ...
        callback(@[@{@"UserInfo":properties ? properties : @{}}]);
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] getRemoteUserWorkInfoWithUserId:userId withCallBack:nil];
        });
    }
}
...
RCT_EXPORT_METHOD(saveRemark:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    [[STKit sharedInstance] updateUserMarkupNameWithUserId:userId WithMarkupName:remark];
    callback(@[@{@"ok" : @(YES)}]);
}
...
RCT_EXPORT_METHOD(showUserPhoneNumber:(NSDictionary *)param) {
    ...
    if (!showNumberId) {
        showNumberId = userId;
        [[STKit sharedInstance] getPhoneNumberWithUserId:userId withCallBack:^(NSString *phoneNumberStr) {
...
RCT_EXPORT_METHOD(sendEmail:(NSDictionary *)param) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[STFastEntrance sharedInstance] sendMailWithRootVc:navVC ByUserId:userId];
    });
}
...
RCT_EXPORT_METHOD(commentUser:(NSDictionary *)param) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (baseUrl.length > 0) {
            ...
            [STFastEntrance openWebViewForUrl:commentUrl showNavBar:YES];
        }
    });
}
...
RCT_EXPORT_METHOD(addUserFriend:(NSDictionary *)param) {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *userId = [param objectForKey:@"UserId"];
        NSDictionary *modeDic = [[STKit sharedInstance] getVerifyFreindModeWithXmppId:userId];
...
//        UINavigationController *navVC = (UINavigationController *)[[[UIApplication sharedApplication] keyWindow] rootViewController];
        ...
        switch (mode) {
            ...
            {
                [[STKit sharedInstance] addFriendPresenceWithXmppId:userId WithAnswer:nil];
            }
                break;
...
//        UINavigationController *navVC = (UINavigationController *)[[[UIApplication sharedApplication] keyWindow] rootViewController];
        ...
        UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:@"删除好友" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                BOOL isSuccess = [[STKit sharedInstance] deleteFriendWithXmppId:userId WithMode:2];
                dispatch_async(dispatch_get_main_queue(), ^{
...
RCT_EXPORT_METHOD(clearImessage:(NSDictionary *)params) {
    ...
    [[STKit sharedInstance] deleteMessageWithXmppId:xmppId];
}

// 获取群二维码图片
RCT_EXPORT_METHOD(getGroupQRCode:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    QRCodeGenerator *qr = [[QRCodeGenerator alloc] initWithString:[NSString stringWithFormat:@"qtalk://group?id=%@",groupId]];
    qr.size = CGSizeMake(256.0f, 256.0f);
    UIImage *qrCodeImage = [qr getImage];
...
RCT_EXPORT_METHOD(getGroupMember:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            [[STKit sharedInstance] syncgroupMember:groupId];
        });
    });
}
...
RCT_EXPORT_METHOD(selectMemberFromGroup:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    NSArray *userList = [[STKit sharedInstance] qimDB_getGroupMember:groupId BySearchStr:searchText];
    ...
    for (NSDictionary *userDic in userList) {
        ...
        if (headerUri.length <= 0) {
            headerUri = [STKit defaultUserHeaderImagePath];
        }
...
RCT_EXPORT_METHOD(selectGroupMemberForKick:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    NSArray *userList = [[STKit sharedInstance] qimDB_getGroupMember:groupId WithGroupIdentity:affiliation];
    ...
    for (NSDictionary *userDic in userList) {
        ...
        NSString *xmppId = [userDic objectForKey:@"xmppjid"];
        NSString *name = [userDic objectForKey:@"name"];
        NSString *userName = [[STKit sharedInstance] getUserMarkupNameWithUserId:xmppId];
        NSString *headerUri = [[QIMImageManager sharedInstance] qim_getHeaderCachePathWithJid:xmppId];
        if (headerUri.length <= 0) {
            headerUri = [STKit defaultUserHeaderImagePath];
        }
...
RCT_EXPORT_METHOD(kickGroupMember:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    for (NSDictionary *groupMemberDic in [selectGroupMemebers allValues]) {
        ...
        NSDictionary *groupMemberInfo = [[STKit sharedInstance] getUserInfoByUserId:xmppId];
        NSString *memberName = [groupMemberInfo objectForKey:@"Name"];
        kickSuccess = [[STKit sharedInstance] removeGroupMemberWithName:memberName WithJid:xmppId ForGroupId:groupId];
    }
    if (kickSuccess == YES) {
...
RCT_EXPORT_METHOD(setGroupAdmin:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    BOOL setSuccess = [[STKit sharedInstance] setGroupAdminWithGroupId:groupId withIsAdmin:isAdmin WithAdminNickName:name ForJid:xmppId];
    if (YES == setSuccess) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
RCT_EXPORT_METHOD(showRedView:(RCTResponseSenderBlock)callback) {
    BOOL show = [[[STKit sharedInstance] userObjectForKey:@"qimrn_searchlocal"] boolValue];
    callback(@[@{@"show" : @(!show)}]);
}

RCT_EXPORT_METHOD(isShowRedView) {
    [[STKit sharedInstance] setUserObject:@(YES) forKey:@"qimrn_searchlocal"];
}

// 获取群PushState
RCT_EXPORT_METHOD(syncPushState:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    BOOL pushState = [[STKit sharedInstance] groupPushState:groupId];
    callback(@[@{@"state" : @(pushState)}]);
}

// 更新群PushState
RCT_EXPORT_METHOD(updatePushState:(NSString *)groupId :(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] updatePushState:groupId withOn:state withCallback:^(BOOL isSuccess) {
...
RCT_EXPORT_METHOD(syncGroupStickyState:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    ...
    BOOL stickyState = [[[STKit sharedInstance] stickList] objectForKey:combineId];
    callback(@[@{@"state" : @(stickyState)}]);
}
...
RCT_EXPORT_METHOD(updateGroupStickyState:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    if (groupId.length > 0) {
        if ([[STKit sharedInstance] isStickWithCombineJid:[NSString stringWithFormat:@"%@<>%@", groupId, groupId]]) {
            [[STKit sharedInstance] removeStickWithCombineJid:[NSString stringWithFormat:@"%@<>%@", groupId, groupId] WithChatType:ChatType_GroupChat withCallback:^(BOOL isSuccess) {
               callback(@[@{@"ok" : @(isSuccess)}]);
            }];
        } else {
            [[STKit sharedInstance] setStickWithCombineJid:[NSString stringWithFormat:@"%@<>%@", groupId, groupId] WithChatType:ChatType_GroupChat withCallback:^(BOOL isSuccess) {
               callback(@[@{@"ok" : @(isSuccess)}]);
            }];
        }
    }
...
RCT_EXPORT_METHOD(getGroupInfo:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] updateGroupCardByGroupId:groupId];
        });
...
RCT_EXPORT_METHOD(saveGroupName:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    if (groupTopic.length <= 0) {
        NSDictionary *groupCardDic = [[STKit sharedInstance] getGroupCardByGroupId:groupId];
        groupTopic = [groupCardDic objectForKey:@"Topic"];
    }
    [[STKit sharedInstance] setMucVcardForGroupId:groupId WithNickName:groupName WithTitle:groupTopic WithDesc:nil WithHeaderSrc:nil withCallBack:^(BOOL successed) {
        callback(@[@{@"ok" : @(successed)}]);
    }];
}
...
RCT_EXPORT_METHOD(addGroupMember:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    if (selectMembers.count <= 0) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [[STKit sharedInstance] clearNotReadMsgByGroupId:groupId];
            UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
            if (!navVC) {
                navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
            }
    ...
    NSString *nickName = [[STKit sharedInstance] getMyNickName];
    ...
    if (isGroup) {
        [[STKit sharedInstance] joinGroupWithBuddies:groupId groupName:@"" WithInviteMember:memberIds withCallback:^{
            dispatch_async(dispatch_get_main_queue(), ^{
                [[STKit sharedInstance] clearNotReadMsgByGroupId:groupId];
                UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
                if (!navVC) {
        ...
        NSString *nickName = [[STKit sharedInstance] getMyNickName];
        [[STKit sharedInstance] createGroupByGroupName:[QIMUUIDTools UUID]
                                         WithMyNickName:nickName
                                       WithInviteMember:memberIds
                                            WithSetting:[[STKit sharedInstance] defaultGroupSetting]
                                               WithDesc:@""
...
RCT_EXPORT_METHOD(quitGroup:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    if (groupId.length > 0) {
        BOOL result = [[STKit sharedInstance] quitGroupId:groupId];
        if (result) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString *combineGroupId = [NSString stringWithFormat:@"%@<>%@", groupId, groupId];
                [[STKit sharedInstance] removeStickWithCombineJid:combineGroupId WithChatType:ChatType_GroupChat withCallback:nil];
                UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
                if (!navVC) {
                    navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
                }
...
RCT_EXPORT_METHOD(destructionGroup:(NSString *)groupId :(RCTResponseSenderBlock)callback) {
    if (groupId.length > 0) {
        BOOL result = [[STKit sharedInstance] destructionGroup:groupId];
        if (result) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString *combineGroupId = [NSString stringWithFormat:@"%@<>%@", groupId, groupId];
                [[STKit sharedInstance] removeStickWithCombineJid:combineGroupId WithChatType:ChatType_GroupChat withCallback:nil];
...
RCT_EXPORT_METHOD(selectUserListByText:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    NSArray *users = [[STKit sharedInstance] searchUserListBySearchStr:[searchText lowercaseString]];
    ...
    for (NSDictionary *memberDic in users) {
        ...
        BOOL hasInGroup = [[STKit sharedInstance] isGroupMemberByUserId:xmppId ByGroupId:groupId];
        [dic setQIMSafeObject:name forKey:@"name"];
        [dic setQIMSafeObject:xmppId forKey:@"xmppId"];
...
RCT_EXPORT_METHOD(savePersonalSignature:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    if ([userId isEqualToString:[[STKit sharedInstance] getLastJid]]) {
        [[STKit sharedInstance] updateUserSignature:signature withCallBack:^(BOOL successed) {
            callback(@[@{@"ok":@(successed)}]);
        }];
    } else {
...
// NavAddress QR
RCT_EXPORT_METHOD(getNavQRCode:(NSString *)navUrl :(RCTResponseSenderBlock)callback) {
    QRCodeGenerator *qr = [[QRCodeGenerator alloc] initWithString:[NSString stringWithFormat:@"qtalk://user?id=%@",navUrl]];
    qr.size = CGSizeMake(256.0f, 256.0f);
    UIImage *qrCodeImage = [qr getImage];
    NSString *base64Image = [UIImage qim_image2DataURL:qrCodeImage];
    callback(@[@{@"qrCode":base64Image?base64Image:@""}]);
...
RCT_EXPORT_METHOD(getUserQRCode:(NSString *)userId :(RCTResponseSenderBlock)callback) {
    QRCodeGenerator *qr = [[QRCodeGenerator alloc] initWithString:[NSString stringWithFormat:@"qtalk://user?id=%@",userId]];
    qr.size = CGSizeMake(256.0f, 256.0f);
    UIImage *qrCodeImage = [qr getImage];
    NSString *base64Image = [UIImage qim_image2DataURL:qrCodeImage];
    callback(@[@{@"qrCode":base64Image?base64Image:@""}]);
...
- (void)saveImage:(UIImage *)image WithImageName:(NSString *)ImageName{
    ...
    if (currentImageData) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            [[STKit sharedInstance] updateMyPhoto:currentImageData];
        });
    }
}
...
RCT_EXPORT_METHOD(sendAdviceMessage:(NSDictionary *)adviceParam :(RCTResponseSenderBlock)callback) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
       [[STLocalLog sharedInstance] submitFeedBackWithContent:adviceMsg WithLogSelected:logSelected];
    });
#if __has_include("QIMAutoTracker.h")
...
RCT_EXPORT_METHOD(getContactsNick:(NSString *)xmppId :(RCTResponseSenderBlock)callback) {
    ...
    NSString *remarkName = [[STKit sharedInstance] getUserMarkupNameWithUserId:xmppId];
...
RCT_EXPORT_METHOD(selectFriendsForGroupAdd:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    NSString *groupId = [param objectForKey:@"groupId"];
    NSArray *friendList = [[STKit sharedInstance] qimDB_selectFriendListInGroupId:groupId];
    if (friendList) {
...
RCT_EXPORT_METHOD(getContacts:(RCTResponseSenderBlock)callback) {
    NSArray *friendList = [[STKit sharedInstance] selectFriendList];
    if (friendList) {
        NSMutableArray *contactList = [NSMutableArray array];
        for (NSDictionary *userInfo in friendList) {
            NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:userInfo];
            ...
            NSString *pinyin = [QIMPinYinForObjc chineseConvertToPinYin:name];
            NSString *uri = [[QIMImageManager sharedInstance] qim_getHeaderCachePathWithJid:jid];
            NSDictionary *userDic = [[STKit sharedInstance] getUserInfoByUserId:jid];
...
RCT_EXPORT_METHOD(getGroupList:(RCTResponseSenderBlock)callback) {
    NSArray *tempList = [[STKit sharedInstance] getMyGroupList];
...
RCT_EXPORT_METHOD(getPublicNumberList:(RCTResponseSenderBlock)callback) {
    NSArray *tempList = [[STKit sharedInstance] getPublicNumberList];
    ...
    for (NSDictionary *pInfo in tempList) {
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:headerPath] || !headerPath) {
            headerPath = [[STKit sharedInstance] getPublicNumberDefaultHeaderPath];
        }
        [pNumberDic setQIMSafeObject:headerPath ? headerPath : [[STKit sharedInstance] getPublicNumberDefaultHeaderPath] forKey:@"HeaderUri"];
...
RCT_EXPORT_METHOD(isStarOrBlackContact:(NSString *) xmppid ConfigKey:(NSString *) pkey :(RCTResponseSenderBlock)callback){
    BOOL flag = [[STKit sharedInstance] isStarOrBlackContact:xmppid ConfigKey:pkey];
    callback(@[@{@"ok" : @(flag)}]);
}
//设置取消星标&黑名单
RCT_EXPORT_METHOD(setStarOrblackContact:(NSString *)xmppid ConfigKey:(NSString *)pkey :(BOOL)value :(RCTResponseSenderBlock)callback){
    [[STKit sharedInstance] setStarOrblackContact:xmppid ConfigKey:pkey Flag:value withCallback:^(BOOL flag) {
       callback(@[@{@"ok" : @(flag)}]);
    }];
}
//设置取消星标&黑名单(多个)
RCT_EXPORT_METHOD(setStarOrBlackContacts:(NSDictionary*)map ConfigKey:(NSString *)pkey :(BOOL)value :(RCTResponseSenderBlock)callback){
    [[STKit sharedInstance] setStarOrblackContacts:map ConfigKey:pkey Flag:value withCallback:^(BOOL flag) {
       callback(@[@{@"ok" : @(flag)}]);
    }];
}
...
RCT_EXPORT_METHOD(updateOnLineNotifyState:(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] setMsgNotifySettingWithIndex:QIMMSGSETTINGPUSH_ONLINE WithSwitchOn:state withCallBack:^(BOOL updateSuccess) {
...
RCT_EXPORT_METHOD(getNotifySoundState:(RCTResponseSenderBlock)callback) {
    BOOL state = [[STKit sharedInstance] getLocalMsgNotifySettingWithIndex:QIMMSGSETTINGSOUND_INAPP];
...
RCT_EXPORT_METHOD(updateNotifySoundState:(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] setMsgNotifySettingWithIndex:QIMMSGSETTINGSOUND_INAPP WithSwitchOn:state withCallBack:^(BOOL updateSuccess) {
       callback(@[@{@"ok" : @(updateSuccess)}]);
    }];
}
...
RCT_EXPORT_METHOD(getStartPushState:(RCTResponseSenderBlock)callback) {
    BOOL state = [[STKit sharedInstance] getLocalMsgNotifySettingWithIndex:QIMMSGSETTINGPUSH_SWITCH];
    callback(@[@{@"state" : @(state)}]);
}

//设置开启消息推送状态
RCT_EXPORT_METHOD(updateStartNotifyState:(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] setMsgNotifySettingWithIndex:QIMMSGSETTINGPUSH_SWITCH WithSwitchOn:state withCallBack:^(BOOL updateSuccess) {
       callback(@[@{@"ok" : @(updateSuccess)}]);
    }];
}
...
RCT_EXPORT_METHOD(getNotifyVibrationState:(RCTResponseSenderBlock)callback) {
    BOOL state = [[STKit sharedInstance] getLocalMsgNotifySettingWithIndex:QIMMSGSETTINGVIBRATE_INAPP];
    callback(@[@{@"state" : @(state)}]);
}

//设置用户通知震动状态
RCT_EXPORT_METHOD(updateNotifyVibrationState:(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] setMsgNotifySettingWithIndex:QIMMSGSETTINGVIBRATE_INAPP WithSwitchOn:state withCallBack:^(BOOL updateSuccess) {
       callback(@[@{@"ok" : @(updateSuccess)}]);
    }];
}
...
RCT_EXPORT_METHOD(getNotifyPushDetailsState:(RCTResponseSenderBlock)callback) {
    BOOL state = [[STKit sharedInstance] getLocalMsgNotifySettingWithIndex:QIMMSGSETTINGSHOW_CONTENT];
    callback(@[@{@"state" : @(state)}]);
}

//设置用户通知是否显示详情
RCT_EXPORT_METHOD(updateNotifyPushDetailsState:(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] setMsgNotifySettingWithIndex:QIMMSGSETTINGSHOW_CONTENT WithSwitchOn:state withCallBack:^(BOOL updateSuccess) {
       callback(@[@{@"ok" : @(updateSuccess)}]);
    }];
}
...
RCT_EXPORT_METHOD(getShowUserModState:(RCTResponseSenderBlock)callback) {
    BOOL state = [[STKit sharedInstance] moodshow];
...
RCT_EXPORT_METHOD(updateShowUserModState:(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] setMoodshow:state];
    callback(@[@{@"ok" : @(YES)}]);
}

//获取水印状态
RCT_EXPORT_METHOD(getWaterMark:(RCTResponseSenderBlock)callback) {
    BOOL state = [[STKit sharedInstance] waterMarkState];
    callback(@[@(state)]);
}

//设置水印状态，仅限本地
RCT_EXPORT_METHOD(setWaterMark:(BOOL)isOpen) {
    [[STKit sharedInstance] setWaterMarkState:isOpen];
}

//获取驼圈提醒
RCT_EXPORT_METHOD(getworkWorldRemind:(RCTResponseSenderBlock)callback) {
    BOOL state = [[STKit sharedInstance] getLocalWorkMomentNotifyConfig];
    callback(@[@{@"state" : @(state)}]);
}

//设置驼圈提醒
RCT_EXPORT_METHOD(updateWorkWorldRemind:(BOOL)state :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] updateRemoteWorkMomentNotifyConfig:state withCallBack:^(BOOL successed) {
       callback(@[@{@"ok" : @(successed)}]);
    }];
}
...
RCT_EXPORT_METHOD(getServiceState:(RCTResponseSenderBlock)callback) {
    ...
    [[STKit sharedInstance] getSeatSeStatusWithCallback:^(NSArray *list) {
...
RCT_EXPORT_METHOD(setServiceState:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    if (sid) {
        [[STKit sharedInstance] updateSeatSeStatusWithShopId:sid WithStatus:st withCallBack:^(BOOL res) {
            callback(@[@{@"result" : @(res)}]);
        }];
    }
}
...
RCT_EXPORT_METHOD(getAlertVoiceType:(RCTResponseSenderBlock)callback) {
    NSString *soundName = [[STKit sharedInstance] getClientNotificationSoundName];
...
RCT_EXPORT_METHOD(changeConfigAlertStatus:(BOOL)state :(RCTResponseSenderBlock)callback) {
    ...
    [[STKit sharedInstance] setClientNotificationSound:soundName withCallback:^(BOOL success) {
        callback(@[@{@"ok" : @(success)}]);
    }];
}
...
RCT_EXPORT_METHOD(getAppVersion:(RCTResponseSenderBlock)callback) {
    NSString *appVersion = [NSString stringWithFormat:@"%@", [[STKit sharedInstance] AppVersion]];
...
RCT_EXPORT_METHOD(openSearchHistoryVc) {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *searchUrl = [NSString stringWithFormat:@"%@/lookback/main_controller.php", [[STKit sharedInstance] qimNav_InnerFileHttpHost]];
        if (searchUrl.length > 0) {
            [STFastEntrance openWebViewForUrl:searchUrl showNavBar:YES];
        }
    });
}
...
//                [[QIMKit sharedInstance] sendNoPush];
            ...
            [[STKit sharedInstance] quitLogin];
            [[STKit sharedInstance] clearLogginUser];
            [[STKit sharedInstance] setCacheName:@""];
            [self reloginAccount];
        }
    }
...
RCT_EXPORT_METHOD(getAppCache:(RCTResponseSenderBlock)callback) {
    ...
    NSString *str = [[STDataContraller getInstance] transfromTotalSize:totalSize];
...
RCT_EXPORT_METHOD(selectUserTripByDate:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    NSArray *list = [[STKit sharedInstance] selectTripByYearMonth:date];
...
RCT_EXPORT_METHOD(getTripAreaAvailableRoom:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    [[STKit sharedInstance] getTripAreaAvailableRoom:params callBack:^(NSArray *availableRooms) {
...
RCT_EXPORT_METHOD(getTripCity:(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] getAllCityList:^(NSArray *allCitys) {
...
RCT_EXPORT_METHOD(getNewTripArea:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] getAreaByCityId:params :^(NSArray *availableRooms) {
...
RCT_EXPORT_METHOD(tripMemberCheck:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    [[STKit sharedInstance] tripMemberCheck:params callback:^(BOOL isConform) {
...
RCT_EXPORT_METHOD(createTrip:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    ...
    for (NSDictionary *memberInfo in memberList) {
        ...
        if (memberId.length > 0) {
            if ([memberId isEqualToString:[[STKit sharedInstance] getLastJid]]) {
                checkMe = YES;
            }
    ...
    if (!checkMe) {
        [newMemberList addObject:@{@"memberId":[[STKit sharedInstance] getLastJid]}];
    }
    [newParam setObject:newMemberList forKey:@"memberList"];
    [newParam setObject:@(111) forKey:@"updateTime"];
    [newParam setObject:[[STKit sharedInstance] getLastJid] forKey:@"tripInviter"];
    [[STKit sharedInstance] createTrip:newParam callBack:^(BOOL success, NSString *errMsg) {
...
RCT_EXPORT_METHOD(createRedEnvelope:(NSDictionary *)params :(RCTResponseSenderBlock)callback) {
    [[STKit sharedInstance] sendRedEnvelop:params withCallBack:^(BOOL successed,NSString *payParams) {
        callback(@[@{@"ok" : @(successed)}]);
    }];
}
...
RCT_EXPORT_METHOD(redEnvelopeGet:(NSString *)rid :(NSString *)xmppid :(NSInteger) isChatRoom :(RCTResponseSenderBlock)callback){
    [[STKit sharedInstance] getRedEnvelopDetail:xmppid RedRid:rid IsChatRoom:isChatRoom withCallBack:^(NSDictionary* data){
        ...
        if(ret){
            ...
            NSString* user_name = [[STKit sharedInstance] getUserMarkupNameWithUserId:userId];
            NSString* user_img = [[QIMImageManager sharedInstance] qim_getHeaderCachePathWithJid:userId];
            if(!user_img){
                user_img = [STKit defaultUserHeaderImagePath];
            }
            ...
            for (NSDictionary *itemDic in items) {
                ...
                NSString *name = [[STKit sharedInstance] getUserMarkupNameWithUserId:host_user_id];
...
RCT_EXPORT_METHOD(redEnvelopeReceive:(NSInteger)page :(NSInteger)pagesie :(NSInteger)year :(RCTResponseSenderBlock)callback){
    [[STKit sharedInstance] redEnvelopReceive:page PageSize:pagesie WithYear:year withCallBack:^(NSDictionary* data){
        BOOL ret = [[data objectForKey:@"ret"] boolValue];
        if(ret){
            NSDictionary* result = [data objectForKey:@"data"];
            NSString* user_img = [[QIMImageManager sharedInstance] qim_getHeaderCachePathWithJid:[[STKit sharedInstance] getLastJid]];
...
RCT_EXPORT_METHOD(redEnvelopeSend:(NSInteger)page :(NSInteger)pagesie :(NSInteger)year :(RCTResponseSenderBlock)callback){
    [[STKit sharedInstance] redEnvelopSend:page PageSize:pagesie WithYear:year withCallBack:^(NSDictionary* data){
        [[STKit sharedInstance] redEnvelopSend:page PageSize:pagesie WithYear:year withCallBack:^(NSDictionary* data){
            BOOL ret = [[data objectForKey:@"ret"] boolValue];
            if(ret){
                NSDictionary* result = [data objectForKey:@"data"];
...
RCT_EXPORT_METHOD(openChatForLocalSearch:(NSString *)xmppid :(NSString *)realjid :(NSString *)chattype :(nonnull NSNumber *)time){
    ...
    [STFastEntrance openFastChatVCByXmppId:xmppid WithRealJid:realjid WithChatType:chatType WithFastMsgTimeStamp:fastMsgTime];
}
...
RCT_EXPORT_METHOD(openLocalSearchImage:(NSDictionary *)param) {
    ...
    [STFastEntrance openLocalMediaWithXmppId:xmppId withRealJid:realjid withChatType:chattype];
}
...
RCT_EXPORT_METHOD(selectStarOrBlackContacts:(NSString *)pkey :(RCTResponseSenderBlock)callback){
    NSMutableArray *contacts = [[STKit sharedInstance] selectStarOrBlackContacts:pkey];
...
RCT_EXPORT_METHOD(selectFriendsNotInStarContacts:(RCTResponseSenderBlock)callback){
    NSMutableArray *contacts = [[STKit sharedInstance] selectFriendsNotInStarContacts];
    ...
    for (NSDictionary *friendInfo in contacts) {
        ...
        NSString *userName = [[STKit sharedInstance] getUserMarkupNameWithUserId:userXmppJid];
...
RCT_EXPORT_METHOD(selectUserNotInStartContacts:(NSString *)key :(RCTResponseSenderBlock)callback){
    NSMutableArray *contacts = [[STKit sharedInstance] selectUserNotInStartContacts:key];
...
RCT_EXPORT_METHOD(openUserWorkWorld:(NSDictionary *)param) {
    [[STFastEntrance sharedInstance] openUserWorkWorldWithParam:param];
}
