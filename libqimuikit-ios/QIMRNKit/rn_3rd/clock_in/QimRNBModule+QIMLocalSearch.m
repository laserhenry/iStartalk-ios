...
+ (NSDictionary *)qimrn_searchLocalMsgWithUserParam:(NSDictionary *)param {
    ...
    if (chatType == ChatType_Consult || chatType == ChatType_ConsultServer) {
        localMsgs = [[STKit sharedInstance] getMsgsByKeyWord:searchText ByXmppId:xmppId ByReadJid:realjid];
        NSLog(@"msgs : %@", localMsgs);
    } else {
        localMsgs = [[STKit sharedInstance] getMsgsByKeyWord:searchText ByXmppId:xmppId ByReadJid:nil];
        NSLog(@"msgs : %@", localMsgs);
    }
    NSMutableArray *dateArray = [NSMutableArray arrayWithCapacity:3];
    for (STMsgModel * msg in localMsgs) {
        ...
        NSString *remarkName = [[STKit sharedInstance] getUserMarkupNameWithUserId:msgSendJid];
        NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:msgSendJid];
        ...
        [msgDic setQIMSafeObject:headerUrl ? headerUrl : [STKit defaultUserHeaderImagePath] forKey:@"headerUrl"];
...
+ (NSDictionary *)qimrn_searchLocalFileWithUserParam:(NSDictionary *)param {
    ...
    NSArray *localMsgs = @[];
    if (chatType == ChatType_Consult || chatType == ChatType_ConsultServer) {
        localMsgs = [[STKit sharedInstance] getMsgsForMsgType:@[@(QIMMessageType_File)] ByXmppId:xmppId ByReadJid:realjid];
        NSLog(@"msgs : %@", localMsgs);
    } else {
        localMsgs = [[STKit sharedInstance] getMsgsForMsgType:@[@(QIMMessageType_File)] ByXmppId:xmppId];
        NSLog(@"msgs : %@", localMsgs);
    }
    NSMutableArray *dateArray = [NSMutableArray arrayWithCapacity:3];
    for (STMsgModel * msg in localMsgs) {
...
+ (NSDictionary *)qimrn_searchLocalLinkWithUserParam:(NSDictionary *)param {
    ...
    NSArray *localMsgs = @[];
    if (chatType == ChatType_Consult || chatType == ChatType_ConsultServer) {
        localMsgs = [[STKit sharedInstance] getMsgsForMsgType:@[@(QIMMessageType_CommonTrdInfo), @(QIMMessageType_CommonTrdInfoPer)] ByXmppId:xmppId ByReadJid:realjid];
        NSLog(@"msgs : %@", localMsgs);
    } else {
        localMsgs = [[STKit sharedInstance] getMsgsForMsgType:@[@(QIMMessageType_CommonTrdInfo), @(QIMMessageType_CommonTrdInfoPer)] ByXmppId:xmppId];
        NSLog(@"msgs : %@", localMsgs);
    }
    NSMutableArray *dateArray = [NSMutableArray arrayWithCapacity:3];
    for (STMsgModel * msg in localMsgs) {
        ...
        if (linkIcon.length <= 0) {
            linkIcon = [STKit defaultCommonTrdInfoImagePath];
        }
        NSDate *msgDate = [NSDate qim_dateWithTimeIntervalInMilliSecondSince1970:msg.messageDate];
