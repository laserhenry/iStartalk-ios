...
+ (NSArray *)qimrn_getNewUserMedalByUserId:(NSString *)xmppId {
    ...
    return [[STKit sharedInstance] getUserWearMedalSmallIconListByUserid:xmppId];
}
...
+ (NSArray *)qimrn_getNewUserHaveMedalByUserId:(NSString *)xmppId {
    ...
    return [[STKit sharedInstance] getUserHaveMedalSmallIconListByUserid:xmppId];
}
...
+ (NSArray *)qimrn_getUserMedalByUserId:(NSString *)xmppId {
    ...
    NSArray *localUserMedals = [[STKit sharedInstance] getLocalUserMedalWithXmppJid:xmppId];
    return localUserMedals;
}

+ (NSDictionary *)qimrn_getUserLeaderInfoByUserId:(NSString *)userId {
    NSDictionary *userWorkInfo = nil;
    if ([STKit getQIMProjectType] != QIMProjectTypeQChat && [[[STKit sharedInstance] getDomain] isEqualToString:@"ejabhost1"]) {
        userWorkInfo = [[STKit sharedInstance] getUserWorkInfoByUserId:userId];
    } else {
        userWorkInfo = nil;
    }
    ...
    if (leaderId.length > 0 && ![leaderId containsString:@"@"]) {
        leaderId = [leaderId stringByAppendingFormat:@"@%@", [[STKit sharedInstance] getDomain]];
    }
    NSMutableDictionary *properties = [NSMutableDictionary dictionary];
