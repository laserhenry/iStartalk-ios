...
RCT_EXPORT_METHOD(getLoginInfo:(RCTResponseSenderBlock)success:(RCTResponseSenderBlock)error) {
    ...
    NSString *qtalkToken = [[STKit sharedInstance] myRemotelogginKey];
    qtalkToken = qtalkToken.length ? qtalkToken : @"";
    
    NSString *key = [[STKit sharedInstance] thirdpartKeywithValue];
    key = key.length ? key : @"";

    NSString *httpHost = [[STKit sharedInstance] qimNav_HttpHost];
    httpHost = httpHost.length ? httpHost : @"";

    if ([userName.lowercaseString isEqualToString:@"appstore"] == NO) {
        NSDictionary *responseData = @{@"userid" : userName, @"q_auth" : qtalkToken, @"c_key" : key, @"checkUserKeyHost" : httpHost, @"showOA":@([[STKit sharedInstance] qimNav_ShowOA])};
