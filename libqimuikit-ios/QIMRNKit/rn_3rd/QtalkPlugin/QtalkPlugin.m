...
RCT_EXPORT_METHOD(browseBigImage:(NSDictionary *)param :(RCTResponseSenderBlock)callback){
    [[STFastEntrance sharedInstance] browseBigHeader:param];
}

RCT_EXPORT_METHOD(openDownLoad:(NSDictionary *)param :(RCTResponseSenderBlock)callback){
    [[STFastEntrance sharedInstance] openQIMFilePreviewVCWithParam:param];
}

RCT_EXPORT_METHOD(openNativeWebView:(NSDictionary *)param) {
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([STFastEntrance handleOpsasppSchema:param] == NO) {
...
RCT_EXPORT_METHOD(gotoWiki:(NSDictionary *)param) {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *wikiUrl = [[STKit sharedInstance] qimNav_WikiUrl];
        if (wikiUrl.length > 0) {
            [STFastEntrance openWebViewForUrl:wikiUrl showNavBar:YES];
        }
    });
}
...
RCT_EXPORT_METHOD(getUserInfo:(RCTResponseSenderBlock)callback) {
    ...
    */
    ...
    [map setObject:[[STKit sharedInstance] getLastJid] forKey:@"userId"];
    [map setObject:[[STKit sharedInstance] getDomain] forKey:@"domain"];
    [map setObject:@"192.168.0.1" forKey:@"clientIp"];
    
    [map setObject:[[STKit sharedInstance] thirdpartKeywithValue] forKey:@"ckey"];
    [map setObject:[[STKit sharedInstance] qimNav_HttpHost] forKey:@"httpHost"];
    [map setObject:[[STKit sharedInstance] qimNav_InnerFileHttpHost] forKey:@"fileUrl"];
    callback(@[map ? map : @{}]);
}
...
RCT_EXPORT_METHOD(getWorkWorldItem:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    NSDictionary *momentDic = [[STKit sharedInstance] getLastWorkMoment];
    NSLog(@"getWorkWorldItem : %@", momentDic);
    callback(@[momentDic ? momentDic : @{}]);
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        [[STKit sharedInstance] getRemoteLastWorkMoment];
    });
}

RCT_EXPORT_METHOD(getWorkWorldNotRead:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    NSInteger notReadMsgCount = [[STKit sharedInstance] getWorkNoticeMessagesCountWithEventType:@[@(QIMWorkFeedNotifyTypeComment), @(QIMWorkFeedNotifyTypePOSTAt), @(QIMWorkFeedNotifyTypeCommentAt)]];
    BOOL showNewPOST = NO;
...
RCT_EXPORT_METHOD(openWorkWorld:(NSDictionary *)param) {
    [[STFastEntrance sharedInstance] openWorkFeedViewController];
}
...
RCT_EXPORT_METHOD(openTravelCalendar:(NSDictionary *)param) {
    [STFastEntrance openTravelCalendarVc];
}

//获取发现页应用列表
RCT_EXPORT_METHOD(getFoundInfo:(RCTResponseSenderBlock)callback) {
    NSString *foundListStr = [[STKit sharedInstance] getLocalFoundNavigation];
...
RCT_EXPORT_METHOD(getSearchInfo:(NSString *)searchUrl :(NSDictionary *)params :(NSDictionary *)cookie :(RCTResponseSenderBlock)callback1 :(RCTResponseSenderBlock)callback2) {
    ...
    NSLog(@"params : %@", params);
    NSLog(@"Cookie : %@", cookie);
    [[STKit sharedInstance] searchWithUrl:searchUrl withParams:params withSuccessCallBack:^(BOOL successed, NSString *responseJson) {
...
RCT_EXPORT_METHOD(showSearchHistoryResult:(NSDictionary *)params) {
    ...
    if (todoType == 16) {
        ...
        [STFastEntrance openGroupChatVCByGroupId:userJid withFastTime:time withRemoteSearch:YES];
    } else {
        //单聊
        if([[[STKit sharedInstance] getLastJid] isEqualToString:from]){
//            userJid = userJid;
//            realJid = realto;
        }else{
            userJid = from;
        ...
        [STFastEntrance openSingleChatVCByUserId:userJid withFastTime:time withRemoteSearch:YES];
    }
}
...
RCT_EXPORT_METHOD(getLocalSearchKeyHistory:(NSDictionary *)param :(RCTResponseSenderBlock)callback) {
    ...
    NSArray *searchKeys = [[STKit sharedInstance] getLocalSearchKeyHistoryWithSearchType:searchType withLimit:limit];
    callback(@[@{@"ok" : @(YES), @"searchKeys" : searchKeys ? searchKeys : @[]}]);
}

//更新本地搜索Key历史
RCT_EXPORT_METHOD(updateLocalSearchKeyHistory:(NSDictionary *)param) {
    [[STKit sharedInstance] updateLocalSearchKeyHistory:param];
}

//清空本地搜索历史Key
RCT_EXPORT_METHOD(clearLocalSearchKeyHistory:(NSInteger)searchType) {
    [[STKit sharedInstance] deleteSearchKeyHistory];
}
