...
@interface STPickerView () <UIPickerViewDelegate, UIPickerViewDataSource>
...
+ (STPickerView*)sharedView {
  static dispatch_once_t once;
  static STPickerView *sharedView;
...
-(void)dismiss{
 [STPickerView dismissWithCompletion:self.onDismissCompletion];
}
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, _pickerViewContainerView.bounds.size.height - 260.0, CGRectGetWidth([UIScreen mainScreen].bounds), 260.0)];
  ...
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 44.0, CGRectGetWidth([UIScreen mainScreen].bounds), 216.0)];
