...
- (void)onClick {
    ...
    if ([self.delegate playingVoiceWithMsgId:self.message.messageId]) {
        ...
        if (self.message.messageDirection == QIMMessageDirection_Received) {
            //如果语音未读，mssageID落地
            [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] setVoiceNoReadStateWithMsgId:self.message.messageId ChatId:self.chatId withState:YES];
            [self refreshUI];
        }
    }
}
...
- (void)doChatVCRefresh {
    ...
    if (self.message.messageType == QIMMessageType_Voice) {
        ...
        if (self.message.messageDirection == QIMMessageDirection_Received) {
            ...
            //如果未读， 小红点显示
            BOOL unreadHidden = [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] isReadWithMsgId:self.message.messageId ChatId:self.chatId];
