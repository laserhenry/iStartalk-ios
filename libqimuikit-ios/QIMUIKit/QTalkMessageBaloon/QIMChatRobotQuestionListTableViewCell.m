...
+ (CGFloat)getCellHeightWithMessage:(STMsgModel *)msg chatType:(ChatType)chatType {
    return [self consultRbtCellHeightForMsg:msg];
}

+ (CGFloat)consultRbtCellHeightForMsg:(STMsgModel *)msg {
...
- (void)actionsHandle:(UITapGestureRecognizer *)tap {
    ...
    if (index >= 0 && index < maxIndex) {
        ...
        if ([[clickType lowercaseString] isEqualToString:@"interface"]) {
            if (url.length > 0) {
                [[STKit sharedInstance] sendTPPOSTRequestWithUrl:url withSuccessCallBack:^(NSData *responseData) {
...
//                        [self.delegate sendTextMessageForText:afterClickSendMsg isSendToServer:YES userType:@"cRbt"];
        ...
        popVc.items = items;
        [STCustomPopMgr showPopVC:popVc withRootVC:[[UIApplication sharedApplication] visibleViewController]];
    }
    /*
...
- (void)resolveBtnClicked:(UIButton *)btn{
    ...
    [[STKit sharedInstance] sendTPPOSTRequestWithUrl:_yesBtnUrl withSuccessCallBack:^(NSData *responseData) {
...
- (void)unResolveBtnClicked:(UIButton *)btn{
    ...
    __weak typeof(self) weakSelf = self;
    [[STKit sharedInstance] sendTPPOSTRequestWithUrl:_noBtnUrl withSuccessCallBack:^(NSData *responseData) {
