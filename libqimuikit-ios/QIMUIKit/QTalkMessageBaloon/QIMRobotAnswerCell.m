...
+ (CGFloat)getCellHeightWithMessage:(STMsgModel *)message chatType:(ChatType)chatType {
    ...
    if (content.length) {
        STMsgModel *contentMsg = [[STMsgModel alloc] init];
    ...
    QIMMessageRemoteReadState readState = [[STKit sharedInstance] getReadStateWithMsgId:message.messageId];
    ...
    if (readState == 4) {
        ...
        if (middleContent.length) {
            STMsgModel *middleContentMsg = [[STMsgModel alloc] init];
...
- (void)refreshUI {
    ...
    QIMMessageReadFlag readState = [[STKit sharedInstance] getMessageStateWithMsgId:self.message.messageId];
...
- (void)setupControlButton {
    STButton *likeButton = [STButton buttonWithType:UIButtonTypeCustom];
    ...
    STButton *dislikeButton = [STButton buttonWithType:UIButtonTypeCustom];
    ...
    STButton *teachButton = [STButton buttonWithType:UIButtonTypeCustom];
...
- (void)feedbackAnswer:(id)sender {
    ...
    [[STKit sharedInstance] sendTPPOSTRequestWithUrl:requestUrl withRequestBodyData:bodydata withSuccessCallBack:^(NSData *responseData) {
        ...
        [[STKit sharedInstance] sendReadStateWithMessagesIdArray:@[self.message.messageId] WithMessageReadFlag:QIMMessageReadFlagDidControl WithXmppId:self.message.from];
