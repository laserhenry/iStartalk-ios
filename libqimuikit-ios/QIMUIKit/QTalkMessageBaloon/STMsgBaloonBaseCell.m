...
@interface STMsgBaloonBaseCell() <QIMMenuImageViewDelegate>

@property (nonatomic, strong) QIMTextContainer *textContainer;

@end

@implementation STMsgBaloonBaseCell
...
- (void)setMessage:(STMsgModel *)message {
   STMsgModel *tempMsg = _message;
    _message = message;
...
- (UIImageView *)HeadView {
    if (!_HeadView) {
        ...
        _HeadView.image = [UIImage imageWithData:[STKit defaultUserHeaderImage]];
    }
    return _HeadView;
}
...
- (void)updateNameLabel {
    if (self.message.messageDirection == QIMMessageDirection_Received) {
        ...
        dispatch_async([[STKit sharedInstance] getLoadMsgNickNameQueue], ^{
            if (self.chatType != ChatType_CollectionChat) {
                //备注
                NSString * remarkName = [[STKit sharedInstance] getUserMarkupNameWithUserId:self.message.from];
                if (remarkName.length > 0) {
                    nickName = remarkName;
                }
            } else {
                NSDictionary *userInfo = [[STKit sharedInstance] getCollectionUserInfoByUserId:nickName];
...
- (void)updateMedalListView {
    if (self.message.messageDirection == QIMMessageDirection_Received) {
        dispatch_async([[STKit sharedInstance] getLoadMsgMedalListQueue], ^{
           
            NSArray *medals = [[STKit sharedInstance] getUserWearMedalSmallIconListByUserid:self.message.from];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateMessageSendState {
    ...
    if (self.message.messageDirection == QIMMessageDirection_Sent) {
        ...
        if (![[STKit sharedInstance] isMiddleVirtualAccountWithJid:self.message.to] && self.message.chatType != ChatType_GroupChat && self.message.chatType != ChatType_System && self.chatType != ChatType_GroupChat && self.chatType != ChatType_System && self.chatType != ChatType_CollectionChat) {
            if ([[STKit sharedInstance] qimNav_Showmsgstat]) {
//                [self.contentView addSubview:self.messgaeStateLabel];
        ...
        if (self.message.messageSendState == QIMMessageSendState_Waiting || self.message.messageSendState == QIMMessageSendState_Faild) {
            self.message.messageSendState = [[STKit sharedInstance] getMessageStateWithMsgId:self.message.messageId];
        }
        if ([[[STKit sharedInstance] qimNav_getDebugers] containsObject:[STKit getLastUserName]]) {
            self.messgaeRealStateLabel.frame = CGRectMake(self.backView.left - 80, self.backView.top, 70, 24);
            [self.contentView addSubview:self.messgaeRealStateLabel];
...
- (void)setBackViewWithWidth:(CGFloat)backWidth WithHeight:(CGFloat)backHeight{
    switch (self.message.messageDirection) {
        case QIMMessageDirection_Received: {
            ...
            [self.backView setImage:[STMsgBaloonBaseCell leftBallocImage]];
        }
            break;
        case QIMMessageDirection_Sent: {
           ...
           if (self.chatType == ChatType_SingleChat) {
            ...
            [self.backView setFrame:frame];
            [self.backView setImage:[STMsgBaloonBaseCell rightBallcoImage]];
            _messgaeStateLabel.frame = CGRectMake(self.backView.left - 40, self.backView.bottom - 12, 35, 12);
            _statusButton.frame = CGRectMake(self.backView.left - 30, 0, 24, 24);
...
- (void)onHeaderViewClick:(UITapGestureRecognizer *)tapGesture {
    if (self.message.from.length > 0 && self.chatType != ChatType_CollectionChat && self.chatType != ChatType_Consult) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STFastEntrance openUserCardVCByUserId:self.message.from];
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [STFastEntrance openUserCardVCByUserId:self.message.xmppId];
        });
    } else {
...
- (void)onMedalViewClick:(UITapGestureRecognizer *)tap {
    if (self.message.from.length > 0 && self.chatType != ChatType_CollectionChat && self.chatType != ChatType_Consult) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STFastEntrance openUserMedalFlutterWithUserId:self.message.from];
        });
    } else if (self.message.xmppId.length > 0 && self.chatType == ChatType_Consult && self.message.messageDirection == QIMMessageDirection_Received) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [STFastEntrance openUserMedalFlutterWithUserId:self.message.xmppId];
        });
    } else {
...
- (void)refreshHeaderView {
    ...
    dispatch_async([[STKit sharedInstance] getLoad_msgHeaderImageQueue], ^{
        if (self.chatType != ChatType_GroupChat) {
            switch (self.message.messageDirection) {
                    case QIMMessageDirection_Sent: {
                        self.message.from = [[STKit sharedInstance] getLastJid];
                    }
                    break;
                    case QIMMessageDirection_Received: {
        ...
        if (self.chatType == ChatType_CollectionChat) {
            NSString *collectionUserUrl = [[STKit sharedInstance] getCollectionUserHeaderUrlWithXmppId:self.message.from];
            if (![collectionUserUrl qim_hasPrefixHttpHeader]) {
                collectionUserUrl = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], collectionUserUrl];
            }
            [self.HeadView qim_setImageWithURL:collectionUserUrl placeholderImage:[UIImage imageWithData:[STKit defaultUserHeaderImage]]];
        } else if (self.chatType == ChatType_Consult) {
            if (self.message.messageDirection == QIMMessageDirection_Sent) {
                [self.HeadView qim_setImageWithJid:[[STKit sharedInstance] getLastJid] WithChatType:ChatType_SingleChat];
            } else {
