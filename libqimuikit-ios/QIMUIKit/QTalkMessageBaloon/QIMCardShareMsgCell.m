...
- (void)tapGesHandle:(UITapGestureRecognizer *)tap{
    if (self.message.messageSendState == QIMMessageSendState_Faild) {
        ...
        NSDictionary *userInfoDic = [[STKit sharedInstance] getUserInfoByUserId:self.message.from];
...
- (void)refreshUI {
    ...
    NSDictionary *userInfoDic = [[STKit sharedInstance] getUserInfoByUserId:userId];
