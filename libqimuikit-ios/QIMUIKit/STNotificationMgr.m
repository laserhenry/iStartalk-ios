...
@interface STNotificationMgr()
@property (nonatomic,assign) BOOL isEnterBackGround;
@end

@implementation STNotificationMgr

static STNotificationMgr *_notificationManager = nil;
+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _notificationManager = [[STNotificationMgr alloc] init];
...
- (void)submitLog:(NSNotification *)notify {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
       [[STLocalLog sharedInstance] submitFeedBackWithContent:conent withUserInitiative:NO];
    });
#endif
}
        ...
        if (schemaUrl.absoluteString.length > 0 && [schemaUrl.scheme isEqualToString:@"qtalkaphone"]) {
            ...
            if ([host isEqualToString:@"qunarchat"]) {
                ...
                if ([path isEqualToString:@"/openGroupChat"]) {
                    ...
                    NSString *realJid = [tempDic objectForKey:@"realJid"];
                    [STFastEntrance openUserChatInfoByUserId:jid];
                ...
                [STFastEntrance openQIMRNVCWithModuleName:module WithProperties:@{@"Screen":Screen}];
