...
#import "STPHGridViewController.h"
#import "STPHImgPickerController.h"
#import "STPHAlbumsViewController.h"
#import "STPHGridViewCell.h"
...
@interface STPHImgPickerController ()
...
@interface STPHGridViewController () <UIActionSheetDelegate,QIMImageEditViewControllerDelegate,PHPhotoLibraryChangeObserver>
{
    UIView   *_bottomView;
...
@property (nonatomic, weak) STPHImgPickerController *picker;
...
@implementation STPHGridViewController
{
    CGFloat screenWidth;
...
-(id)initWithPicker:(STPHImgPickerController *)picker
{
    ...
    if (self = [super initWithCollectionViewLayout:layout])
    {
        ...
        [self.collectionView registerClass:STPHGridViewCell.class
...
{
    [super viewDidLoad];
    self.picker.isOriginal = [[STKit sharedInstance] pickerPixelOriginal];
    [self setupViews];
    [self initBottomView];
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    for (STPHGridViewCell *cell in [self.collectionView visibleCells]) {
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    if (indexPaths.count > 0) {
        ...
        NSInteger maxNumber = [[STPHImgPickerManager sharedInstance] maximumNumberOfSelection];
        if (maxNumber > 0) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    STPHGridViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:QTPHGridViewCellIdentifier
...
{
    PHAsset *asset = self.assetsFetchResults[indexPath.item];
    BOOL mixedSelection = [[STPHImgPickerManager sharedInstance] mixedSelection];
    if ((self.picker.selectedAssets.count && [(PHAsset *)self.picker.selectedAssets.firstObject mediaType] != asset.mediaType) || mixedSelection == NO) {
        [STTipsView showTips:[NSBundle qim_localizedStringForKey:@"You cannot select photos and videos at the same time"] InView:self.view];
        return NO;
    }
    
    if (asset.mediaType == PHAssetMediaTypeVideo) {
        
        BOOL workFeedPicker = [[STPHImgPickerManager sharedInstance] workFeedImagePicker];
        BOOL canContinueSelectionVideo = [[STPHImgPickerManager sharedInstance] canContinueSelectionVideo];
        if (NO == canContinueSelectionVideo) {
            //不允许选择视频
            [STTipsView showTips:[NSBundle qim_localizedStringForKey:@"Unable to upload more videos"] InView:self.view];
            return NO;
        }
        
        BOOL notAllowSelectVideo = [[STPHImgPickerManager sharedInstance] notAllowSelectVideo];
        if (notAllowSelectVideo == YES) {
            //不允许选择视频
            [STTipsView showTips:[NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"Unable to upload videos"]] InView:self.view];
        } else {
            int duration = (int)asset.duration;

            NSInteger configDuration = [[[STKit sharedInstance] userObjectForKey:@"videoMaxTimeLen"] integerValue];
            if (configDuration <= 0) {
                //视频默认15s时长限制
            ...
            if (duration * 1000 > configDuration && workFeedPicker) {
                [STTipsView showTips:[NSString stringWithFormat:@"不支持上传超过%lds的视频", configDuration / 1000] InView:self.view];
            } else {
                ...
                [_imageManager requestAVAssetForVideo:asset options:nil resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        STVideoAssetViewController *vc = [[STVideoAssetViewController alloc]  init];
    ...
    if (maxNumber > 0) {
        if (self.picker.selectedAssets.count >= maxNumber) {
            [STTipsView showTips:[NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"Select a maximum of %d photos"],maxNumber] InView:self.view];
            return NO;
        }
    } else {
        if (self.picker.selectedAssets.count >= kMaximumNumberOfSelection) {
            [STTipsView showTips:[NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"Select a maximum of %d photos"],kMaximumNumberOfSelection] InView:self.view];
            return NO;
        }
    }
    
    STPHGridViewCell *cell = (STPHGridViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
    ...
    [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        ...
        if (imageSize >= 30) {
            //判断图片大于30M，提示，取消选择状态
            [STTipsView showTips:[NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"Unable to select photos exceed 30M"]] InView:self.view];
