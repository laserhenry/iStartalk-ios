...
- (void)qim_setImageWithJid:(NSString *)jid placeholderImage:(UIImage *)placeholder{
    [self qim_setImageWithJid:jid WithChatType:ChatType_SingleChat placeholderImage:[UIImage imageWithData:[STKit defaultUserHeaderImage]]];
}
...
- (void)qim_setImageWithJid:(NSString *)jid WithRealJid:(NSString *)realJid WithChatType:(ChatType)chatType placeholderImage:(UIImage *)placeholder {
    ...
    dispatch_async([[STKit sharedInstance] getLoadHeaderImageQueue], ^{

        switch (chatType) {
            case ChatType_SingleChat: {
                headerUrl = [[STKit sharedInstance] getUserBigHeaderImageUrlWithUserId:jid];
                if (!placeholder) {
                    placeholderImage = [UIImage imageWithData:[STKit defaultUserHeaderImage]];
                }
            }
                break;
            case ChatType_GroupChat: {
                headerUrl = [[STKit sharedInstance] getGroupBigHeaderImageUrlWithGroupId:jid];
                if (!placeholder) {
                    placeholderImage = [STKit defaultGroupHeaderImage];
                }
            }
                break;
            ...
            case ChatType_Consult: {
                headerUrl = [[STKit sharedInstance] getUserHeaderSrcByUserId:jid];
                if (!placeholderImage) {
                    placeholderImage = [UIImage imageWithData:[STKit defaultUserHeaderImage]];
                }
            }
            ...
            case ChatType_ConsultServer: {
                headerUrl = [[STKit sharedInstance] getUserHeaderSrcByUserId:realJid];
                if (!placeholderImage) {
                    placeholderImage = [UIImage imageWithData:[STKit defaultUserHeaderImage]];
                }
            }
        ...
        if (![headerUrl qim_hasPrefixHttpHeader] && headerUrl.length > 0) {
            ...
            if (!headerUrl.length && (jid || realJid)) {
                if (chatType == ChatType_GroupChat) {
                    [[STKit sharedInstance] updateGroupCardByGroupId:jid withCache:YES];
                } else if (chatType == ChatType_ConsultServer) {
                    [[STKit sharedInstance] updateUserCard:realJid withCache:YES];
                } else {
...
- (void)qim_setCollectionImageWithJid:(NSString *)jid WithChatType:(ChatType)chatType {
    ...
    dispatch_async([[STKit sharedInstance] getLoadHeaderImageQueue], ^{
        switch (chatType) {
            case ChatType_SingleChat: {
                headerUrl = [[STKit sharedInstance] getCollectionUserHeaderUrlWithXmppId:jid];
                placeholderImage = [UIImage imageWithData:[STKit defaultUserHeaderImage]];
            }
                break;
            case ChatType_GroupChat: {
                headerUrl = [[STKit sharedInstance] getCollectionGroupHeaderUrlWithCollectionGroupId:jid];
                placeholderImage = [STKit defaultGroupHeaderImage];
            }
                break;
            default:
        ...
        if (![headerUrl qim_hasPrefixHttpHeader] && headerUrl.length > 0) {
            ...
            if (!headerUrl && jid) {
                if (chatType == ChatType_GroupChat) {
                    [[STKit sharedInstance] updateCollectionGroupCardByGroupId:jid];
                } else {
                    [[STKit sharedInstance] updateCollectionUserCardByUserIds:@[jid]];
                }
            } else {
