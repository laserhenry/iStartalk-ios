    ...
    if (self.videoModel.LocalVideoOutPath > 0) {
        if ([[NSFileManager defaultManager] fileExistsAtPath:self.videoModel.LocalVideoOutPath]) {
            ...
            if (![self.videoModel.FileUrl qim_hasPrefixHttpHeader]) {
                self.videoModel.FileUrl = [[STKit sharedInstance].qimNav_InnerFileHttpHost stringByAppendingFormat:@"/%@", self.videoModel.FileUrl];
            }
...
- (void)viewDidLoad {
    ...
    if (self.videoModel.newVideo == YES) {
        ...
        NSString *fileMd5 = [[STKit sharedInstance] md5fromUrl:self.playUrl];
        NSString *fileExt = [[STKit sharedInstance] getFileExtFromUrl:self.playUrl];
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:videoCachePath] && videoCachePath.length > 0) {
            ...
            [[STKit sharedInstance] downloadFileRequest:self.playUrl withTargetFilePath:videoCachePath withProgressBlock:^(float progressValue) {
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)shareVideoToWorkMoment {
    ...
    [STFastEntrance presentWorkMomentPushVCWithVideoDic:videoDic withNavVc:self.navigationController];
}
