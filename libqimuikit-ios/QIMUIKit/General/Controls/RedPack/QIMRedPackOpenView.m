...
- (UILabel *)desLabel {
    if (!_desLabel) {
        ...
        NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:self.userJid];
...
- (void)openRedPack:(id)sender {
    ...
    [[STKit sharedInstance] grapRedEnvelop:self.chatId RedRid:self.redId IsChatRoom:self.isRoom withCallBack:^(NSString *rid) {
