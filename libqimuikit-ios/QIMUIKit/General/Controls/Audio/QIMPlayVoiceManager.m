...
- (void)playNextVoice:(NSNotification *)notify {
    ...
    NSString *msgId = [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] getMsgIdWithChatId:self.chatId index:index];
...
- (NSInteger)currentMsgIndex {
    ...
    return [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] getIndexOfMsgIdWithChatId:self.chatId msgId:self.currentMsgId];
}
...
- (void)playVoiceWithMsgId:(NSString *)msgId {
    ...
    NSDictionary *infoDic = [[STKit sharedInstance] getMsgDictByMsgId:msgId];
