...
- (void)prepareForVoiceURL:(NSString *)voiceUrl playAfterReady:(BOOL)playAfterReady {
    ...
    if (![voiceUrl qim_hasPrefixHttpHeader]) {
        voiceUrl =  [[STKit sharedInstance].qimNav_InnerFileHttpHost stringByAppendingFormat:@"/%@", voiceUrl];
    } 
    if ([voiceUrl rangeOfString:@"?"].location == NSNotFound) {
        voiceUrl = [voiceUrl stringByAppendingFormat:@"?u=%@&k=%@",[[STKit getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[[STKit sharedInstance] myRemotelogginKey]];
    } else {
        voiceUrl = [voiceUrl stringByAppendingFormat:@"&u=%@&k=%@",[[STKit getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[[STKit sharedInstance] myRemotelogginKey]];
    }
    [[STKit sharedInstance] sendTPGetRequestWithUrl:[voiceUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] withProgressCallBack:^(float progressValue) {
