...
@interface STFilePreviewVC ()<WKNavigationDelegate>{
...
@implementation STFilePreviewVC
...
- (void)viewDidLoad {
    ...
    NSString *downLoad = [[STKit sharedInstance] getDownloadFilePath];
    ...
    if (![fileUrl qim_hasPrefixHttpHeader]) {
        fileUrl = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_HttpHost], fileUrl];
    }
    ...
    NSString *fileMd5 = [[STKit sharedInstance] getFileNameFromUrl:fileUrl];
    if (!fileMd5.length) {
        fileMd5 = [infoDic objectForKey:@"FileMd5"];
    }
    NSString *fileExt = [[STKit sharedInstance] getFileExtFromUrl:fileUrl];
    if (!fileExt.length) {
...
- (void)onDownloadButton:(UIButton *)sender{
    ...
    [[STKit sharedInstance] downloadFileRequest:[fileUrl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] withTargetFilePath:_filePath withProgressBlock:^(float progressValue) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)initWebView{
    ...
    _previewWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, _bottomView.top) configuration:wkWebConfig];
...
- (void)loadFileError {
    ...
    [infoLabel setTextAlignment:NSTextAlignmentCenter];
    [infoLabel setText:[NSString stringWithFormat:@"%@%@", [STKit getQIMProjectTitleName], @"暂不支持打开此类文件 \n 可使用前其他应用打开并预览"]];
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showUI)];
