...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if ([CLLocationManager authorizationStatus] == kCLAuthorizationStatusDenied || [CLLocationManager authorizationStatus] == kCLAuthorizationStatusRestricted) {
        ...
        if (self.shareLocationId) {
            NSArray * userList = [[STKit sharedInstance] getShareLocationUsersByShareLocationId:self.shareLocationId];
            
            [[STKit sharedInstance] joinShareLocationToUsers:userList WithShareLocationId:self.shareLocationId];
        }else{
            self.shareLocationId = [QIMUUIDTools UUID];
            if ([self.userId rangeOfString:@"@conference."].location != NSNotFound) {
                [[STKit sharedInstance] beginShareLocationToGroupId:self.userId WithShareLocationId:self.shareLocationId];
            }else{
               STMsgModel * msg = [[STKit sharedInstance] beginShareLocationToUserId:self.userId WithShareLocationId:self.shareLocationId];
...
- (void)closeBtnHandle:(id)sender{
    ...
    [[STKit sharedInstance] quitShareLocationToUsers:[[STKit sharedInstance] getShareLocationUsersByShareLocationId:self.shareLocationId] WithShareLocationId:self.shareLocationId];
    [self dismissViewControllerAnimated:YES completion:nil];
}
...
- (void)myLctBtnHandle:(id)sender{
    [self selectAnnotationForUserId:[[STKit sharedInstance] getLastJid]];
}
...
-(void)mapView:(MKMapView *)mapView didUpdateUserLocation:(MKUserLocation *)userLocation{
    ...
    if (![_annotationsDic.allKeys containsObject:[[STKit sharedInstance] getLastJid]]) {
   ...
   CLLocationDirection mapAngle = -_mapView.camera.heading;
    MyAnnotation * annotation = [_annotationsDic objectForKey:[[STKit sharedInstance] getLastJid]];
    if (annotation == nil) {
        MyAnnotation * annotation = [[MyAnnotation alloc] initWithCoordinates:coord title:@"当前位置" subTitle:@"我的子标题"];
        annotation.identifier = [[STKit sharedInstance] getLastJid];
        annotation.userId = [[STKit sharedInstance] getLastJid];
        annotation.headingDirection = mapAngle + _mapView.userLocation.heading.trueHeading;
        [_annotationsDic setQIMSafeObject:annotation forKey:[[STKit sharedInstance] getLastJid]];
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations{
    ...
    CLLocation * srcLocation = locations.firstObject;
        ...
        if (_annotationsDic.allKeys.count == 0 || ![_annotationsDic.allKeys containsObject:[[STKit sharedInstance] getLastJid]]) {
            //        CLLocationCoordinate2D centerCoordinate = CLLocationCoordinate2DMake((srcLocation.coordinate.latitude + desLocation.coordinate.latitude) / 2, (srcLocation.coordinate.longitude + desLocation.coordinate.longitude) / 2);
...
-(void)locationManager:(CLLocationManager *)manager didUpdateHeading:(CLHeading *)newHeading{
    ...
    CLLocationDirection mapAngle = -_mapView.camera.heading;
    MKAnnotationView * antView = [_antViewsDic objectForKey:[[STKit sharedInstance] getLastJid]];
    if (antView) {
        ShareLocationUserImageView * shareView = [antView viewWithTag:9999];
        [shareView updateDirectionTo: mapAngle +  _currentHeading.trueHeading];
    }
    
    MyAnnotation * ant = [_annotationsDic objectForKey:[[STKit sharedInstance] getLastJid]];
    if (ant) {
        ant.headingDirection = mapAngle + _currentHeading.trueHeading;
        [_annotationsDic setQIMSafeObject:ant forKey:[[STKit sharedInstance] getLastJid]];
    }
...
- (void)sendShareLocationMsgWithLatitude:(double)latitude longitude:(double)longitude headingDirection:(double)direction{
    ...
    [[STKit sharedInstance] sendMyLocationToUsers:[[STKit sharedInstance] getShareLocationUsersByShareLocationId:self.shareLocationId] WithLocationInfo:[[QIMJSONSerializer sharedInstance] serializeObject:infoDic] ByShareLocationId:self.shareLocationId];
}
...
- (void)updateShareLocationMsg:(NSNotification *)noti{
   STMsgModel * msg = [noti.userInfo objectForKey:@"message"];
    if ([msg.to isEqualToString:[[STKit sharedInstance] getLastJid]]) {
        CLLocationDirection mapAngle = -_mapView.camera.heading;
