...
-(void)setImageURL:(NSString *)imageURL
{
    ...
    if (imageURL) {
        ...
        id image =  [[STDataContraller getInstance] getResourceImage:imageURL];
...
//                    [[DataController getInstance] addResource:[dic objectForKey:@"ResultData"] withKey:[dic objectForKey:@"UrlStr"]];
                    [[STDataContraller getInstance] saveResourceWithFileName:[dic objectForKey:@"UrlStr"] data:[dic objectForKey:@"ResultData"]];
                }
                else {
                    [self setImage:nil];
                    [[STDataContraller getInstance] addResource:[NSNull null] withKey:[dic objectForKey:@"UrlStr"]];
                }
            }];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (image) {
        ...
        [[STDataContraller getInstance] addResource:_receivedData withKey:[[self.urlConnDict allKeys] objectAtIndex:0]];
        [self setImage:image];
    }
    else
    {
        [[STDataContraller getInstance] addResource:[NSNull null] withKey:[[self.urlConnDict allKeys] objectAtIndex:0]];
        [self setImage:nil];
    }
}
