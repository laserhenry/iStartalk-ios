    ...
    for (STMsgModel * msg in localMsgs) {
        ...
        if (msgType == QIMMessageType_SmallVideo) {
            ...
            if (![ThumbUrl qim_hasPrefixHttpHeader] && ThumbUrl.length > 0) {
                ThumbUrl = [[STKit sharedInstance].qimNav_InnerFileHttpHost stringByAppendingFormat:@"/%@", ThumbUrl];
            } else {
                ThumbUrl = ThumbUrl;
            }
            NSString *fileUrl = [videoExtendInfoDic objectForKey:@"FileUrl"];
            if (![fileUrl qim_hasPrefixHttpHeader] && fileUrl.length > 0) {
                fileUrl = [[STKit sharedInstance].qimNav_InnerFileHttpHost stringByAppendingFormat:@"/%@", fileUrl];
            } else {
                fileUrl = fileUrl;
            }
...
- (void)loadLocalMedia {
    ...
    if (self.chatType == ChatType_ConsultServer || self.chatType == ChatType_Consult) {
        localMedia = [[STKit sharedInstance] getLocalMediasByXmppId:self.xmppId ByRealJid:self.realJid];
        NSLog(@"localMedia : %@", localMedia);
    } else {
        localMedia = [[STKit sharedInstance] getLocalMediasByXmppId:self.xmppId ByRealJid:nil];
        NSLog(@"localMedia : %@", localMedia);
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[STFastEntrance sharedInstance] browseBigHeader:@{@"MWPhotoList": self.dimensionalPhotos, @"CurrentIndex":@(tag)}];
}
...
- (void)shareMedias:(id)sender {
    if (self.selectMediaArray.count) {
        ...
        UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:[NSBundle qim_localizedStringForKey:@"Reminder"] message:[NSBundle qim_localizedStringForKey:@"Please share after selecting videos or photos"] preferredStyle:UIAlertControllerStyleAlert];
...
- (void)downloadMedia:(id)sender {
    if (self.selectMediaArray.count > 0) {
        ...
        UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:[NSBundle qim_localizedStringForKey:@"Reminder"] message:@"请选择视频或图片后再保存" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"ok" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
