    ...
    switch (buttonIndex) {
        case 1: {
            ...
            if (photoUrl.length > 0) {
                ...
                [[STKit sharedInstance] qim_uploadImageWithImageData:photo.photoData WithMsgId:nil WithMsgType:QIMMessageType_ImageNew WithPathExtension:nil withCallBack:^(NSString *fileName) {
                    NSString *fileUrl = @"";
                    if ([fileName qim_hasPrefixHttpHeader]) {
                        fileUrl = fileName;
                    } else {
                        fileUrl = [NSString stringWithFormat:@"%@/LocalFileName=%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], fileName];
                    }
