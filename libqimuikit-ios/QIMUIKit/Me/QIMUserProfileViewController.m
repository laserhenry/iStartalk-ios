...
- (NSMutableArray *)dataSource {
    if (!_dataSource) {
        ...
        if (self.myOwnerProfile) {
            ...
            if ([[STKit sharedInstance] qimNav_OpsHost].length > 0 && [STKit getQIMProjectType] != QIMProjectTypeQChat) {
...
- (void)loadUserInfo {
    self.userInfo = [[STKit sharedInstance] getUserInfoByUserId:self.userId];
}

- (void)getUserVcardInfo {
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:weakSelf.userId];
...
- (void)viewDidLoad {
    ...
    [self getUserVcardInfo];
    [self loadUserInfo];
    NSDictionary *friendDic = [[STKit sharedInstance] selectFriendInfoWithUserId:[self.userInfo objectForKey:@"XmppId"]];
...
- (void)deleteFriend {
    ...
    UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:@"删除好友" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            BOOL isSuccess = [[STKit sharedInstance] deleteFriendWithXmppId:[self.userInfo objectForKey:@"XmppId"] WithMode:2];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)addFriend {
    NSDictionary *modeDic = [[STKit sharedInstance] getVerifyFreindModeWithXmppId:self.userId];
    ...
    switch (mode) {
        ...
        {
            [[STKit sharedInstance] addFriendPresenceWithXmppId:self.userId WithAnswer:nil];
        }
            break;
...
- (void)openChatSession {
    [STFastEntrance openSingleChatVCByUserId:self.userId];
}
