...
- (void)loadUserInfo {
    if ([STKit getQIMProjectType] == QIMProjectTypeQChat) {
        self.userInfo = [[STKit sharedInstance] getUserInfoByUserId:[[STKit sharedInstance] getLastJid]];
        _userId = [[STKit sharedInstance] getLastJid];
        ...
        self.userInfo = [NSMutableDictionary dictionaryWithDictionary:[[STKit sharedInstance] getUserInfoByUserId:[[STKit sharedInstance] getLastJid]]];
...
- (void)getUserVcardInfo {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        if (weakSelf.userId) {
            NSDictionary * usersInfo = [[STKit sharedInstance] getUserInfoByUserId:weakSelf.userId];
