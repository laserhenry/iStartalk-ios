...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (position) {
        ...
        if (textField.text.length + string.length - range.length >= searchMinCharacterCount) {
            ...
            [[STKit sharedInstance] searchMomentWithKey:searchText withSearchTime:0 withStartNum:0 withPageNum:20 withSearchType:0 withCallBack:^(NSArray *result) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    __weak __typeof(self) weakSelf = self;
    [self showProgressHUDWithMessage:[NSBundle qim_localizedStringForKey:@"moment_searhing"]];
    [[STKit sharedInstance] searchMomentWithKey:textField.text withSearchTime:0 withStartNum:0 withPageNum:20 withSearchType:0 withCallBack:^(NSArray *result) {
        __typeof(self) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
- (void)searchMore {
    __weak __typeof(self) weakSelf = self;
    [[STKit sharedInstance] searchMomentWithKey:self.textField.text withSearchTime:0 withStartNum:self.searchDataList.count withPageNum:20 withSearchType:0 withCallBack:^(NSArray *result) {
        __typeof(self) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
