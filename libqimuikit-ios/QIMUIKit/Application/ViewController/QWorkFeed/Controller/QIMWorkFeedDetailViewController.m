...
- (QIMWorkMomentModel *)momentModel {
    if (!_momentModel) {
        NSDictionary *momentDic = [[STKit sharedInstance] getWorkMomentWithMomentId:self.momentId];
        if (!momentDic.count) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                [[STKit sharedInstance] getRemoteMomentDetailWithMomentUUId:self.momentId withCallback:^(NSDictionary *momentDic) {
...
- (void)loadLocalComments {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[STKit sharedInstance] getWorkCommentWithLastCommentRId:0 withMomentId:self.momentId WithLimit:20 WithOffset:0 withFirstLocalComment:YES WithComplete:^(NSArray * comments) {
            if (comments) {
...
- (void)loadComments {
    ...
    __weak typeof(self) weakSelf = self;
    [[STKit sharedInstance] getRemoteRecentHotCommentsWithMomentId:self.momentId withHotCommentCallBack:^(NSArray *hotComments) {
        if ([hotComments isKindOfClass:[NSArray class]]) {
            ...
            [[STKit sharedInstance] setHotCommentUUIds:hotCommentIds ForMomentId:self.momentId];
            [weakSelf.commentListView reloadCommentsData];
        } else {
            [[STKit sharedInstance] setHotCommentUUIds:@[] ForMomentId:self.momentId];
        }

        //拉完热评之后再拉新评
        [[STKit sharedInstance] getRemoteRecentNewCommentsWithMomentId:self.momentId withNewCommentCallBack:^(NSArray *comments) {
            if (comments) {
                [weakSelf.commentListView.commentModels removeAllObjects];
                for (NSDictionary *commentDic in comments) {
                    QIMWorkCommentModel *model = [weakSelf getCommentModelWithDic:commentDic];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [[STKit sharedInstance] removeHotCommentUUIdsForMomentId:self.momentId];
}
...
//        NSDictionary *postDic = @{@"ChildModel": commentModel, @"ParentIndexPath" : self.parentCommentIndexPath};
        ...
        if ([self.commentInputBar isInputBarFirstResponder] == NO) {
            ...
            if ([fromUserId isEqualToString:[[STKit sharedInstance] getLastJid]]) {
                //自己实名发的评论
                ...
                LCActionSheet *actionSheet = [LCActionSheet sheetWithTitle:nil
                                                         cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"Cancel"]
                                                                   clicked:^(LCActionSheet * _Nonnull actionSheet, NSInteger buttonIndex) {
                                                                       ...
                                                                       if (buttonIndex == 1) {
                                                                           //删评论
                                                                           [[STKit sharedInstance] deleteRemoteCommentWithComment:commentModel.commentUUID withPostUUId:commentModel.postUUID withSuperParentUUId:commentModel.superParentUUID withCallback:^(BOOL success, NSInteger superStatus) {
                                                                               if (success) {
                                                                                   dispatch_async(dispatch_get_main_queue(), ^{
                                                                                       if (superStatus == 2) {
...
- (void)loadMoreComments {
    ...
    [[STKit sharedInstance] getWorkCommentWithLastCommentRId:lastModel.rId withMomentId:self.momentId WithLimit:20 WithOffset:self.commentListView.commentModels.count withFirstLocalComment:NO WithComplete:^(NSArray * _Nonnull array) {
...
- (void)didaddCommentWithStr:(NSString *)str withAtList:(NSArray *)atList {
    if (self.staticCommentModel) {
        ...
        [commentDic setQIMSafeObject:[STKit getLastUserName] forKey:@"fromUser"];
        [commentDic setQIMSafeObject:[[STKit sharedInstance] getDomain] forKey:@"fromHost"];
        ...
        [commentDic setQIMSafeObject:[[STKit sharedInstance] getHotCommentUUIdsForMomentId:self.momentId] forKey:@"hotCommentUUID"];
        [commentDic setQIMSafeObject:atList forKey:@"atList"];

        [[STKit sharedInstance] uploadCommentWithCommentDic:commentDic];
    } else {
        //评论帖子
        QIMWorkMomentUserIdentityModel *lastUserModel = [[QIMWorkMomentUserIdentityManager sharedInstanceWithPOSTUUID:self.momentId] userIdentityModel];
