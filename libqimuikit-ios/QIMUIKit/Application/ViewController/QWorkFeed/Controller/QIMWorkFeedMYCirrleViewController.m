...
- (NSArray *)qImWorkFeedMessageViewOriginDataSourceWithViewTag:(NSInteger)viewTag {
    if (viewTag == 0) {
        return [[STKit sharedInstance] getWorkNoticeMessagesWithLimit:20 WithOffset:0 eventTypes:@[@(QIMWorkFeedNotifyTypeMyComment)]];
    } else {
        
        return [[STKit sharedInstance] getWorkNoticeMessagesWithLimit:20 WithOffset:0 eventTypes:@[@(QIMWorkFeedNotifyTypePOSTAt), @(QIMWorkFeedNotifyTypeCommentAt)]];
    }
}

- (NSDictionary *)qImWorkFeedMessageViewModelWithMomentPostUUID:(NSString *)momentId viewTag:(NSInteger)viewTag{
    return [[STKit sharedInstance] getWorkMomentWithMomentId:momentId];
}
...
- (void)qImWorkFeedMessageViewMoreDataSourceWithviewTag:(NSInteger)viewTag finishBlock:(void(^)(NSArray * arr))block{
    if (viewTag == 0) {
        if (self.myReplyView.noticeMsgs.count > 0) {
            NSArray *tempArr = [[STKit sharedInstance] getWorkNoticeMessagesWithLimit:20 WithOffset:self.myReplyView.noticeMsgs.count eventTypes:@[@(QIMWorkFeedNotifyTypeMyComment)]];
            if (tempArr.count > 0) {
                block(tempArr);
            }
            else{
                QIMWorkNoticeMessageModel * model = self.myReplyView.noticeMsgs.lastObject;
                [[STKit sharedInstance] getRemoteOwnerCamelGetMyReplyWithCreateTime:model.createTime pageSize:20 complete:block];
            }
        }
        else{
            [[STKit sharedInstance] getRemoteOwnerCamelGetMyReplyWithCreateTime:0 pageSize:20 complete:block];
        }
    }
    else{
        if (self.atMeView.noticeMsgs.count > 0) {
            NSArray *tempArr = [[STKit sharedInstance] getWorkNoticeMessagesWithLimit:20 WithOffset:self.atMeView.noticeMsgs.count eventTypes:@[@(QIMWorkFeedNotifyTypePOSTAt), @(QIMWorkFeedNotifyTypeCommentAt)]];
            if (tempArr.count > 0) {
                block(tempArr);
            }
            else{
                QIMWorkNoticeMessageModel * model = self.atMeView.noticeMsgs.lastObject;
                [[STKit sharedInstance] getRemoteOwnerCamelGetAtListWithCreateTime:model.createTime pageSize:20 complete:block];
            }
        }
        else{
            [[STKit sharedInstance] getRemoteOwnerCamelGetAtListWithCreateTime:0 pageSize:20 complete:block];
        }
    }
}
