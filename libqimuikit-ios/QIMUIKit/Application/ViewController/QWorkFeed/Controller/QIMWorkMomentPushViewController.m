...
- (void)updatePushUserIdentityView {
    ...
    if (isAnonymous == NO) {
        [self.iconView qim_setImageWithJid:[[STKit sharedInstance] getLastJid]];
...
//        [[QTPHImagePickerManager sharedInstance] setNotAllowSelectVideo:((self.workFeedContentType == QIMWorkFeedContentTypeVideo) || (self.workFeedContentType == QIMWorkFeedContentTypeImage))];
        [[STPHImgPickerManager sharedInstance] setCanContinueSelectionVideo:NO];
        [[STPHImgPickerManager sharedInstance] setMixedSelection:NO];
...
//            [[QTPHImagePickerManager sharedInstance] setNotAllowSelectVideo:((self.workFeedContentType == QIMWorkFeedContentTypeVideo) || (self.workFeedContentType == QIMWorkFeedContentTypeImage))];
            ...
            [[STPHImgPickerManager sharedInstance] setMaximumNumberOfSelection:maxCount];
            [[STPHImgPickerManager sharedInstance] setCanContinueSelectionVideo:NO];
            ...
            [[STPHImgPickerManager sharedInstance] setMaximumNumberOfSelection:9];
            [[STPHImgPickerManager sharedInstance] setMixedSelection:YES];
            [[STPHImgPickerManager sharedInstance] setCanContinueSelectionVideo:YES];
        }
    }
...
- (void)viewDidLoad {
    ...
    [[STPHImgPickerManager sharedInstance] setCanContinueSelectionVideo:YES];
    [[STPHImgPickerManager sharedInstance] setMixedSelection:YES];
    [[STPHImgPickerManager sharedInstance] setWorkFeedImagePicker:YES];
    ...
    [qNoticeVC onQIMWorkFeedSelectUser:^(NSArray *selectUsers) {
        ...
        for (NSString *userXmppJid in selectUsers) {
            if (userXmppJid.length > 0) {
                NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:userXmppJid];
                if (userInfo.count > 0) {
...
- (void)pushNewMoment:(id)sender {
    ...
    if (self.textView.text.length > QIMWORKMOMENTLIMITNUM) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [STTipsView showTips:[NSString stringWithFormat:@"动态内容不能超过%ld字哦～", QIMWORKMOMENTLIMITNUM] InView:self.view];
        });
        return;
    }
    ...
    if (emptyContent == YES) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [momentDic setQIMSafeObject:[STKit getLastUserName] forKey:@"owner"];
            [momentDic setQIMSafeObject:[[STKit sharedInstance] getDomain] forKey:@"ownerHost"];
            ...
            for (id mediaData in self.selectPhotos) {
                ...
                if (mediaType == QIMWorkMomentMediaTypeImage) {
                    ...
                    if ([imageData isKindOfClass:[NSData class]]) {
                        ...
                        [[STKit sharedInstance] qim_uploadImageWithImageData:imageData WithMsgId:[QIMUUIDTools UUID] WithMsgType:QIMMessageType_Image WithPathExtension:@"png" withCallBack:^(NSString *fileUrl) {
                            if (fileUrl.length > 0) {
...
//                    NSDictionary *videoDic = @{@"MediaType":@(1), @"VideoDic": @{@"LocalVideoOutPath" : LocalVideoOutPath, @"fileSizeStr":fileSizeStr, @"videoDuration":@(videoDuration), @"thumbImage":thumbImage}};
                    ...
                    if (videoReady == YES) {
                        ...
                        if ([videoData isKindOfClass:[NSData class]]) {
                            ...
                            [[STKit sharedInstance] qim_uploadVideo:LocalVideoOutPath videoDic:videoDic withCallBack:^(NSDictionary *videoDic, BOOL needTrans) {
                ...
                if (NO == uploadSuccessed) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        dispatch_after(3, dispatch_get_main_queue(), ^{
                            [STTipsView showTips:[NSString stringWithFormat:@"上传驼圈失败，请稍后重试"] InView:self.view];
                        });
                    });
                } else {
                    ...
                    [[STKit sharedInstance] pushNewMomentWithMomentDic:momentDic withCallBack:^(BOOL successed) {
                        if (successed) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [self hideProgressHUD:YES];
                            });
                            ...
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [self hideProgressHUD:YES];
                                dispatch_after(3, dispatch_get_main_queue(), ^{
                                    [STTipsView showTips:[NSString stringWithFormat:[NSBundle qim_localizedStringForKey:@"moment_faild_share"]] InView:self.view];
                                });
                            });
                        }
                    }];
            ...
            if (videoReady == YES) {
                ...
                if (firstThumbUrl.length > 0) {
                    if (![firstThumbUrl qim_hasPrefixHttpHeader]) {
                        firstThumbUrl = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], firstThumbUrl];
                    }
    ...
    if ([photoData isKindOfClass:[NSString class]]) {
        ...
        if (mediaType == QIMWorkMomentMediaTypeImage) {
            ...
            if (videoReady == YES) {
                ...
                [STFastEntrance openVideoPlayerForVideoModel:videoModel];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (isAnonymous == NO) {
        [cell.iconView qim_setImageWithJid:[[STKit sharedInstance] getLastJid]];
...
- (void)assetsPickerController:(STPHImgPickerController *)picker didFinishPickingAssets:(NSArray *)assets {
...
- (void)dealloc {
    ...
    [[STPHImgPickerManager sharedInstance] setCanContinueSelectionVideo:YES];
    [[STPHImgPickerManager sharedInstance] setMixedSelection:YES];
    [[STPHImgPickerManager sharedInstance] setWorkFeedImagePicker:NO];
}
