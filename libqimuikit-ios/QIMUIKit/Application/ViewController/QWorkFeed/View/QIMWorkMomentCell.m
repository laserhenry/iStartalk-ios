...
- (void)setMoment:(QIMWorkMomentModel *)moment {
    ...
    if ([userId isEqualToString:[[STKit sharedInstance] getLastJid]] && self.notShowControl == NO) {
    ...
    if (moment.isAnonymous == NO) {
        ...
        _nameLab.text = [[STKit sharedInstance] getUserMarkupNameWithUserId:userId];
        _nameLab.textColor = [UIColor qim_colorWithHex:0x00CABE];
        [_nameLab sizeToFit];
...
//                [[QIMEmotionManager sharedInstance] decodeHtmlUrlForText:moment.content.exContent];
    ...
    if (self.isSearch == YES && _moment.likeNum <= 0) {
        NSDictionary *momentDic = [[STKit sharedInstance] getWorkMomentWithMomentId:self.moment.momentId];
...
- (void)didLikeMoment:(UIButton *)sender {
    ...
    [[STKit sharedInstance] likeRemoteMomentWithMomentId:self.moment.momentId withLikeFlag:likeFlag withCallBack:^(NSDictionary *responseDic) {
...
- (void)didTapWorkMomentShareLinkUrl:(QIMWorkMomentContentLinkModel *)linkModel {
    if (linkModel.linkurl.length > 0) {
        [STFastEntrance openWebViewForUrl:linkModel.linkurl showNavBar:linkModel.showbar];
    }
}
