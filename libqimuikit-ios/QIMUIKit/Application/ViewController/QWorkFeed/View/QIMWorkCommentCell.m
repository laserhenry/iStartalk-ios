...
- (NSArray *)getChildComments {
    ...
    NSArray *childComments = [[STKit sharedInstance] getWorkChildCommentsWithParentCommentUUID:self.commentModel.commentUUID];
...
//        QIMVerboseLog(@"commentFromUserId : %@ --- %@", commentFromUserId, commentModel.commentUUID);
        ...
        _nameLab.text = [[STKit sharedInstance] getUserMarkupNameWithUserId:commentFromUserId];
        [_nameLab sizeToFit];
        
        _organLab.frame = CGRectMake(self.nameLab.right + 5, self.nameLab.top, 66, 20);
        NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:commentFromUserId];
        ...
        if (![anonymousPhoto qim_hasPrefixHttpHeader]) {
            anonymousPhoto = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], anonymousPhoto];
        }
    ...
    if (isChildComment) {
        if (toisAnonymous) {
            ...
            NSString *toUserName = [[STKit sharedInstance] getUserMarkupNameWithUserId:toUserId];
    ...
    STMsgModel *msg = [[STMsgModel alloc] init];
...
- (void)didLikeComment:(UIButton *)sender {
    ...
    [[STKit sharedInstance] likeRemoteCommentWithCommentId:self.commentModel.commentUUID withSuperParentUUID:self.commentModel.superParentUUID withMomentId:self.commentModel.postUUID withLikeFlag:likeFlag withCallBack:^(NSDictionary *responseDic) {
...
- (void)attributedLabel:(QIMAttributedLabel *)attributedLabel textStorageClicked:(id<QIMTextStorageProtocol>)textStorage atPoint:(CGPoint)point {
    if ([textStorage isMemberOfClass:[QIMLinkTextStorage class]]) {
        ...
        if (![storage.linkData length]) {
            ...
            [STFastEntrance openWebViewForUrl:storage.linkData showNavBar:YES];
        }
    } else {
