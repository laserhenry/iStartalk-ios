...
- (void)setNoticeMsgModel:(QIMWorkNoticeMessageModel *)noticeMsgModel {
    ...
    if (fromIsAnonymous) {
        ...
        if (![fromAnonymousPhoto qim_hasPrefixHttpHeader]) {
            fromAnonymousPhoto = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], fromAnonymousPhoto];
        }
        ...
        if (toUser.length > 0) {
            ...
            if (toIsAnonymous) {
                ...
                //给实名用户评论
                NSString *toUserId = [NSString stringWithFormat:@"%@@%@", noticeMsgModel.userTo, noticeMsgModel.userToHost ? noticeMsgModel.userToHost : [[STKit sharedInstance] getDomain]];
                NSString *userName = [[STKit sharedInstance] getUserMarkupNameWithUserId:toUserId];
                if ([toUserId isEqualToString:[[STKit sharedInstance] getLastJid]]) {
                    userName = @"我";
                }
        ...
        NSString *userId = [NSString stringWithFormat:@"%@@%@", userFrom, userFromHost ? userFromHost : [[STKit sharedInstance] getDomain]];
        if (userId.length > 0) {
            [self.headerImageView qim_setImageWithJid:userId placeholderImage:[UIImage imageWithData:[STKit defaultUserHeaderImage]]];
            self.nameLabel.text = [[STKit sharedInstance] getUserMarkupNameWithUserId:userId];
            self.nameLabel.centerY = self.headerImageView.centerY;
            [self.nameLabel sizeToFit];
