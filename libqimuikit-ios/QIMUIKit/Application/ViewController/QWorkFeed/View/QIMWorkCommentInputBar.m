...
- (void)reloadUserIdentifier {
    ...
    if (isAnonymous == NO) {
        [_headerImageView qim_setImageWithJid:[[STKit sharedInstance] getLastJid]];
    } else {
...
- (void)didLikeComment:(UIButton *)sender {
    ...
    [[STKit sharedInstance] likeRemoteMomentWithMomentId:self.momentId withLikeFlag:likeFlag withCallBack:^(NSDictionary *responseDic) {
...
- (void)sendComment {
    if (self.commentTextView.text.length > 200) {
        [STTipsView showTips:[NSString stringWithFormat:@"评论不可以超过200个字哦～"] InView:[[[[UIApplication sharedApplication] keyWindow] rootViewController] view]];
        return;
    }
