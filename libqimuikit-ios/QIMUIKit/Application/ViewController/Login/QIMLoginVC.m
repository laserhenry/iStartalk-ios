...
- (NSMutableArray *)loginUsers {
    if (!_loginUsers) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:[[STKit sharedInstance] userObjectForKey:@"Users"]];
...
//    [self startLoginAnimation];
    ...
    if ([[lastUserName lowercaseString] isEqualToString:@"appstore"]) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] loginWithUserName:lastUserName WithPassWord:lastUserName];
        });
    } else {
...
- (void)loadLoginType {
    self.loginType = [[STKit sharedInstance] qimNav_LoginType];
    [self initUI];
}
...
- (void)initWritingAnimations{
    if (_writingLayer == nil) {
        UIBezierPath *bezierPath = [self transformToBezierPath:[STKit getQIMProjectTitleName]];
            ...
            if ([userName isEqualToString:@"appstore"]) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    if (self.loginType == QTLoginTypeSms) {
                        __block NSString *token = [[STKit sharedInstance] getLastUserToken];
                        //                [[QIMKit sharedInstance] userObjectForKey:@"userToken"];
                        if (token.length <= 0) {
                            [[STKit sharedInstance] getUserTokenWithUserName:userName WithVerifyCode:validCode withCallback:^(NSDictionary *tokenDic) {
                                ...
                                if (statusId == 0) {
                                    ...
                                    [[STKit sharedInstance] updateLastTempUserToken:token];
                        ...
                        [[STKit sharedInstance] getNewUserTokenWithUserName:userName WithPassword:validCode withCallback:^(NSDictionary *result) {
                            if (result) {
                        ...
                        [[STKit sharedInstance] updateLastTempUserToken:validCode];
                        //                [[QIMKit sharedInstance] setUserObject:validCode forKey:@"kTempUserToken"];
                        [[STKit sharedInstance] loginWithUserName:userName WithPassWord:validCode];
                    }
                });
            }
        } else {
...
- (void)getValidCodeTapHandle:(UITapGestureRecognizer *)sender {
    ...
    __weak id weakSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[STKit sharedInstance] getVerifyCodeWithUserName:userName withCallback:^(NSDictionary *result) {
            if (!result) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakSelf showNetWorkUnableAlert];
                    [_getValidCodeBtn setUserInteractionEnabled:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *userFullJid = [accountDict objectForKey:@"userFullJid"];
    NSString *userId = [[userFullJid componentsSeparatedByString:@"@"] firstObject];
    [[STKit sharedInstance] setUserObject:userId forKey:@"currentLoginUserName"];
