...
-(void)login:(id)sender
{
    if (([[self.passwordTextField text] length] > 0)&&([[self.usernameTextField text] length] > 0)) {
        ...
        NSString * RSAPassword = [STRSACoder RSAForPassword:[self.passwordTextField text]];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ 
            ...
            [[STKit sharedInstance] QChatLoginWithUserId:self.usernameTextField.text rsaPassword:RSAPassword type:type withCallback:^(NSDictionary *loginDic) {
                if ([[loginDic objectForKey:@"ret"] boolValue]) {
                    ...
                    if ([type isEqualToString:@"merchant"]) {
                        [[STKit sharedInstance] setIsMerchant:YES];
                    } else {
                        [[STKit sharedInstance] setIsMerchant:NO];
                    }
                    ...
                    [[STKit sharedInstance] loginWithUserName:userName WithPassWord:password];
                    [[STKit sharedInstance] saveUserInfoWithName:userName passWord:[self.passwordTextField text]];
                } else {
...
- (void)initWithUI
{
    ...
    NSString *password = [[STKit sharedInstance] getLastPassword];
