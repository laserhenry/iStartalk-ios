...
+ (NSString *)parseForJsonStrFromMsgList:(NSArray *)msgList withTitle:(NSString *)title{
    ...
    for (STMsgModel * msg in msgList) {
        NSNumber * type = @(msg.messageType);
        NSString *body = msg.message;
        if ([[STKit sharedInstance] getRegisterMsgCellClassForMessageType:msg.messageType]) {
...
+ (NSString *)exportMsgList:(NSArray *)msgList withTitle:(NSString *)title{
    NSString * msgListStr = @"";
    NSString * dateFrom = [[[NSDate qim_dateWithTimeIntervalInMilliSecondSince1970:[(STMsgModel *)msgList.firstObject messageDate]] qim_formattedDateDescription] componentsSeparatedByString:@" "].firstObject;
    NSString * dateTo = [[[NSDate qim_dateWithTimeIntervalInMilliSecondSince1970:[(STMsgModel *)msgList.lastObject messageDate]] qim_formattedDateDescription] componentsSeparatedByString:@" "].firstObject;
...
+ (NSString *)htmlStrForMsg:(STMsgModel *)msg firstTitle:(NSString *)firstTitle {
    ...
    NSDictionary * userInfo = [[STKit sharedInstance] getUserInfoByUserId:msg.from];
    NSString *headerUrl = [[STKit sharedInstance] getUserHeaderSrcByUserId:userInfo[@"XmppId"]];
...
+ (NSString *)getMsgTextForMsg:(STMsgModel *)msg {
    NSString * msgText = @"";
