...
- (void)getDomainList {
    ...
    [[QIMHttpRequestMonitor sharedInstance] syncRunBlock:^{
        ...
        [[STKit sharedInstance] sendTPPOSTFormUrlEncodedRequestWithUrl:urlStr withRequestBodyData:requestData withSuccessCallBack:^(NSData *responseData) {
...
- (void)selectDomain {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [STPickerView dismissWithCompletion:^(NSString *str) {
        ...
        [STPickerView showPickerViewInView:self.view withStrings:_stringsArray withOptions:dict completion:^(NSString *selectedString) {
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    [self.searchBar resignFirstResponder];
    [STPickerView dismissWithCompletion:^(NSString *dismissRow) {
...
- (void)searchList {
    ...
    [[STKit sharedInstance] searchUserListBySearchStr:searchString Url:self.domainURL id:self.domainId limit:limitCount offset:0 withCallBack:^(NSArray *searchList) {
...
{
    [[STKit sharedInstance] openChatSessionByUserId:_infoDic[@"XmppId"]];
    [STFastEntrance openSingleChatVCByUserId:_infoDic[@"XmppId"]];
}
