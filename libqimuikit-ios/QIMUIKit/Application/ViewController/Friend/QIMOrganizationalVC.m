...
- (void)loadRosterList {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_TARGET_QUEUE_DEFAULT, 0), ^{
        if ([[[[STDatasourceItemMgr sharedInstance] getTotalItems] allKeys] count] <= 0) {
            [weakSelf showProgressHUDWithMessage:@"正在加载中..."];
            [[STDatasourceItemMgr sharedInstance] createDataSource];
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)inviteMemberToCompany:(id)sender {
    NSString *inviteStr = [NSString stringWithFormat:@"https://im.qunar.com/new/#/user_list?domain=%@", [[STKit sharedInstance] getDomain]];
    [STFastEntrance openWebViewForUrl:inviteStr showNavBar:YES];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if ([[[[STDatasourceItemMgr sharedInstance] getTotalItems] allKeys] count] <= 0) {
        ...
        NSInteger count = [[[STDatasourceItemMgr sharedInstance] getQIMMergedRootBranch] count];
        return count;
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *key = [[[STDatasourceItemMgr sharedInstance] getQIMMergedRootBranch] objectAtIndex:indexPath.row];
    STDatasourceItem *item = [[STDatasourceItemMgr sharedInstance] getTotalDataSourceItemWithId:key];
    if (!item) {
        item = [[STDatasourceItemMgr sharedInstance] getChildDataSourceItemWithId:key];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *key = [[[STDatasourceItemMgr sharedInstance] getQIMMergedRootBranch] objectAtIndex:indexPath.row];
    STDatasourceItem *item = [[[STDatasourceItemMgr sharedInstance] getTotalItems] objectForKey:key];
    if (!item) {
        item = [[STDatasourceItemMgr sharedInstance] getChildDataSourceItemWithId:key];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (item.childNodesDict.count > 0) {
        if ([item isExpand]) {
            //合起
            [[STDatasourceItemMgr sharedInstance] collapseBranchAtIndex:indexPath.row];
            ...
            //展开
            [[STDatasourceItemMgr sharedInstance] expandBranchAtIndex:indexPath.row];
            id cell = [tableView cellForRowAtIndexPath:indexPath];
            if ([cell isKindOfClass:[QIMBuddyTitleCell class]]) {
        ...
        if (self.shareCard == YES) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [STFastEntrance openUserCardVCByUserId:xmppId];
            });
        }
    }
}
