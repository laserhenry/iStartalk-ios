...
- (void)onHeaderViewClick{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [STFastEntrance openUserCardVCByUserId:[weakSelf.userInfoDic objectForKey:@"XmppId"]];
    });
}
...
- (void)onAgreeClick:(UIButton *)sender{
    ...
    if (xmppId.length > 0) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] agreeFriendRequestWithXmppId:xmppId];
        });
    } else {
...
//       QIMMessageModel *msg = [[QIMKit sharedInstance] createMessageWithMsg:@"我通过了你的朋友验证请求，现在我们可以开始聊天了" extenddInfo:nil userId:xmppid userType:ChatType_SingleChat msgType:QIMMessageType_Text];
        [[STKit sharedInstance] sendMessage:@"我通过了你的朋友验证请求，现在我们可以开始聊天了" WithInfo:nil ToUserId:xmppid WithMsgType:QIMMessageType_Text];
    }
}

- (void)onRefuesClick:(UIButton *)sender{
    [[STKit sharedInstance] refusedFriendRequestWithXmppId:[self.userInfoDic objectForKey:@"XmppId"]];
