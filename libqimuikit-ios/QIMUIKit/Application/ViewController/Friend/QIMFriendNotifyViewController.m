...
- (void)onReceiveFriendPresence:(NSNotification *)notify{
    ...
    if ([jid isEqualToString:notify.object]) {
        ...
        if ([result isEqualToString:@"success"]) {
            ...
            [[STKit sharedInstance] sendMessage:@"我通过了你的朋友验证请求，现在我们可以开始聊天了" WithInfo:nil ToUserId:jid WithMsgType:QIMMessageType_Text];
        } else {
...
- (void)viewDidLoad {
    ...
    _dataSource = [[STKit sharedInstance] selectFriendNotifys];
    [self initWithNav];
...
//    NSString *name = [_currentAgreeDic objectForKey:@"Name"];
    [[STKit sharedInstance] openChatSessionByUserId:xmppid];
