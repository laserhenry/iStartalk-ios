...
- (void)searchList{
    if (self.fromQChat) {
        ...
        [[STKit sharedInstance] searchQunarUserBySearchStr:searchString withCallback:^(NSArray *searchList) {
            dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (searchString.length > 0) {
            searchList = [[STKit sharedInstance] searchUserListBySearchStr:searchString];
        }
        [_searchResults removeAllObjects];
        if (searchList) {
            [_searchResults addObjectsFromArray:searchList];
...
- (void)addFriend {
    ...
    if (infoStrArr.count > 1) {
        ...
        if (self.fromQChat) {
            ...
            userXmppId = [NSString stringWithFormat:@"%@@%@",rtxID, [[STKit sharedInstance] getDomain]];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [STFastEntrance openUserCardVCByUserId:userXmppId];
        });
    }
}
