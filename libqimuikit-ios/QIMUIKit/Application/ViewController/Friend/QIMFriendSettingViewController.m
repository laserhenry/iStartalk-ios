...
- (void)updateMySetting{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSDictionary *verifyDic = [[STKit sharedInstance] getVerifyFreindModeWithXmppId:[[STKit sharedInstance] getLastJid]];
        ...
        int receMsgMode = [[STKit sharedInstance] getReceiveMsgLimitWithXmppId:[[STKit sharedInstance] getLastJid]];
        for (QIMFriendSettingItem * item in _dataSource) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (item.isVerifyMode && item.mode == VerifyMode_Question_Answer) {
        ...
        if (item.isVerifyMode) {
            BOOL isSuccess = [[STKit sharedInstance] setVerifyFreindMode:item.mode WithQuestion:nil WithAnswer:nil];
            if (isSuccess) {
            ...
            BOOL isSuccess = [[STKit sharedInstance] setReceiveMsgLimitWithMode:item.mode];
            if (isSuccess) {
                for (int i = 0; i < _dataSource.count; i++) {
                    QIMFriendSettingItem *item = [_dataSource objectAtIndex:i];
...
- (void)setQuestion:(NSString *)question Answer:(NSString *)answer{
    ...
    [[STKit sharedInstance] setVerifyFreindMode:VerifyMode_Question_Answer WithQuestion:question WithAnswer:answer];
    [_tableView reloadData];
}
