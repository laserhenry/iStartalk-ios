...
+ (void)updateMicroTourModel{
    NSString *oldVersion = [[STKit sharedInstance] userObjectForKey:@"MicroTourModelVersion"];
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"https://touch.dujia.qunar.com/tour/qd/gnt.json?v=%@&p=ios&u=%@&tv=%@",[[STKit sharedInstance] AppBuildVersion],[[STKit getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],oldVersion]];
        ...
        if (response.code == 200) {
            ...
            if (ret) {
                ...
                if ( version && template.length > 0) {
                    [[STKit sharedInstance] setUserObject:version forKey:@"MicroTourModelVersion"];
