...
- (void)searchBar:(SearchBar *)SectionSearchBar textDidChange:(NSString *)searchText
{
    ...
    if (searchText.length > 0 && [[self groupID] length] > 0) {
        [_searchResults addObjectsFromArray:[[STKit sharedInstance] searchGroupUserBySearchStr:searchText inGroup:[self groupID]]];
...
- (void)setupSearchBar {
    ...
    iconView.image = [STKit defaultGroupHeaderImage];
...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        self.dataSource = [NSMutableArray arrayWithArray:[[STKit sharedInstance] syncgroupMember:[self groupID]]];
...
- (void)reloadGroup:(NSNotification *)notify {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([groupIds containsObject:self.groupID]) {
            NSArray *members = [[STKit sharedInstance] qimDB_getGroupMember:[self groupID]];
            [[STKit sharedInstance] bulkInsertGroupMember:members WithGroupId:[self groupID]];
...
- (void)loadGroupData {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray *roomMembers = [NSMutableArray arrayWithArray:[[STKit sharedInstance] getGroupMembersByGroupId:self.groupID]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_searchBarKeyTmp.text.length > 0) {
        ...
        NSString *remarkName = [[STKit sharedInstance] getUserMarkupNameWithUserId:[dict objectForKey:@"XmppId"]];
