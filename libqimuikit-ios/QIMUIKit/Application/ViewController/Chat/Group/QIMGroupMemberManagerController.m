...
-(instancetype)initWithMembers:(NSArray *)members
{
    if (self = [self init]) {
        ...
        for (NSDictionary * _memberInfo in members) {
            if ([[_memberInfo objectForKey:@"xmppjid"] isEqualToString:[[STKit sharedInstance] getLastJid]]) {
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = [NSBundle qim_localizedStringForKey:@"Manage group members"];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:@"警告！" message:@"是否确定将该成员从群组中移除？" delegate:self cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"Cancel"] otherButtonTitles:[NSBundle qim_localizedStringForKey:@"Confirm"], nil];
        [alertView show];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        BOOL success = [[STKit sharedInstance] removeGroupMemberWithName:name WithJid:jid ForGroupId:[infoDic objectForKey:@"jid"]];
