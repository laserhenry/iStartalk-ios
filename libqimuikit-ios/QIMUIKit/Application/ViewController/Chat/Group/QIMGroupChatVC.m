    ...
    if (currentTime - prepCheckTime > 10 * 60) {
        ...
        dispatch_async(
                       ...
                       dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                           ...
                           // 频度控制？
                           [[STKit sharedInstance] updateQChatGroupMembersCardForGroupId:self.chatId];
...
- (void)setupNav {
    [self setBackBtn];
    NSDictionary *memoryGroupCardDic = [[STKit sharedInstance] getMemoryGroupCardByGroupId:self.chatId];
    if (memoryGroupCardDic.count > 0) {
        [self updateGroupChatTitleWithCardDic:memoryGroupCardDic];
    } else {
        dispatch_async([[STKit sharedInstance] getLoadGroupCardFromDBQueue], ^{
            ...
            if (!self.bindId) {
                ...
                if (groupCardVersion <= 0) {
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                        [[STKit sharedInstance] updateGroupCardByGroupId:self.chatId];
                    });
                }
            }
        });
...
- (void)updateGroupChatTitleWithCardDic:(NSDictionary *)groupCardDic {
    ...
    if (self.chatType == ChatType_CollectionChat) {
        NSDictionary *groupCardDic = [[STKit sharedInstance] getCollectionGroupCardByGroupId:self.chatId];
        self.groupCardDic = groupCardDic;
        if (groupCardDic) {
...
- (void)synchronizeChatSession {
    [[STKit sharedInstance] synchronizeChatSessionWithUserId:self.chatId WithChatType:self.chatType WithRealJid:nil];
}
...
- (void)forceReloadGroupMessages:(NSNotification *)notify {
    long long currentMaxGroupTime = [[STKit sharedInstance] getMaxMsgTimeStampByXmppId:self.chatId];
...
- (void)updateGroupNickName:(NSNotification *)notify {
    ...
    if ([groupIds isKindOfClass:[NSArray class]] && [groupIds containsObject:self.chatId]) {
        ...
        NSDictionary *cardDic = [[STKit sharedInstance] getGroupCardByGroupId:self.chatId];
...
- (void)moveToFirstNotReadMsg {
   ...
   self.readedMsgTimeStamp = [[STKit sharedInstance] getReadedTimeStampForUserId:self.chatId WithRealJid:self.chatId WithMsgDirection:QIMMessageDirection_Received withUnReadCount:self.notReadCount];
    __weak __typeof(self) weakSelf = self;
    [[STKit sharedInstance] getMsgListByUserId:self.chatId
                                        ...
                                        FromTimeStamp:_readedMsgTimeStamp
                                         WithComplete:^(NSArray *list) {
                                             ...
                                             dispatch_async(dispatch_get_main_queue(), ^{
                                                 ...
                                                 [[STKit sharedInstance] clearAtMeMessageWithJid:weakSelf.chatId];
...
- (void)showNotReadAtMsgView {
    //未读消息按钮
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        NSArray *atMeMessageArray = [[STKit sharedInstance] getHasAtMeByJid:self.chatId];
...
- (void)updateNotAtReadAtMsgWithMsgArray:(NSArray *)array {
    ...
    for (STMsgModel *msg in array) {
        [msgIds addObject:msg.messageId];
    }
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
       [[STKit sharedInstance] updateAtMeMessageWithJid:self.chatId withMsgIds:msgIds withReadState:QIMAtMsgHasReadState];
    });
}

- (void)updateNotReadAtMsgTipView {
    NSArray *atMsgArray = [[STKit sharedInstance] getHasAtMeByJid:self.chatId];
...
- (void)moveToLastNotReadAtMsg {
    ...
    if (atMsgArray.count > 0) {
        ...
        if (messageDate <= 0) {
            STMsgModel *msgModel = [[STKit sharedInstance] getMsgByMsgId:msgId];
            messageDate = msgModel.messageDate;
        }
        [[STKit sharedInstance] getMsgListByUserId:self.chatId WithRealJid:self.chatId FromTimeStamp:messageDate WithComplete:^(NSArray *list) {
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.messageManager.dataSource = [NSMutableArray arrayWithArray:list];
                ...
                [[STKit sharedInstance] updateAtMeMessageWithJid:self.chatId withMsgIds:@[msgId] withReadState:QIMAtMsgHasReadState];
...
- (void)reloadTableData {
    ...
    __weak __typeof(self) weakSelf = self;
    if (self.chatType == ChatType_CollectionChat) {
        NSArray *list = [[STKit sharedInstance] getCollectionMsgListForUserId:self.bindId originUserId:self.chatId];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if (weakSelf.fastMsgTimeStamp > 0) {
                [[STKit sharedInstance] getMsgListByUserId:weakSelf.chatId WithRealJid:weakSelf.chatId FromTimeStamp:weakSelf.fastMsgTimeStamp WithComplete:^(NSArray *list) {
                        weakSelf.messageManager.dataSource = [NSMutableArray arrayWithArray:list];
                ...
                [[STKit sharedInstance] getMsgListByUserId:weakSelf.chatId
...
- (void)markReadedForChatRoom {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        if (self.messageManager.dataSource.count > 0) {
            [[STKit sharedInstance] clearNotReadMsgByGroupId:self.chatId];
        }
    });
}
...
- (void)selfPopedViewController {
    ...
    [[STKit sharedInstance] setNotSendText:[self.textBar getSendAttributedText]
...
- (void)expandViewItemHandleNotificationHandle:(NSNotification *)notify {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([trId isEqualToString:QIMTextBarExpandViewItem_VideoCall]) {
            ...
            if ([[STKit sharedInstance] aaCollectionUrlHost]) {
                QIMWebView *webView = [[QIMWebView alloc] init];
                webView.url = [NSString stringWithFormat:@"%@?username=%@&sign=%@&company=qunar&group_id=%@&rk=%@&q_d=%@", [[STKit sharedInstance] aaCollectionUrlHost], [STKit getLastUserName], [[NSString stringWithFormat:@"%@00d8c4642c688fd6bfa9a41b523bdb6b", [STKit getLastUserName]] qim_getMD5], [self.chatId stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding], [[STKit sharedInstance] myRemotelogginKey],  [[STKit sharedInstance] getDomain]];
                webView.navBarHidden = YES;
                [webView setFromRegPackage:YES];
                [self.navigationController pushViewController:webView animated:YES];
            } else {
            ...
            if (linkUrl.length > 0) {
                if ([linkUrl rangeOfString:@"qunar.com"].location != NSNotFound) {
                    linkUrl = [linkUrl stringByAppendingFormat:@"%@username=%@&company=qunar&group_id=%@&rk=%@", [linkUrl rangeOfString:@"?"].location != NSNotFound ? @"&" : @"?", [STKit getLastUserName], [self.chatId stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding], [[STKit sharedInstance] remoteKey]];
                } else {
                    linkUrl = [linkUrl stringByAppendingFormat:@"%@from=%@&to=%@&chatType=%lld", [linkUrl rangeOfString:@"?"].location != NSNotFound ? @"&" : @"?", [[STKit sharedInstance] getLastJid],  [self.chatId stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding], self.chatType];
                }
            ...
            NSString *linkUrl = [trdExtendDic objectForKey:@"linkurl"];
            if (openQIMRN) {
                [STFastEntrance openQIMRNWithScheme:linkUrl withChatId:self.chatId withRealJid:nil withChatType:self.chatType];
            } else if (openRequeset) {
                [[STKit sharedInstance] sendTPPOSTRequestWithUrl:linkUrl withChatId:self.chatId withRealJid:nil withChatType:self.chatType];
            } else {
                if (linkUrl.length > 0) {
                    if ([linkUrl rangeOfString:@"qunar.com"].location != NSNotFound) {
                        linkUrl = [linkUrl stringByAppendingFormat:@"%@from=%@&to=%@&chatType=%lld", [linkUrl rangeOfString:@"?"].location != NSNotFound ? @"&" : @"?", [[STKit sharedInstance] getLastJid], [self.chatId stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding], self.chatType];
...
- (void)reSendMsg {
    ...
    if (message.messageType == QIMMessageType_LocalShare) {
        ...
        if ([self isImageMessage:message.message]) {
            ...
            message = [[STKit sharedInstance] createMessageWithMsg:message.message extenddInfo:message.extendInformation userId:self.chatId userType:ChatType_GroupChat msgType:message.messageType forMsgId:_resendMsg.messageId];
...
- (void)WillSendRedPackNotificationHandle:(NSNotification *)noti {
   ...
   STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:@"【红包】请升级最新版本客户端查看红包~"
                                                           extenddInfo:infoStr
                                                                userId:self.chatId
...
- (void)refreshTableViewCell:(UITableViewCell *)cell {
    if (cell && [cell isKindOfClass:[UITableViewCell class]]) {
        ...
        STMsgModel *message = [self.messageManager.dataSource objectAtIndex:indexPath.row];
...
- (void)processEvent:(int)event withMessage:(id)message {
    ...
    if (event == MA_Repeater) {
        ...
        [dicInfo setQIMSafeObject:[[STKit sharedInstance] getLastJid] forKey:@"fromId"];
        [dicInfo setQIMSafeObject:[(STMsgModel *) eventMsg messageId] forKey:@"messageId"];
        [dicInfo setQIMSafeObject:[(STMsgModel *) eventMsg message] forKey:@"message"];
        NSString *msgInfo = [[QIMJSONSerializer sharedInstance] serializeObject:dicInfo];
        
        [[STKit sharedInstance] revokeGroupMessageWithMessageId:[(STMsgModel *) eventMsg messageId]
                                                               message:msgInfo
                                                                 ToJid:self.chatId];
...
- (void)updateHistoryMessageList:(NSNotification *)notify {
    ...
    if ([self.chatId isEqualToString:notify.object]) {
        [[STKit sharedInstance] getMsgListByUserId:self.chatId WithRealJid:nil WithLimit:kPageCount WithOffset:0 withLoadMore:NO WithComplete:^(NSArray *list) {
    ...
    if ([originFrom isEqualToString:self.chatId] && [originTo isEqualToString:self.bindId]) {
       ...
       STMsgModel *msg = [[STKit sharedInstance] getCollectionMsgListForMsgId:msgId];
        if (msg) {
            ...
            if ([msg isKindOfClass:[STMsgModel class]] && msg.messageDirection == QIMMessageDirection_Received) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    [[STKit sharedInstance] sendReadstateWithGroupLastMessageTime:msg.messageDate
...
- (void)sendMessage:(NSString *)message WithInfo:(NSString *)info ForMsgType:(int)msgType {
    if (msgType == QIMMessageType_LocalShare) {
        ...
        STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:message extenddInfo:info userId:self.chatId userType:ChatType_GroupChat msgType:QIMMessageType_LocalShare forMsgId:_resendMsg.messageId];
        ...
        STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:message extenddInfo:info userId:self.chatId userType:ChatType_GroupChat msgType:msgType forMsgId:_resendMsg.messageId];
        [self.messageManager.dataSource addObject:msg];
        [self updateGroupUsersHeadImgForMsgs:@[msg]];
        [self.tableView beginUpdates];
        [self.tableView insertRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:self.messageManager.dataSource.count - 1 inSection:0]] withRowAnimation:UITableViewRowAnimationBottom];
...
- (void)sendCollectionFaceStr:(NSString *)faceStr {
    if ([faceStr isEqualToString:kImageFacePageViewAddFlagName]) {
        ...
        if (faceStr.length) {
            ...
            msg = [[STKit sharedInstance] createMessageWithMsg:msgText extenddInfo:nil userId:self.chatId userType:ChatType_GroupChat msgType:QIMMessageType_Text];
    ...
    if (faceStr && packageId) {
        ...
        if ([text length] > 0) {
           STMsgModel *msg = nil;
            ...
            msg = [[STKit sharedInstance] createMessageWithMsg:text
...
- (void)qim_textbarSendImageWithImagePath:(NSString *)imagePath {
    ...
    STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:msgText extenddInfo:nil userId:self.chatId userType:ChatType_GroupChat msgType:QIMMessageType_Text];
...
- (void)sendVideoPath:(NSString *)videoPath WithThumbImage:(UIImage *)thumbImage WithFileSizeStr:(NSString *)fileSizeStr WithVideoDuration:(float)duration forMsgId:(NSString *)mId {
    ...
    BOOL VideoConfigUseAble = [[[STKit sharedInstance] userObjectForKey:@"VideoConfigUseAble"] boolValue];
...
- (void)jumpToConverstaionSearch {
    ...
    [[STFastEntrance sharedInstance] openLocalSearchWithXmppId:self.chatId withRealJid:nil withChatType:self.chatType];
}
...
- (void)addPersonToPgrup:(id)sender {
    ...
    [STFastEntrance openQIMGroupCardVCByGroupId:self.chatId];
}
