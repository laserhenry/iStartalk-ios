...
- (float)currentFontSize {
    ...
    if (_qtCurrentFontSize <= 8) {
        NSDictionary * infoDic = [[STKit sharedInstance] userObjectForKey:kCurrentFontInfo];
...
- (void)setCurrentFontSize:(float)fontSize{
    ...
    NSDictionary *dic = [[STKit sharedInstance] userObjectForKey:kCurrentFontInfo];
    if (dic) {
    ...
    [[STKit sharedInstance] setUserObject:infoDic forKey:kCurrentFontInfo];
}
