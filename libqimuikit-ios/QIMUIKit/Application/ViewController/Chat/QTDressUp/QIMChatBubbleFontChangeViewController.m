...
- (void)viewDidLoad {
    ...
    NSDictionary *dic = [[STKit sharedInstance] userObjectForKey:kChatColorInfo];
    if (dic) {
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    if (_isSaved == NO) {
        [[STKit sharedInstance] setUserObject:_lastChatColorInfo forKey:kChatColorInfo];
    }
    if (_isValueChange) {
        NSString *infoDicStr = [[QIMJSONSerializer sharedInstance] serializeObject:_chatColorInfo];
        [[STKit sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKChatColorInfo WithSubKey:[[STKit sharedInstance] getLastJid] WithConfigValue:infoDicStr WithDel:NO withCallback:nil];
...
//    KZColorPicker * cp = [self.view viewWithTag:kColorSelectViewTag];
    ...
    [[STKit sharedInstance] setUserObject:_chatColorInfo forKey:kChatColorInfo];
...
- (void)refresh{
    [self initSettingView];
    for (STMsgModel * msg in _placeholderMsgs) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
   STMsgModel * message = [_placeholderMsgs objectAtIndex:indexPath.row];
