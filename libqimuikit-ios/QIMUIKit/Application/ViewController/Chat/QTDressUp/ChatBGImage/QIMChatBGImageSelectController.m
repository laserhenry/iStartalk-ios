...
//    [self saveChatBGImage:image];
    UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:[NSBundle qim_localizedStringForKey:@"Reminder"] message:@"设置成功，快去查看吧~" delegate:nil cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"Confirm"] otherButtonTitles:nil, nil];
    [alertView show];
}
...
- (void)saveChatBGImage : (UIImage *)image
{
    ...
    NSDictionary *dic = [[STKit sharedInstance] userObjectForKey:@"chatBGImageDic"];
    if (dic) {
    ...
    if (!chatBGImageDic) {
        ...
        if (_useForAllUser) {
            for (NSString * key in chatBGImageDic.allKeys) {
                [[STDataContraller getInstance] deleteResourceWithFileName:key];
            }
    ...
    [[STKit sharedInstance] setUserObject:chatBGImageDic forKey:@"chatBGImageDic"];
    if (image) {
        [[STDataContraller getInstance] saveResourceWithFileName:[NSString stringWithFormat:@"chatBGImageFor_%@",_useForAllUser? @"Common" : self.userID] data:UIImageJPEGRepresentation(image, 1.0)];
    } else { 
        [[STDataContraller getInstance] deleteResourceWithFileName:[NSString stringWithFormat:@"chatBGImageFor_%@",_useForAllUser? @"Common" : self.userID]];
    }
