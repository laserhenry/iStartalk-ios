...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    if (_isValueChanged) {
        NSDictionary *fontInfo = [[STKit sharedInstance] userObjectForKey:kCurrentFontInfo];
        NSString * infoDicStr = [[QIMJSONSerializer sharedInstance] serializeObject:fontInfo];
        [[STKit sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKCurrentFontInfo WithSubKey:[[STKit sharedInstance] getLastJid] WithConfigValue:infoDicStr WithDel:NO withCallback:nil];
...
//        msg.nickName = [QIMKit getLastUserName];
        ...
        STMsgModel * msg1 = [STMsgModel new];
...
//        msg1.nickName = [NSBundle qim_localizedStringForKey:@"qtalk_team"];
        ...
        STMsgModel * msg2 = [STMsgModel new];
...
//        msg2.nickName = [NSBundle qim_localizedStringForKey:@"qtalk_team"];
        ...
        STMsgModel * msg3 = [STMsgModel new];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == _chatTableView) {
       STMsgModel * message = [_chatDatasource objectAtIndex:indexPath.row];
...
-(void)sliderView:(QIMSliderView *)slider didChangeSelectedValue:(NSInteger)index{
    for (STMsgModel * msg in _chatDatasource) {
