...
- (QIMTextBar *)textBar {
    ...
    if (!_textBar) {
        ...
        } else if ([[STKit sharedInstance] isMiddleVirtualAccountWithJid:self.chatId]) {
            textBarType = QIMTextBarExpandViewTypeRobot;
        } else if ([self.chatId isEqualToString:[[STKit sharedInstance] getLastJid]]) {
            textBarType = QIMTextBarExpandViewTypeRobot;
        } else {
        ...
        NSDictionary *notSendDic = [[STKit sharedInstance] getNotSendTextByJid:self.chatId];
...
- (void)forwardBtnHandle:(id)sender {
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0 && ![[STKit sharedInstance] getIsIpad]) {
        ...
        UIAlertAction *quickForwardAction = [UIAlertAction actionWithTitle:[NSBundle qim_localizedStringForKey:@"One-by-One Forward"] style:UIAlertActionStyleDefault handler:^(UIAlertAction *_Nonnull action) {
            NSArray *forwardIndexpaths = [self.messageManager.forwardSelectedMsgs.allObjects sortedArrayUsingComparator:^NSComparisonResult(id _Nonnull obj1, id _Nonnull obj2) {
                return [(STMsgModel *)obj1 messageDate] > [(STMsgModel *)obj2 messageDate];
            }];
            
            NSMutableArray *msgList = [NSMutableArray arrayWithCapacity:1];
            for (STMsgModel *message in forwardIndexpaths) {
        ...
        UIAlertAction *mergerForwardAction = [UIAlertAction actionWithTitle:[NSBundle qim_localizedStringForKey:@"Combine and Forward"] style:UIAlertActionStyleDefault handler:^(UIAlertAction *_Nonnull action) {
            ...
            NSDictionary *userInfoDic = [[STKit sharedInstance] getUserInfoByUserId:[[STKit sharedInstance] getLastJid]];
            NSString *userName = [userInfoDic objectForKey:@"Name"];
            
            _jsonFilePath = [QIMExportMsgManager parseForJsonStrFromMsgList:msgList withTitle:[NSString stringWithFormat:@"%@和%@的聊天记录", userName ? userName : [STKit getLastUserName], self.title]];
...
- (void)setupNavBar {
    ...
    if (self.chatType != ChatType_System) {
        ...
        if ([STKit getQIMProjectType] != QIMProjectTypeQChat) {
            ...
            NSString *qCloudHost = [[STKit sharedInstance] qimNav_QCloudHost];
            if (qCloudHost.length > 0 && ![[STKit sharedInstance] isMiddleVirtualAccountWithJid:self.chatId] && self.chatType == ChatType_SingleChat) {
            ...
            if (self.chatType == ChatType_ConsultServer && [[[STKit sharedInstance] getMyhotLinelist] containsObject:self.virtualJid]) {
...
- (void)initUI {
    ...
    _shareLctId = [[STKit sharedInstance] getShareLocationIdByJid:self.chatId];
    if (_shareLctId.length > 0 && [[STKit sharedInstance] getShareLocationUsersByShareLocationId:_shareLctId].count > 0) {
        _shareFromId = [[STKit sharedInstance] getShareLocationFromIdByShareLocationId:_shareLctId];
...
- (void)initTitleView {
    ...
    if (self.chatType == ChatType_ConsultServer) {
        NSDictionary *infoDic = [[STKit sharedInstance] getUserInfoByUserId:self.chatId];
        ...
        NSDictionary *consultUserInfo = [[STKit sharedInstance] getUserInfoByUserId:self.virtualJid];
        ...
        NSDictionary *collectionUserInfo = [[STKit sharedInstance] getCollectionUserInfoByUserId:self.chatId];
    ...
    if (self.title.length <= 0 || !self.title) {
        ...
        NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:xmppId];
        if (userInfo.count) {
    ...
    if (self.chatType != ChatType_Consult) {
        ...
        if (self.chatType == ChatType_ConsultServer) {
            ...
            if (![[STKit sharedInstance] moodshow]) {
                NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:self.chatId];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (_shareLctId && [[STKit sharedInstance] getShareLocationUsersByShareLocationId:_shareLctId].count == 0) {
    ...
    for (int i = 0; i < (int) self.messageManager.dataSource.count - kPageCount * 2; i++) {
        [[QIMMessageCellCache sharedInstance] removeObjectForKey:[(STMsgModel *) self.messageManager.dataSource[i] messageId]];
    }
}
...
#warning 加密会话
    ...
    NSInteger securitySettingTime = [[[STKit sharedInstance] userObjectForKey:@"securityMinute"] integerValue];
    if (securitySettingTime == 0) {
        //15分钟安全时间
        [[STKit sharedInstance] setUserObject:@(15 * 60) forKey:@"securityMinute"];
    }
    ...
    [[STKit sharedInstance] setCurrentSessionUserId:self.chatId];
...
-(void)requestRobotConsult{
    ...
    NSString * urlStr = [NSString stringWithFormat:@"%@/robot/qtalk_robot/sendtips?rexian_id=%@&m_from=%@&m_to=%@",[[STKit sharedInstance] qimNav_NewHttpUrl],rexian_id,self.chatId,[[STKit sharedInstance] getLastJid]];
    
    [[STKit sharedInstance] sendTPGetRequestWithUrl:urlStr withSuccessCallBack:^(NSData *responseData) {
...
- (void)synchronizeDujiaWarning {
    [[STKit sharedInstance] synchronizeDujiaWarningWithJid:self.chatId];
}
...
- (void)synchronizeChatSession {
    ...
    [[STKit sharedInstance] synchronizeChatSessionWithUserId:userId WithChatType:self.chatType WithRealJid:realJid];
}

- (void)forceReloadSingleMessages:(NSNotification *)notify {
    long long currentMaxSingleMsgTime = [[STKit sharedInstance] getMaxMsgTimeStampByXmppId:self.chatId];
    STMsgModel *msg = [self.messageManager.dataSource lastObject];
...
- (void)gotoSendRedPackRnView:(NSNotificationCenter *)notify {
    [STFastEntrance openSendRedPacket:self.chatId isRoom:true];
}
        ...
        [[AlipaySDK defaultService] auth_V2WithInfo:authInfoStr
                                         fromScheme:appScheme
                                           callback:^(NSDictionary *resultDic) {
                                               ...
                                               if(openid && aliuid){
                                                   [[STKit sharedInstance] bindAlipayAccount:openid withAliUid:aliuid userId:[STKit getLastUserName]];
                                               }
                                           }];
    }else{
        //获取绑定关系失败
...
- (void)checkAddNewMsgTag {
    STMsgModel *firstMsg = [self.messageManager.dataSource firstObject];
    if (firstMsg.messageDate > _readedMsgTimeStamp) {
        return;
    }
    int index = 0;
    BOOL needAdd = NO;
    for (STMsgModel *msg in self.messageManager.dataSource) {
...
- (void)moveToFirstNotReadMsg {
    ...
    [[STKit sharedInstance] getMsgListByUserId:self.chatId WithRealJid:realJid FromTimeStamp:_readedMsgTimeStamp WithComplete:^(NSArray *list) {
        if (list.count) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)loadSystemData {
    ...
    if ([STKit getQIMProjectType] != QIMProjectTypeQChat) {
        ...
        [[STKit sharedInstance] getSystemMsgLisByUserId:self.chatId WithFromHost:domain WithLimit:kPageCount WithOffset:(int)self.messageManager.dataSource.count withLoadMore:NO WithComplete:^(NSArray *list) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                dispatch_after(time, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    //标记已读
                    [[STKit sharedInstance] clearSystemMsgNotReadWithJid:weakSelf.chatId];
                });
            });
        }];
    } else {
        [[STKit sharedInstance] getMsgListByUserId:self.chatId WithRealJid:self.chatId WithLimit:kPageCount WithOffset:0 withLoadMore:NO WithComplete:^(NSArray *list) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf.messageManager.dataSource addObjectsFromArray:list];
                [weakSelf.tableView reloadData];
                [weakSelf scrollBottom];
...
- (void)loadData {
    ...
    NSString *realJid = nil;
    if (self.chatType == ChatType_CollectionChat) {
        NSArray *collectionMsgs = [[STKit sharedInstance] getCollectionMsgListForUserId:self.bindId originUserId:self.chatId];
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.messageManager.dataSource removeAllObjects];
        ...
        if (self.fastMsgTimeStamp > 0) {
            [[STKit sharedInstance] getMsgListByUserId:userId WithRealJid:realJid FromTimeStamp:self.fastMsgTimeStamp WithComplete:^(NSArray *list) {
                dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.chatType == ChatType_ConsultServer) {
                [[STKit sharedInstance] getConsultServerMsgLisByUserId:realJid WithVirtualId:userId WithLimit:kPageCount WithOffset:0 withLoadMore:NO WithComplete:^(NSArray *list) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self.messageManager.dataSource removeAllObjects];
                        [self.messageManager.dataSource addObjectsFromArray:list];
                        [_tableView reloadData];
                ...
                [[STKit sharedInstance] getMsgListByUserId:userId WithRealJid:realJid WithLimit:kPageCount WithOffset:0 withLoadMore:NO WithComplete:^(NSArray *list) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self.messageManager.dataSource removeAllObjects];
                        [self.messageManager.dataSource addObjectsFromArray:list];
                        [_tableView reloadData];
...
- (void)loadNetWorkData {
    //搜索服务器消息
    __weak __typeof(self) weakSelf = self;
    [[STKit sharedInstance] getRemoteSearchMsgListByUserId:self.chatId WithRealJid:self.chatId withVersion:self.fastMsgTimeStamp withDirection:QIMGetMsgDirectionUp WithLimit:20 WithOffset:0 WithComplete:^(NSArray *list) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.messageManager.dataSource removeAllObjects];
            [self.messageManager.dataSource addObjectsFromArray:list];
            [weakSelf.tableView reloadData];
...
- (void)markReadFlag {
    ...
    count ++;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        NSArray *markReadMsgList = [[STKit sharedInstance] getNotReadMsgIdListByUserId:userId WithRealJid:realJid];
        QIMVerboseLog(@"markReadMsgList : %d", markReadMsgList);
        if (markReadMsgList.count > 0) {
            [[STKit sharedInstance] sendReadStateWithMessagesIdArray:markReadMsgList WithMessageReadFlag:QIMMessageReadFlagDidRead WithXmppId:self.chatId WithRealJid:realJid];
        }
    });
}
...
//右上角名片信息
- (void)onCardClick {
    if (![[[STKit sharedInstance] userObjectForKey:@"kRightCardRemindNotification"] boolValue]) {
        [[NSNotificationCenter defaultCenter] postNotificationName:kRightCardRemindNotification object:nil];
        [[STKit sharedInstance] setUserObject:@(YES) forKey:kRightCardRemindNotification];
    }
    NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:self.chatId];
    ...
    if (userId.length > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [STFastEntrance openUserChatInfoByUserId:self.chatId];
        });
    }
}
...
- (void)onCreateGroupClcik {
    ...
    groupCreateVC.userInfo = [[STKit sharedInstance] getUserInfoByUserId:self.chatId];
...
- (void)endChatSession {
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:[NSBundle qim_localizedStringForKey:@"Confirm"] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        [[STKit sharedInstance] closeSessionWithShopId:self.virtualJid WithVisitorId:self.chatId withBlock:^(NSString *closeMsg) {
...
- (void)selfPopedViewController {
    ...
    [[STKit sharedInstance] setNotSendText:[self.textBar getSendAttributedText] inputItems:[self.textBar getAttributedTextItems] ForJid:self.chatId];
...
- (void)playVoiceWithMsgId:(NSString *)msgId WithFilePath:(NSString *)filePath {
    ...
    self.isNoReadVoice = [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] playVoiceIsNoReadWithMsgId:msgId ChatId:self.chatId];
    if (msgId) {
        self.currentPlayVoiceIndex = [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] getIndexOfMsgIdWithChatId:self.chatId msgId:msgId];
        // 开始播放
...
- (void)remoteAudioPlayerReady:(QIMRemoteAudioPlayer *)player {
    ...
    [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] setVoiceNoReadStateWithMsgId:msgId ChatId:self.chatId withState:YES];
}
...
- (void)remoteAudioPlayerDidFinishPlaying:(QIMRemoteAudioPlayer *)player {
    ...
    NSInteger count = [[STVoiceNoReadStateMgr sharedVoiceNoReadStateManager] getVisibleNoReadSoundsCountWithChatId:self.chatId];
...
- (void)shareLocationCancelBtnHandle:(id)sender {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:_shareFromId];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([trId isEqualToString:QIMTextBarExpandViewItem_Shock]) {
            ...
            if (timeInterval > 10 || isnanTimeInterval) {
                ...
                STMsgModel *msg = [[STKit sharedInstance] sendShockToUserId:self.chatId];
            ...
            [STFastEntrance openTransferConversation:self.virtualJid withVistorId:self.chatId];
            ...
            if ([STKit getQIMProjectType] == QIMProjectTypeStartalk || (![[[STKit sharedInstance] getDomain] isEqualToString:@"ejabhost1"] && ![[[STKit sharedInstance] getDomain] isEqualToString:@"ejabhost2"])) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    [[STKit sharedInstance] getBindPayAccount:[STKit getLastUserName] withCallBack:^(BOOL successed){
                        if(successed){
                            [STFastEntrance openSendRedPacket:self.chatId isRoom:false];
                        }else{
            ...
            QIMWebView *webView = [[QIMWebView alloc] init];
            webView.url = [NSString stringWithFormat:@"%@?username=%@&sign=%@&company=qunar&user_id=%@&rk=%@&q_d=%@", [[STKit sharedInstance] aaCollectionUrlHost], [STKit getLastUserName], [[NSString stringWithFormat:@"%@00d8c4642c688fd6bfa9a41b523bdb6b", [STKit getLastUserName]] qim_getMD5], [self.chatId stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding], [[STKit sharedInstance] myRemotelogginKey],  [[STKit sharedInstance] getDomain]];
            webView.navBarHidden = YES;
            [webView setFromRegPackage:YES];
            [self.navigationController pushViewController:webView animated:YES];
            ...
            NSDictionary *trdExtendDic = [[STKit sharedInstance] getExpandItemsForTrdextendId:trId];
            ...
            if (openQIMRN) {
                [STFastEntrance openQIMRNWithScheme:linkUrl withChatId:userId withRealJid:realJid withChatType:self.chatType];
            } else if (openRequeset) {
                [[STKit sharedInstance] sendTPPOSTRequestWithUrl:linkUrl withChatId:userId withRealJid:realJid withChatType:self.chatType];
            } else {
                if (linkUrl.length > 0) {
                    if ([linkUrl rangeOfString:@"qunar.com"].location != NSNotFound) {
                        linkUrl = [linkUrl stringByAppendingFormat:@"%@from=%@&to=%@&realJid=%@&chatType=%lld", [linkUrl rangeOfString:@"?"].location != NSNotFound ? @"&" : @"?", [[STKit sharedInstance] getLastJid], userId, realJid, self.chatType];
                    } else {
                        linkUrl = [linkUrl stringByAppendingFormat:@"%@from=%@&to=%@&realJid=%@&chatType=%lld", ([linkUrl rangeOfString:@"?"].location != NSNotFound ? @"&" : @"?"), [[STKit sharedInstance] getLastJid], userId, realJid, self.chatType];
                    }
                    [STFastEntrance openWebViewForUrl:linkUrl showNavBar:YES];
                }
            }
        }
    });
...
- (void)removeFailedMsg {
    ...
    for (STMsgModel *msg in self.messageManager.dataSource) {
        if ([msg isEqual:message]) {
            ...
            [[STKit sharedInstance] deleteMsg:message ByJid:self.chatId];
            break;
        }
    }
}
...
- (void)reSendMsg {
    ...
    if (message.messageType == QIMMessageType_LocalShare) {
        if (self.chatType == ChatType_ConsultServer || self.chatType == ChatType_Consult) {
            [[STKit sharedInstance] sendConsultMessageId:message.messageId WithMessage:message.message WithInfo:message.extendInformation toJid:self.virtualJid realToJid:self.chatId WithChatType:self.chatType WithMsgType:message.messageType];
        } else {
        ...
        if ([self isImageMessage:message.message]) {
            ...
            message = [[STKit sharedInstance] createMessageWithMsg:message.message extenddInfo:message.extendInformation userId:self.chatId userType:self.chatType msgType:message.messageType forMsgId:_resendMsg.messageId];
            ...
            [_tableView endUpdates];
            if (self.chatType == ChatType_ConsultServer || self.chatType == ChatType_Consult) {
                message = [[STKit sharedInstance] sendConsultMessageId:message.messageId WithMessage:message.message WithInfo:message.extendInformation toJid:self.virtualJid realToJid:self.chatId WithChatType:self.chatType WithMsgType:message.messageType];
            } else {
                message = [[STKit sharedInstance] sendMessage:message ToUserId:self.chatId];
            }
            [self scrollToBottomWithCheck:YES];
        }
...
- (void)revokeMsgNotificationHandle:(NSNotification *)notify {
    ...
    for (STMsgModel *msg in self.messageManager.dataSource) {
        if ([msg.messageId isEqualToString:msgID]) {
            ...
            [[STKit sharedInstance] updateMsg:msg ByJid:self.chatId];
            NSIndexPath *thisIndexPath = [NSIndexPath indexPathForRow:[self.messageManager.dataSource indexOfObject:msg] inSection:0];
            BOOL isVisable = [[_tableView indexPathsForVisibleRows] containsObject:thisIndexPath];
            if (isVisable) {
                [_tableView reloadRowsAtIndexPaths:@[thisIndexPath] withRowAnimation:UITableViewRowAnimationNone];
...
- (void)WillSendRedPackNotificationHandle:(NSNotification *)noti {
    ...
    STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:@"【红包】请升级最新版本客户端查看红包~" extenddInfo:infoStr userId:self.chatId userType:self.chatType msgType:QIMMessageType_RedPack];
    ...
    if (self.chatType == ChatType_ConsultServer || self.chatType == ChatType_Consult) {
        msg = [[STKit sharedInstance] sendConsultMessageId:msg.messageId WithMessage:msg.message WithInfo:msg.extendInformation toJid:self.virtualJid realToJid:self.chatId WithChatType:self.chatType WithMsgType:msg.messageType];
    } else {
        msg = [[STKit sharedInstance] sendMessage:msg ToUserId:self.chatId];
    }
}
...
- (void)updateCollectionMessageList:(NSNotification *)notify {
    ...
    if ([originFrom isEqualToString:self.chatId] && [originTo isEqualToString:self.bindId]) {
        ...
        STMsgModel *msg = [[STKit sharedInstance] getCollectionMsgListForMsgId:msgId];
        if (msg) {
...
- (void)scrollToBottomWithCheck:(BOOL)flag {
    ...
    STMsgModel *message = self.messageManager.dataSource.lastObject;
...
- (void)contactSelectionViewController:(QIMContactSelectionViewController *)contactVC chatVC:(QIMChatVC *)vc {
    ...
    STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:@"您收到了一个消息记录文件文件，请升级客户端查看。" extenddInfo:nil userId:[contactVC getSelectInfoDic][@"userId"] userType:[[contactVC getSelectInfoDic][@"isGroup"] boolValue] ? ChatType_GroupChat : ChatType_SingleChat msgType:QIMMessageType_CommonTrdInfo];
    
    NSMutableDictionary *infoDic = [NSMutableDictionary dictionaryWithCapacity:1];
    [infoDic setQIMSafeObject:[NSString stringWithFormat:@"%@和%@的聊天记录", userName ? userName : [STKit getLastUserName], self.title] forKey:@"title"];
    ...
    [[STKit sharedInstance] qim_uploadFileWithFilePath:_jsonFilePath forMessage:msg];
}
...
#endif
    [[STKit sharedInstance] saveMsg:msg ByJid:self.chatId];
    [self.messageManager.dataSource addObject:msg];
    [_tableView beginUpdates];
    [_tableView insertRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:self.messageManager.dataSource.count - 1 inSection:0]] withRowAnimation:UITableViewRowAnimationBottom];
    [_tableView endUpdates];
    [self scrollToBottomWithCheck:YES];
    [[STKit sharedInstance] qim_uploadVideoPath:videoPath forMessage:msg];
}
...
- (void)sendMessage:(NSString *)message WithInfo:(NSString *)info ForMsgType:(int)msgType {
    if (msgType == QIMMessageType_LocalShare) {
        ...
        if (self.chatType == ChatType_Consult || self.chatType == ChatType_ConsultServer) {
            msg = [[STKit sharedInstance] createMessageWithMsg:message extenddInfo:info userId:self.virtualJid realJid:self.chatId userType:self.chatType msgType:QIMMessageType_LocalShare forMsgId:[QIMUUIDTools UUID] willSave:YES];
        } else {
            msg = [[STKit sharedInstance] createMessageWithMsg:message extenddInfo:info userId:self.chatId userType:self.chatType msgType:QIMMessageType_LocalShare forMsgId:_resendMsg.messageId];
        }
        ...
        [self scrollToBottomWithCheck:YES];
        [self addImageToImageList];
        [[STKit sharedInstance] qim_uploadImageWithImageKey:localPath forMessage:msg];
    } else {
        STMsgModel *msg = nil;
        if (self.chatType == ChatType_Consult || self.chatType == ChatType_ConsultServer) {
            msg = [[STKit sharedInstance] createMessageWithMsg:message extenddInfo:info userId:self.virtualJid realJid:self.chatId userType:self.chatType msgType:msgType forMsgId:_resendMsg.messageId willSave:YES];
        }
#if __has_include("QIMNoteManager.h")
        else if (self.encryptChatState == QIMEncryptChatStateEncrypting) {
            msg = [[STKit sharedInstance] sendMessage:message WithInfo:info ToUserId:self.chatId WithMsgType:QIMMessageType_Encrypt];
        }
#endif
        else {
            msg = [[STKit sharedInstance] createMessageWithMsg:message extenddInfo:info userId:self.chatId userType:self.chatType msgType:msgType forMsgId:_resendMsg.messageId];
        }
        [self.messageManager.dataSource addObject:msg];
        [_tableView beginUpdates];
        [_tableView insertRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:self.messageManager.dataSource.count - 1 inSection:0]] withRowAnimation:UITableViewRowAnimationBottom];
        ...
        if (self.chatId) {
            if (self.chatType == ChatType_Consult || self.chatType == ChatType_ConsultServer) {
                [[STKit sharedInstance] sendConsultMessageId:msg.messageId WithMessage:msg.message WithInfo:msg.extendInformation toJid:self.virtualJid realToJid:self.chatId WithChatType:self.chatType WithMsgType:msg.messageType];
            }
#if __has_include("QIMNoteManager.h")
            else if (self.encryptChatState == QIMEncryptChatStateEncrypting) {
...
- (void)sendTyping {
    [[STKit sharedInstance] sendTypingToUserId:self.chatId];
}
...
- (void)sendCollectionFaceStr:(NSString *)faceStr {
    if ([faceStr isEqualToString:kImageFacePageViewAddFlagName]) {
        ...
        if (![faceStr qim_hasPrefixHttpHeader]) {
            faceStr = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], faceStr];
        }
        if (faceStr.length > 0) {
            STMsgModel *msg = nil;
            ...
            if (self.chatType == ChatType_ConsultServer || self.chatType == ChatType_Consult) {
                msg = [[STKit sharedInstance] createMessageWithMsg:msgText extenddInfo:nil userId:self.chatId userType:self.chatType msgType:QIMMessageType_Text];
                [[STKit sharedInstance] sendConsultMessageId:msg.messageId WithMessage:msg.message WithInfo:msg.extendInformation toJid:self.virtualJid realToJid:self.chatId WithChatType:self.chatType WithMsgType:msg.messageType];
            }
#if __has_include("QIMNoteManager.h")
            else if(self.encryptChatState == QIMEncryptChatStateEncrypting) {
                ...
                msg = [[STKit sharedInstance] sendMessage:@"[加密收藏表情消息iOS]" WithInfo:content ToUserId:self.chatId WithMsgType:QIMMessageType_Encrypt];
            }
#endif
            else {
                msg = [[STKit sharedInstance] createMessageWithMsg:msgText extenddInfo:nil userId:self.chatId userType:self.chatType msgType:QIMMessageType_Text];
                [[STKit sharedInstance] sendMessage:msg ToUserId:self.chatId];
            }
...
- (void)sendNormalEmotion:(NSString *)faceStr WithPackageId:(NSString *)packageId {
    if (faceStr && packageId) {
        ...
        if ([text length] > 0) {
            STMsgModel *msg = nil;
            if ([STKit getQIMProjectType] == QIMProjectTypeQChat) {
                NSDictionary *dict = [[STKit sharedInstance] conversationParamWithJid:self.chatId];
            ...
            if (self.textBar.isRefer) {
                ...
                NSDictionary *referMsgUserInfo = [[STKit sharedInstance] getUserInfoByUserId:self.textBar.referMsg.from];
            ...
            if (self.chatType == ChatType_Consult || self.chatType == ChatType_ConsultServer) {
                msg = [[STKit sharedInstance] createMessageWithMsg:text extenddInfo:normalEmotionExtendInfoStr userId:self.virtualJid realJid:self.chatId userType:self.chatType msgType:QIMMessageType_ImageNew forMsgId:[QIMUUIDTools UUID] willSave:YES];
            }
#if __has_include("QIMNoteManager.h")
            else if(self.encryptChatState == QIMEncryptChatStateEncrypting) {
                ...
                msg = [[STKit sharedInstance] sendMessage:@"[加密表情消息iOS]" WithInfo:content ToUserId:self.chatId WithMsgType:QIMMessageType_Encrypt];
            }
#endif
            else {
                msg = [[STKit sharedInstance] createMessageWithMsg:text extenddInfo:normalEmotionExtendInfoStr userId:self.chatId userType:self.chatType msgType:QIMMessageType_ImageNew];
            }
...
- (void)sendText:(NSString *)text {
    ...
    if ([text length] > 0) {
        ...
        if (self.chatType == ChatType_Consult || self.chatType == ChatType_ConsultServer) {
            msg = [[STKit sharedInstance] createMessageWithMsg:text extenddInfo:nil userId:self.virtualJid realJid:self.chatId userType:self.chatType msgType:QIMMessageType_Text forMsgId:[QIMUUIDTools UUID] willSave:YES];
        }
#if __has_include("QIMNoteManager.h")
        else if(self.encryptChatState == QIMEncryptChatStateEncrypting) {
            ...
            msg = [[STKit sharedInstance] sendMessage:@"[加密文本消息iOS]" WithInfo:content ToUserId:self.chatId WithMsgType:QIMMessageType_Encrypt];
            [[STKit sharedInstance] saveMsg:msg ByJid:self.chatId];
        }
#endif
        else {
            msg = [[STKit sharedInstance] createMessageWithMsg:text extenddInfo:nil userId:self.chatId userType:self.chatType msgType:QIMMessageType_Text];
        }
...
- (void)sendFileData:(NSData *)fileData fileName:(NSString *)fileName {
    STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:@"您收到了一个消息记录文件文件，请升级客户端查看。" extenddInfo:nil userId:self.chatId userType:ChatType_SingleChat msgType:QIMMessageType_CommonTrdInfo];
    ...
    [_tableView endUpdates];
    [self scrollToBottomWithCheck:YES];
    [[STKit sharedInstance] qim_uploadFileWithFileData:fileData WithPathExtension:nil forMessage:msg];
}
...
- (void)sendimageText:(NSString *)text {
    ...
    if ([text length] > 0) {
        STMsgModel *msg = nil;
        if (self.chatType == ChatType_Consult || self.chatType == ChatType_ConsultServer) {
            msg = [[STKit sharedInstance] createMessageWithMsg:text extenddInfo:nil userId:self.virtualJid realJid:self.chatId userType:self.chatId msgType:QIMMessageType_Text forMsgId:[QIMUUIDTools UUID] willSave:YES];
        } else {
            msg = [[STKit sharedInstance] createMessageWithMsg:text extenddInfo:nil userId:self.chatId userType:self.chatType msgType:QIMMessageType_Text];
        }
        [self.messageManager.dataSource addObject:msg];
...
- (void)refreshCellForMsg:(STMsgModel *)msg {
    for (STMsgModel *message in self.messageManager.dataSource) {
...
- (void)processEvent:(int)event withMessage:(id)message {
    STMsgModel *eventMsg = (STMsgModel *)message;
    ...
    if (event == MA_Repeater) {
        ...
        for (STMsgModel *msg in self.messageManager.dataSource) {
            if ([msg.messageId isEqualToString:[(STMsgModel *) eventMsg messageId]]) {
                NSMutableArray *deleteIndexs = [NSMutableArray array];
                NSInteger index = [self.messageManager.dataSource indexOfObject:msg];
                [deleteIndexs addObject:[NSIndexPath indexPathForRow:index inSection:0]];
                STMsgModel *timeMsg = nil;
                if (index > 0) {
                    STMsgModel *tempMsg = [self.messageManager.dataSource objectAtIndex:index - 1];
                    if (tempMsg.messageType == QIMMessageType_Time) {
                        timeMsg = tempMsg;
                        if (index + 1 < self.messageManager.dataSource.count) {
                            STMsgModel *nMsg = [self.messageManager.dataSource objectAtIndex:index + 1];
                ...
                if (timeMsg) {
                    ...
                    [[STKit sharedInstance] deleteMsg:timeMsg ByJid:self.chatId];
                }
                
                [self.messageManager.dataSource removeObject:msg];
                [_tableView deleteRowsAtIndexPaths:deleteIndexs withRowAnimation:UITableViewRowAnimationAutomatic];
                
                [[STKit sharedInstance] deleteMsg:msg ByJid:self.chatId];
                break;
            }
        }
        ...
        for (STMsgModel *msg in self.messageManager.dataSource) {
            if ([msg.messageId isEqualToString:[(STMsgModel *) eventMsg messageId]]) {
                NSInteger index = [self.messageManager.dataSource indexOfObject:msg];
                [(STMsgModel *) eventMsg setMessageType:QIMMessageType_Revoke];
                [self.messageManager.dataSource replaceObjectAtIndex:index withObject:eventMsg];
                [[STKit sharedInstance] updateMsg:eventMsg ByJid:self.chatId];
                NSIndexPath *thisIndexPath = [NSIndexPath indexPathForRow:[self.messageManager.dataSource indexOfObject:msg] inSection:0];
                BOOL isVisable = [[_tableView indexPathsForVisibleRows] containsObject:thisIndexPath];
                if (isVisable) {
                    [_tableView reloadRowsAtIndexPaths:@[thisIndexPath] withRowAnimation:UITableViewRowAnimationNone];
                ...
                NSMutableDictionary *dicInfo = [NSMutableDictionary dictionary];
                [dicInfo setObject:[[STKit sharedInstance] getLastJid] forKey:@"fromId"];
                [dicInfo setObject:[(STMsgModel *) eventMsg messageId] forKey:@"messageId"];
                [dicInfo setObject:[(STMsgModel *) eventMsg message] forKey:@"message"];
                ...
                if (self.chatType == ChatType_Consult) {
                    ...
                    [[STKit sharedInstance] revokeConsultMessageWithMessageId:[(STMsgModel *) eventMsg messageId] message:msgInfo ToJid:self.chatId realToJid:msg.realJid chatType:self.chatType];
                }
                else{
                    [[STKit sharedInstance] revokeMessageWithMessageId:[(STMsgModel *) eventMsg messageId] message:msgInfo ToJid:self.chatId];
                }
...
- (void)loadRemoteSearchMoreMessageData {
    ...
    STMsgModel *msgModel = [self.messageManager.dataSource firstObject];
    [[STKit sharedInstance] getRemoteSearchMsgListByUserId:self.chatId WithRealJid:self.chatId withVersion:msgModel.messageDate withDirection:QIMGetMsgDirectionDown WithLimit:kPageCount WithOffset:(int)self.messageManager.dataSource.count WithComplete:^(NSArray *list) {
        if (list.count) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)loadMoreMessageData {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (self.chatType == ChatType_ConsultServer) {
            [[STKit sharedInstance] getConsultServerMsgLisByUserId:realJid WithVirtualId:userId WithLimit:kPageCount WithOffset:(int)self.messageManager.dataSource.count withLoadMore:YES WithComplete:^(NSArray *list) {
                if (list.count) {
                    dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [[STKit sharedInstance] getMsgListByUserId:userId WithRealJid:realJid WithLimit:kPageCount WithOffset:(int) self.messageManager.dataSource.count withLoadMore:YES WithComplete:^(NSArray *list) {
                if (list.count > 0) {
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)loadNewSystemMsgList {
    if ([STKit getQIMProjectType] != QIMProjectTypeQChat) {
        [[STKit sharedInstance] getSystemMsgLisByUserId:self.chatId WithFromHost:[[STKit sharedInstance] getDomain] WithLimit:kPageCount WithOffset:(int)self.messageManager.dataSource.count withLoadMore:YES WithComplete:^(NSArray *list) {
        ...
        [[STKit sharedInstance] getMsgListByUserId:self.chatId WithRealJid:nil WithLimit:kPageCount WithOffset:(int)self.messageManager.dataSource.count withLoadMore:YES WithComplete:^(NSArray *list) {
            dispatch_async(dispatch_get_main_queue(), ^{
                CGFloat offsetY = self.tableView.contentSize.height -  self.tableView.contentOffset.y;
                NSRange range = NSMakeRange(0, [list count]);
                NSIndexSet *indexSet = [NSIndexSet indexSetWithIndexesInRange:range];
...
- (void)loadFooterRemoteSearchMsgList {
    ...
    STMsgModel *msgModel = [self.messageManager.dataSource lastObject];
    [[STKit sharedInstance] getRemoteSearchMsgListByUserId:self.chatId WithRealJid:self.chatId withVersion:msgModel.messageDate+1 withDirection:QIMGetMsgDirectionUp WithLimit:kPageCount WithOffset:(int)self.messageManager.dataSource.count WithComplete:^(NSArray *list) {
        if (list.count) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)jumpToConverstaionSearch {
    ...
    [[STFastEntrance sharedInstance] openLocalSearchWithXmppId:userId withRealJid:realJid withChatType:self.chatType];
}
...
- (void)qim_textbarSendImageWithImagePath:(NSString *)imagePath {
    ...
    STMsgModel *msg = nil;
    if (self.chatType == ChatType_Consult || self.chatType == ChatType_ConsultServer) {
        msg = [[STKit sharedInstance] createMessageWithMsg:msgText extenddInfo:nil userId:self.virtualJid realJid:self.chatId userType:self.chatType msgType:QIMMessageType_Text forMsgId:nil willSave:YES];
    } else {
        QIMVerboseLog(@"普通图片消息");
        msg = [[STKit sharedInstance] createMessageWithMsg:msgText extenddInfo:nil userId:self.chatId userType:ChatType_SingleChat msgType:QIMMessageType_Text];
    }
    ...
    [self addImageToImageList];
    [self scrollToBottomWithCheck:YES];
    [[STKit sharedInstance] qim_uploadImageWithImageKey:imagePath forMessage:msg];
}

#pragma mark - 发送文件
- (void)qim_textbarSendFileWithFileName:(NSString *)fileName {
    NSString *filePath = [[STKit sharedInstance] qim_getLocalFileDataWithFileName:fileName];
    if (filePath) {
        ...
        STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:extendInfo extenddInfo:extendInfo userId:self.chatId userType:self.chatType msgType:QIMMessageType_File];
#if __has_include("QIMNoteManager.h")
        if(self.encryptChatState == QIMEncryptChatStateEncrypting) {
            msg.encryptChatState = QIMEncryptChatStateEncrypting;
        } else {
        ...
        [_tableView endUpdates];
        [self scrollToBottomWithCheck:YES];
        [[STKit sharedInstance] qim_uploadFileWithFilePath:filePath forMessage:msg];
    }
}
...
- (void)addImageToImageList {
    ...
    for (STMsgModel *msg in tempDataSource) {
...
- (void)sendVoiceWithDuration:(int)duration WithFileName:(NSString *)filename AndFilePath:(NSString *)filepath {
    ...
    STMsgModel *msg = [[STKit sharedInstance] createMessageWithMsg:msgBodyStr extenddInfo:nil userId:self.chatId userType:self.chatType msgType:QIMMessageType_Voice forMsgId:[QIMUUIDTools UUID]];
#if __has_include("QIMNoteManager.h")
    if(self.encryptChatState == QIMEncryptChatStateEncrypting) {
        msg.encryptChatState = QIMEncryptChatStateEncrypting;
    } else {
...
-(void)hintCell:(QIMHintTableViewCell *)cell linkClickedWithInfo:(NSDictionary *)infoFic {
    ...
    if ([eventType isEqualToString:@"extext"]) {
        ...
        if ([type isEqualToString:@"postInterface"]) {
            ...
            //
            //            }];
            [[STKit sharedInstance] sendTPPOSTRequestWithUrl:eventData[@"url"] withRequestBodyData:versionData withSuccessCallBack:^(NSData *responseData) {
                NSDictionary * responseDic = [[QIMJSONSerializer sharedInstance] deserializeObject:responseData error:nil];
                NSLog(@"转人工 %@",responseDic);
            } withFailedCallBack:^(NSError *error) {
