...
- (void)setCellMessage:(STMsgModel *)message
{
    ...
    NSString *filePath = [[[STKit sharedInstance] getDownloadFilePath] stringByAppendingPathComponent:[[fileUrl pathComponents] lastObject]];
    ...
    if (message.messageDirection == QIMMessageDirection_Received) {
        ...
        if ([[[message.xmppId componentsSeparatedByString:@"@"] objectAtIndex:1] hasPrefix:@"conference"]) {
            NSDictionary *groupDic = [[STKit sharedInstance] getGroupCardByGroupId:message.xmppId];
            if (groupDic.count > 0) {
            ...
            NSDictionary *userDic = [[STKit sharedInstance] getUserInfoByUserId:message.to];
            if (userDic.count > 0) {
