...
- (void)viewDidLoad {
    ...
    NSArray * fileMsgs = [[STKit sharedInstance] getMsgsForMsgType:QIMMessageType_File];
...
#endif
    for (STMsgModel * message in _selectArr) {
       STMsgModel *msg = message;
#if __has_include("QIMNoteManager.h")
        ...
        if (self.messageSaveType == ChatType_GroupChat) {
         msg = [[STKit sharedInstance] sendMessage:[msg message] WithInfo:[msg extendInformation] ToGroupId:self.userId WithMsgType:[msg messageType]];
        }else{
         msg = [[STKit sharedInstance] sendMessage:[msg message] WithInfo:[msg extendInformation] ToUserId:self.userId WithMsgType:[msg messageType]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.isSelect) {
        ...
        if ([cell isCellSelected]) {
           ...
           STMsgModel * message = [[_filesDic objectForKey:[_fileKeys objectAtIndex:indexPath.section]] objectAtIndex:indexPath.row];
            for (STMsgModel * msg in _selectArr) {
        ...
        STFilePreviewVC *preview = [[STFilePreviewVC alloc] init];
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{//请求数据源提交的插入或删除指定行接收者。
    if (editingStyle ==UITableViewCellEditingStyleDelete) {//如果编辑样式为删除样式
       STMsgModel * message = [[_filesDic objectForKey:[_fileKeys objectAtIndex:indexPath.section]] objectAtIndex:indexPath.row];
        ...
        NSString *filePath = [[[STKit sharedInstance] getDownloadFilePath] stringByAppendingPathComponent:fileName];
        [[NSFileManager defaultManager] removeItemAtPath:filePath error:nil];
        [[STKit sharedInstance] deleteMsg:message ByJid:nil];
