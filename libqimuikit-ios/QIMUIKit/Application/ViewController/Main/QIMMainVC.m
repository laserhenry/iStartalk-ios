...
@property(nonatomic, strong) STArrowTableView *arrowPopView;
...
- (void)updateNotReadCount {
    ...
    dispatch_async(self.reloadCountQueue, ^{
        NSUInteger appNotReaderCount = [[STKit sharedInstance] getAppNotReaderCount];
        NSInteger appNotRemindCount = [[STKit sharedInstance] getNotRemindNotReaderCount];
...
- (void)updateExploreNotReadCount:(NSNotification *)notify {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        } else if ([STKit getQIMProjectType] == QIMProjectTypeQChat && [STKit sharedInstance].isMerchant) {
            [[STKit sharedInstance] getLeaveMsgNotReaderCountWithCallBack:^(NSInteger num) {
                count = num;
            }];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateWorkFeedNotReadCount:(NSNotification *)notify {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        BOOL workMoment = [[STKit sharedInstance] getLocalWorkMomentNotifyConfig];
        if (workMoment == YES) {
            ...
            if (newWorkNoticeCount > 0) {
                ...
                NSInteger notReadCount = [[STKit sharedInstance] getWorkNoticeMessagesCountWithEventType:@[@(QIMWorkFeedNotifyTypeComment), @(QIMWorkFeedNotifyTypePOSTAt), @(QIMWorkFeedNotifyTypeCommentAt)]];
                if (notReadCount > 0) {
                    //过来的通知不是新评论时，优先展示未读数
                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([STKit getQIMProjectType] == QIMProjectTypeQTalk || [STKit getQIMProjectType] == QIMProjectTypeStartalk || ([STKit getQIMProjectType] == QIMProjectTypeQChat && [STKit sharedInstance].isMerchant)) {
...
- (void)initTotalTabBarArray {
    ...
    */
    ...
    if ([STKit getQIMProjectType] == QIMProjectTypeQTalk && ![[[STKit getLastUserName] lowercaseString] isEqualToString:@"appstore"] && oldAuthSign == YES && [[[STKit sharedInstance] getDomain] isEqualToString:@"ejabhost1"]) {
...
- (UIView *)userListView {
    if (!_userListView) {
        ...
        QIMVerboseLog(@"RNContactView : %d", [[STKit sharedInstance] qimNav_RNContactView]);
        if ([[STKit sharedInstance] qimNav_RNContactView] == NO) {
...
- (UIView *)rnSuggestView {
    if (!_rnSuggestView) {
        ...
        */
        ...
        QIMVerboseLog(@"showOA : %d", [[STKit sharedInstance] qimNav_ShowOA]);
        QIMVerboseLog(@"Domain : %@", [[STKit sharedInstance] qimNav_Domain]);
        if ([[STKit sharedInstance] qimNav_ShowOA] == YES || [[[STKit sharedInstance] qimNav_Domain] isEqualToString:@"ejabhost2"] || [[[STKit sharedInstance] qimNav_Domain] isEqualToString:@"ejabhost1"]) {
            ...
            UIViewController *vc = nil;
            if ([RunC respondsToSelector:sel]) {
                NSDictionary *param = @{@"module": @"FoundPage", @"properties": @{@"domain": [[STKit sharedInstance] getDomain]}};
                vc = [RunC performSelector:sel withObject:param];
            }
            _rnSuggestView = [vc view];
            [_rnSuggestView setFrame:CGRectMake(0, 0, _contentView.width, _contentView.height)];
...
- (void)customTabBar:(QIMCustomTabBar *)tabBar didSelectIndex:(NSUInteger)index {
    ...
    if ([tabBarId isEqualToString:[NSBundle qim_localizedStringForKey:@"tab_title_chat"]]) {
        ...
        long long lastUpdateMomentTime = [[[STKit sharedInstance] userObjectForKey:@"lastUpdateMomentTime"] longLongValue];
        if (currentTime - lastUpdateMomentTime > 5 * 60 * 60) {
            [self.momentView updateMomentView];
            [[STKit sharedInstance] setUserObject:@(currentTime) forKey:@"lastUpdateMomentTime"];
        }
...
//    [[QIMKit sharedInstance] userObjectForKey:@"userToken"];
    NSString *userFullJid = [[STKit sharedInstance] userObjectForKey:@"kFullUserJid"];
    QIMVerboseLog(@"autoLogin lastUserName : %@, userFullJid : %@, userToken : %@", lastUserName, userFullJid, userToken);
    QIMVerboseLog(@"autoLogin UserDict : %@", [[STKit sharedInstance] userObjectForKey:@"Users"]);
    if ([lastUserName length] > 0 && [userToken length] > 0) {
        if ([lastUserName isEqualToString:@"appstore"]) {
            [[STKit sharedInstance] updateLastTempUserToken:@"appstore"];
//            [[QIMKit sharedInstance] setUserObject:@"appstore" forKey:@"kTempUserToken"];
            [[STKit sharedInstance] loginWithUserName:@"appstore" WithPassWord:@"774713"];
        } else if ([[lastUserName lowercaseString] isEqualToString:@"qtalktest"]) {
            [[STKit sharedInstance] updateLastTempUserToken:@"qtalktest123"];
//            [[QIMKit sharedInstance] setUserObject:@"qtalktest123" forKey:@"kTempUserToken"];
            [[STKit sharedInstance] loginWithUserName:@"qtalktest" WithPassWord:@"qtalktest123"];
        } else {
            if ([[STKit sharedInstance] qimNav_LoginType] == QTLoginTypeSms) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    NSString *pwd = [NSString stringWithFormat:@"%@@%@", [QIMUUIDTools deviceUUID], userToken];
                    [[STKit sharedInstance] updateLastTempUserToken:userToken];
//                    [[QIMKit sharedInstance] setUserObject:userToken forKey:@"kTempUserToken"];
                    [[STKit sharedInstance] loginWithUserName:lastUserName WithPassWord:pwd];
                });
            } else {
                [[STKit sharedInstance] updateLastTempUserToken:userToken];
//                [[QIMKit sharedInstance] setUserObject:userToken forKey:@"kTempUserToken"];
                [[STKit sharedInstance] loginWithUserName:lastUserName WithPassWord:userToken];
            }
        }
    } else {
...
- (STArrowTableView *)arrowPopView {
    ...
    NSString *wikiHost = [[STKit sharedInstance] qimNav_WikiUrl];
    ...
    NSString *Language = [[STKit sharedInstance] currentLanguage];
    if ([Language containsString:@"en"]) {
        arrowWidth = 220;
    }
    point = CGPointMake(rect3.origin.x + rect3.size.width / 2, rect3.origin.y + rect3.size.height / 2);
    _arrowPopView = [[STArrowTableView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height) Origin:point Width:arrowWidth Height:50 * self.moreActionArray.count + 10 Type:Type_UpRight Color:[UIColor whiteColor]];
...
- (void)searchMoment:(id)sender {
    [STFastEntrance openWorkMomentSearchVc];
}

//我的驼圈儿navbar入口
- (void)myOwnerMoment:(id)sender {
    [[STFastEntrance sharedInstance] openUserWorkWorldWithParam:@{@"UserId": [[STKit sharedInstance] getLastJid]}];
#if __has_include("QIMAutoTracker.h")
...
/*
    ...
    */
    [STFastEntrance openQIMRNVCWithModuleName:@"MySetting" WithProperties:@{@"Screen": @"Setting"}];
#if __has_include("QIMAutoTracker.h")
...
- (void)qim_UpdateAlertViewDidClickWithType:(QIMUpgradeType)type withUpdateDic:(QIMUpdateDataModel *)updateModel {
    if (QIMUpgradeNow == type) {
        ...
        [STFastEntrance openWebViewForUrl:updateUrl showNavBar:YES];
    } else {
        //下次再说
        NSInteger version = updateModel.version;
//        [[updateDic objectForKey:@"version"] integerValue];
        [[STKit sharedInstance] setUserObject:@(version) forKey:@"updateAppVersion"];
    }
}
...
- (void)selectIndexPathRow:(NSInteger)index {
    ...
    if ([moreActionId isEqualToString:[NSBundle qim_localizedStringForKey:@"Scan"]]) {
        ...
        if ([[STKit sharedInstance] qimNav_WikiUrl].length > 0) {
            [STFastEntrance openWebViewForUrl:[[STKit sharedInstance] qimNav_WikiUrl] showNavBar:YES];
#if __has_include("QIMAutoTracker.h")
...
- (void)oneKeyRead {
    NSUInteger count = [[STKit sharedInstance] getAppNotReaderCount];
    if (count) {
