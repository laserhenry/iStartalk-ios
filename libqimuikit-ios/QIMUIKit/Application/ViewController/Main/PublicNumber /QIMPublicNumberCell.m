...
- (void)refreshUI{
    ...
    [_iconImageView setImage:[[STKit sharedInstance] getPublicNumberHeaderImageByFileName:self.headerSrc]];
    NSString *text = nil;
    switch (self.msgType) {
        ...
        default:
        {
            if (self.content.length > 0) {
                text = [[STKit sharedInstance] getMsgShowTextForMessageType:(QIMMessageType)self.msgType];
                if (text.length <= 0) {
                    text = @"你收到了一条消息。";
                }
            }
