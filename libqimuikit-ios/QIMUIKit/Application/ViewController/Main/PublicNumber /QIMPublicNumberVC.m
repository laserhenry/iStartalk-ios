...
- (void)viewDidLoad {
    ...
    _dataSource = [NSMutableArray arrayWithArray:[[STKit sharedInstance] getPublicNumberList]];
    [self initWithNav];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        ...
        NSDictionary *cardDic = [[STKit sharedInstance] getPublicNumberCardByJId:[NSString stringWithFormat:@"%@@%@",publicNumberId,[[STKit sharedInstance] getDomain]]];
        if (cardDic.count > 0) {
            ...
            [cardVC setJid:[NSString stringWithFormat:@"%@@%@",publicNumberId,[[STKit sharedInstance] getDomain]]];
...
//    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"查找公众号" message:@"输入公众号ID打开名片，查看关注！" delegate:self cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"Cancel"] otherButtonTitles:[NSBundle qim_localizedStringForKey:@"Confirm"],nil];
...
-(void)searchBarSearchButtonClicked:(UISearchBar *)searchBar { 
    _searchDataSource = [[STKit sharedInstance] searchPublicNumberListByKeyStr:[searchBar text]];
...
{
    _searchDataSource = [[STKit sharedInstance] searchPublicNumberListByKeyStr:searchString];
    return YES;
}
