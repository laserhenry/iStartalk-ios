...
- (void)viewDidLoad {
    ...
    if (self.publicRobotDic) {
        ...
        [tempDic setQIMSafeObject:[NSString stringWithFormat:@"%@@%@",self.publicNumberId,[[STKit sharedInstance] getDomain]] forKey:@"XmppId"];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] updatePublicNumberCardByIds:@[@{@"robot_name":self.publicNumberId,@"version":@(-1)}] WithNeedUpdate:NO withCallBack:^(NSArray *list) {
                _resultList = list;
        ...
        _publicNumberCardDic = [[STKit sharedInstance] getPublicNumberCardByJid:self.jid];
...
- (void)initWithTableView{
    ...
    [iconImageView setImage:[[STKit sharedInstance] getPublicNumberHeaderImageByFileName:headerSrc]];
...
- (void)onConcernClick:(UIButton *)sender{
    ...
    [[STKit sharedInstance] focusOnPublicNumberId:self.publicNumberId withCallBack:^(BOOL isSuccess) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (isSuccess) {
                [[STKit sharedInstance] bulkInsertPublicNumbers:_resultList];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0) {
        [[STKit sharedInstance] cancelFocusOnPublicNumberId:self.publicNumberId withCallBack:^(BOOL isSuccess) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (isSuccess) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([value isEqualToString:@"qcode"]) {
        [STFastEntrance showQRCodeWithQRId:self.publicNumberId withType:QRCodeType_RobotQR];
    }
}
