...
#import "SearchBar.h"
#import "STContactDatasourceMgr.h"
...
-(void)loadContractList {
   ...
   dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
       if ([STKit getQIMProjectType] == QIMProjectTypeQChat) {
           _frientList = [[STKit sharedInstance] selectFriendList];
       }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    STDatasourceItem * item  = [[[STContactDatasourceMgr getInstance] QtalkDataSourceItem]  objectAtIndex:indexPath.row];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    int count = (int)[[[STContactDatasourceMgr getInstance] QtalkDataSourceItem] count];
    return count;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([[[STContactDatasourceMgr getInstance] QtalkDataSourceItem] count] > 0 ) {
        
        STDatasourceItem *qtalkItem = [[[STContactDatasourceMgr getInstance] QtalkDataSourceItem] objectAtIndex:indexPath.row];
        if ([qtalkItem isParentNode]) {
            
            if ([qtalkItem isExpand]) {
                
                [[STContactDatasourceMgr getInstance] collapseBranchAtIndex:indexPath.row];
                ...
                [[STContactDatasourceMgr getInstance] expandBranchAtIndex:indexPath.row];
...
- (void)joinGroupById:(id)sender {
    ...
    if ([_groupIDArray count] > 0 && ([self.groupID length] > 0) && self.existGroup) {
        ...
        [[STKit sharedInstance] joinGroupWithBuddies:self.groupID groupName:self.groupName WithInviteMember:_groupIDArray withCallback:^{
            [weakView goBack:nil];
        }];
    } else {
...
- (void)searchBar:(SearchBar *)SectionSearchBar textDidChange:(NSString *)searchText {
    ...
    if (searchText.length > 0) {
        [searchResults addObjectsFromArray:[[STKit sharedInstance] searchUserListBySearchStr:searchText]];
    }
