...
- (void)viewDidLoad {
    ...
    _groupList = [[NSMutableArray alloc] initWithArray:[[STKit sharedInstance] getGroupList]];
...
- (void)onDoneButtonClick:(UIButton *)sender{
    self.groupId = [NSString stringWithFormat:@"%@@%@.%@",_groupNameField.text,@"conference",[[STKit sharedInstance] getDomain]];
    _nickName = [[STKit sharedInstance] getMyNickName];
    if ([[STKit sharedInstance] isGroupMemberByGroupId:self.groupId]) {
...
- (void)onJoinSuccessNotify:(NSNotification *)notify{
    if ([_groupId isEqualToString:notify.object]) {
        ...
        [[STKit sharedInstance] openGroupSessionByGroupId:self.groupId ByName:_groupNameField.text];
