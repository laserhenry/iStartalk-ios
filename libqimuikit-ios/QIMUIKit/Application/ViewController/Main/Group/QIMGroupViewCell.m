...
- (void)updateCell:(NSNotification *)notify {
    ...
    if ([groupIds containsObject:self.groupID]) {
        NSDictionary *cardDic = [[STKit sharedInstance] getGroupCardByGroupId:self.groupID];
...
- (void)refresh {
    ...
    [_headerView qim_setImageWithJid:self.groupID WithChatType:ChatType_GroupChat];
