...
@property (nonatomic, strong)STMsgModel *currentMsg;          //当前消息
...
- (void)clearNotRead {
    if (!self.bindId) {
        if (self.chatType == ChatType_GroupChat) {
            ...
            [[STKit sharedInstance] clearNotReadMsgByGroupId:self.jid];
        } else if (self.chatType == ChatType_SingleChat) {
            
            [[STKit sharedInstance] clearNotReadMsgByJid:self.jid];
        } else if (self.chatType == ChatType_System) {
            
            [[STKit sharedInstance] clearSystemMsgNotReadWithJid:self.jid];
        } else if (self.chatType == ChatType_PublicNumber) {
            [[STKit sharedInstance] clearNotReadMsgByPublicNumberId:self.jid];
        } else if (self.chatType == ChatType_ConsultServer) {
            NSString *realJid = [self.infoDic objectForKey:@"RealJid"];
            NSString *xmppId = [self.infoDic objectForKey:@"XmppId"];
            [[STKit sharedInstance] clearNotReadMsgByJid:xmppId ByRealJid:realJid];
        } else if (self.chatType == ChatType_Consult) {
            NSString *xmppId = [self.infoDic objectForKey:@"XmppId"];
            [[STKit sharedInstance] clearNotReadMsgByJid:xmppId ByRealJid:xmppId];
        } else if (self.chatType == ChatType_CollectionChat) {
            NSString *xmppId = [self.infoDic objectForKey:@"XmppId"];
            [[STKit sharedInstance] clearNotReadCollectionMsgByJid:xmppId];
        } else {
            
            return;
        }
    } else {
        [[STKit sharedInstance] clearNotReadCollectionMsgByBindId:self.bindId WithUserId:self.jid];
    }
}
...
- (NSString *)refreshContentWithMessage:(NSString *)message {
    ...
    self.nickName = [[STKit sharedInstance] getUserMarkupNameWithUserId:self.msgFrom];
    switch (self.msgType) {
        ...
        case QIMMessageType_Revoke: {
            content = [[STKit sharedInstance] getMsgShowTextForMessageType:self.msgType];
    ...
    NSDictionary *notSendDic = [[STKit sharedInstance] getNotSendTextByJid:self.jid];
...
- (void)refreshNotReadCount {
    ...
    CGFloat contentLabelWidth = timeLabelMaxX - self.nameLabel.left - 35;
    if (self.bindId) {
        self.notReadCount = [[STKit sharedInstance] getNotReadCollectionMsgCountByBindId:self.bindId WithUserId:self.jid];
    }
    ...
    if (self.showName) {
        ...
        switch (self.chatType) {
            case ChatType_GroupChat: {
                if (!self.bindId) {
                    NSDictionary *groupVcard = [[STKit sharedInstance] getGroupCardByGroupId:self.jid];
                    if (groupVcard.count > 0) {
                        ...
                        if (groupName.length > 0 && groupUpdateTime > 0) {
                            ...
                            [[STKit sharedInstance] updateGroupCardByGroupId:self.jid withCache:YES];
                        }
                    } else {
                        [[STKit sharedInstance] updateGroupCardByGroupId:self.jid withCache:YES];
                    }
                } else {
                    NSDictionary *cardDic = [[STKit sharedInstance] getCollectionGroupCardByGroupId:self.jid];
            ...
            case ChatType_SingleChat: {
                if (!self.bindId) {
                    //备注
                    NSString *remarkName = [[STKit sharedInstance] getUserMarkupNameWithUserId:self.jid];
                    ...
                    NSDictionary *userInfo = [[STKit sharedInstance] getCollectionUserInfoByUserId:self.jid];
                    NSString *userName = [userInfo objectForKey:@"Name"];
            ...
            case ChatType_ConsultServer: {
                ...
                NSDictionary *virtualInfo = [[STKit sharedInstance] getUserInfoByUserId:xmppId];
                ...
                NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:realJid];
...
- (void)refeshContent {
    ...
    if (message.length > 0) {
        dispatch_async([[STKit sharedInstance] getLoadSessionContentQueue], ^{
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
                return;
            }
            NSArray *atMeMessages = [[STKit sharedInstance] getHasAtMeByJid:strongSelf.jid];
