...
- (void)qimStickySession:(NSIndexPath *)indexPath {
    ...
    NSDictionary *dict = @{@"topType": @(![[STKit sharedInstance] isStickWithCombineJid:combineJid]), @"chatType": @(chatType)};
    NSString *value = [[QIMJSONSerializer sharedInstance] serializeObject:dict];
    [[STKit sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKStickJidDic WithSubKey:combineJid WithConfigValue:value WithDel:[[STKit sharedInstance] isStickWithCombineJid:combineJid] withCallback:nil];
}
...
- (void)deleteStick:(NSIndexPath *)indexPath {
    ...
    NSString *value = [[QIMJSONSerializer sharedInstance] serializeObject:dict];
    [[STKit sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKStickJidDic WithSubKey:combineJid WithConfigValue:value WithDel:YES withCallback:nil];
}
...
- (void)qimDeleteSession:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < [tempArray count]) {
        ...
        if (sid && (chatType != ChatType_Consult && chatType != ChatType_ConsultServer)) {
            ...
            [[STKit sharedInstance] removeSessionById:sid];
            ...
            [[STKit sharedInstance] removeConsultSessionById:sid RealId:realJid];
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.qtBlock) {
                    self.qtBlock();
                }
