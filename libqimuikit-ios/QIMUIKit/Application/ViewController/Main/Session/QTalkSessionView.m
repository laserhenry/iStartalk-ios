...
#import "STCustomPresentationController.h"
#import "STCustomPopMgr.h"
...
- (UIView *)otherPlatformView {
    if (!_otherPlatformView) {
        ...
        if ([STKit getQIMProjectType] == QIMProjectTypeQTalk) {
            ...
            platTitle = [STKit getQIMProjectTitleName];
        }
...
- (void)showFileTrans {
    [[STFastEntrance sharedInstance] openFileTransMiddleVC];
}
...
- (void)reloadTableView {
    ...
    dispatch_async(self.reloadListViewQueue, ^{
        @autoreleasepool {
            ...
            NSDictionary *friendDic = [[STKit sharedInstance] getLastFriendNotify];
            NSInteger friendNotifyCount = [[STKit sharedInstance] getFriendNotifyCount];
            NSArray *temp = nil;
            if (self.showNotReadList == YES) {
                temp = [[STKit sharedInstance] getNotReadSessionList];
            } else {
                temp = [[STKit sharedInstance] getSessionList];
            }
...
- (void)showNotConnectWebView:(UITapGestureRecognizer *)tapgesture {
    ...
    [STFastEntrance openWebViewWithHtmlStr:htmlString showNavBar:YES];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isSelected == NO) {
        ...
        if ([self.rootViewController isKindOfClass:[QIMMainVC class]]) {
            ...
            if (!rootNav) {
                rootNav = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
...
//                });
                QIMGroupChatVC *chatGroupVC = (QIMGroupChatVC *) [[STFastEntrance sharedInstance] getGroupChatVCByGroupId:jid];
...
//                });
            ...
            case ChatType_SingleChat: {
                QIMChatVC *chatSingleVC = (QIMChatVC *) [[STFastEntrance sharedInstance] getSingleChatVCByUserId:jid];
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {//如果编辑样式为删除样式
        ...
        if (indexPath.row < [self.dataManager.dataSource count]) {
            ...
            if (jid && (chatType != ChatType_Consult || chatType != ChatType_ConsultServer)) {
                ...
                [[STKit sharedInstance] removeSessionById:jid];
                ...
                NSString *realJid = [infoDic objectForKey:@"RealJid"];
                [[STKit sharedInstance] removeConsultSessionById:jid RealId:realJid];
