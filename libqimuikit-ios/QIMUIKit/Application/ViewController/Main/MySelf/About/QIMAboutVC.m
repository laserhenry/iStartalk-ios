...
#import "STGDPerformanceMonitor.h"
...
- (void)showLoggingAction:(UIGestureRecognizer *)gesture {
    ...
    if (![self.dataSource containsObject:@"isLogging"]) {
        UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:[NSBundle qim_localizedStringForKey:@"Reminder"] message:@"确认开始记录App性能数据吗？再次点击屏幕五下关闭！！！" preferredStyle:UIAlertControllerStyleAlert];
...
//            [weakSelf.dataSource addObject:@"isLogging"];
        ...
        [[STKit sharedInstance] setUserObject:@(QIMLocalLogTypeClosed) forKey:@"recordLogType"];
        [[STGDPerformanceMonitor sharedInstance] stopMonitoring];
        [[STKit sharedInstance] setUserObject:@(NO) forKey:@"isInstruments"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([value isEqualToString:@"Rate QTalk"] || [value isEqualToString:@"Rate QChat"]) {
        ...
        UISwitch *switchButton = [[UISwitch alloc] initWithFrame:CGRectMake(0, 0, 60, 25)];
        BOOL isInstrument = [[[STKit sharedInstance] userObjectForKey:@"isInstruments"] boolValue];
...
- (void)recordInstruments:(UISwitch *)sender {
    [[STKit sharedInstance] setUserObject:@(sender.on) forKey:@"isInstruments"];
    if (sender.on) {
        [[STGDPerformanceMonitor sharedInstance] startMonitoring];
    } else {
        [[STGDPerformanceMonitor sharedInstance] stopMonitoring];
    }
}
...
- (NSString *)dataFileSize {
    long long totalSize = [[STDataContraller getInstance] sizeOfDBPath];
    NSString *str = [[STDataContraller getInstance] transfromTotalSize:totalSize];
    return str;
}

- (NSString *)dataWalFileSize {
    long long totalSize = [[STDataContraller getInstance] sizeOfDBWALPath];
    NSString *str = [[STDataContraller getInstance] transfromTotalSize:totalSize];
    return str;
}
