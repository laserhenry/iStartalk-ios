...
{
    NSArray *groups = [[STKit sharedInstance] getGroupList];
...
- (void)getGroupList {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if ([STKit getQIMProjectType] != QIMProjectTypeQChat) {
            [[STKit sharedInstance] getIncrementMucList:0];
        } else {
            [[STKit sharedInstance] quickJoinAllGroup];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            _myGroupListArray = [NSMutableArray arrayWithArray:[[STKit sharedInstance] getMyGroupList]];
            [_tableView reloadData];
        });
    });
}
...
{
    NSArray *groups = [[STKit sharedInstance] getMyGroupList];
    if (groups) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_searchBarKeyTmp.text.length > 0) {
        ...
        if ( [[STKit sharedInstance] isGroupMemberByGroupId:cell.groupID]) {
            [[STKit sharedInstance] clearNotReadMsgByGroupId:cell.groupID];
            [cell refresh];
            [self openGroupChatVC:cell bAddGroupSession:YES];
        }else{
            //群名片页面
...
-(void)openGroupChatVC:(QIMGroupViewCell *)groupViewCell bAddGroupSession:(BOOL)bAddGroupSession {
    ...
    if ([self.delegate respondsToSelector:@selector(selectGroupWithJid:)]) {
        ...
        if (bAddGroupSession == YES) {
            [[STKit sharedInstance] openGroupSessionByGroupId:groupViewCell.groupID ByName:groupViewCell.userName];
        }
        [STFastEntrance openGroupChatVCByGroupId:groupViewCell.groupID];
    }
}
