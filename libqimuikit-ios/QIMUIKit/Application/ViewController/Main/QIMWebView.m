...
- (void)performShareWorkMomentActivity {
    ...
    [STFastEntrance presentWorkMomentPushVCWithLinkDic:linkDic withNavVc:self.navigationController];
}
...
- (void)onMoreClick{
    ...
    QIMVerboseLog(@"onMoreClick webView Url : %@", _webView.request.URL);
    {
        ...
        // 给appstore帐号审核用
        if (tempUrl == nil) {
            tempUrl = [NSURL URLWithString:@"https://uk.startalk.im"];
        }
    }
...
- (void)viewDidLoad {
    ...
    if (![[STKit sharedInstance] getIsIpad] && self.fromRegPackage == NO) {
    ...
    if (self.htmlString) {
        ...
        if (self.fromRegPackage) {
            self.url = [self.url stringByAppendingFormat:@"&q_d=%@", [[STKit sharedInstance] getDomain]];
        }
        ...
        if (_package) {
            ...
            NSString *quserId = [STKit getLastUserName];
            ...
            [quCookieProperties setQIMSafeObject:[[STKit sharedInstance] qimNav_DomainHost] forKey:NSHTTPCookieDomain];
            ...
            [qnmCookieProperties setQIMSafeObject:[[STKit sharedInstance] qimNav_DomainHost] forKey:NSHTTPCookieDomain];
            ...
            if ([qCookie qim_isStringSafe] && [vCookie qim_isStringSafe] && [tCookie qim_isStringSafe])
            {
                ...
                [qcookieProperties setQIMSafeObject:[[STKit sharedInstance] qimNav_DomainHost]forKey:NSHTTPCookieDomain];
                ...
                [vcookieProperties setQIMSafeObject:[[STKit sharedInstance] qimNav_DomainHost] forKey:NSHTTPCookieDomain];
                ...
                [tcookieProperties setQIMSafeObject:[[STKit sharedInstance] qimNav_DomainHost] forKey:NSHTTPCookieDomain];
            ...
            NSString *u = [STKit getLastUserName];
            NSString *k = [[STKit sharedInstance] remoteKey];
            ...
            [ucookieProperties setQIMSafeObject:[[STKit sharedInstance] qimNav_DomainHost] forKey:NSHTTPCookieDomain];
            ...
            [kcookieProperties setValue:[[STKit sharedInstance] qimNav_DomainHost] forKey:NSHTTPCookieDomain];
        ...
        if ([self.url isEqualToString:[[STKit sharedInstance] qimNav_getManagerAppUrl]]) {
            ...
            [confignavProperties setQIMSafeObject:[[STKit sharedInstance] qimNav_DomainHost] forKey:NSHTTPCookieDomain];
...
- (void)openSingleChat:(NSString *)jid{
    ...
    NSDictionary *userInfoDic = [[STKit sharedInstance] getUserInfoByUserId:jid];
    if (userInfoDic == nil) {
//        [[QIMKit sharedInstance] updateUserCard:@[jid]];
        [[STKit sharedInstance] updateUserCard:jid withCache:YES];
        userInfoDic = [[STKit sharedInstance] getUserInfoByUserId:jid];
    }
    if (userInfoDic) {
        NSString *xmppId = [userInfoDic objectForKey:@"XmppId"];
        NSString *name = [userInfoDic objectForKey:@"Name"];
        [[STKit sharedInstance] clearNotReadMsgByJid:xmppId];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if ([urlStr hasPrefix:@"qchatiphone://"] && components.count > 1) {
        ...
        if ([[[components objectAtIndex:1] lowercaseString] hasPrefix:@"//open_singlechat"]) {
            NSString *propertys = navigationAction.request.URL.query;
            ...
            [[STKit sharedInstance] setConversationParam:tempDic WithJid:xmppJid];
        ...
        if ([[[components objectAtIndex:1] lowercaseString] hasPrefix:@"//close"]) {
            ...
            [self shareUrlImg:navigationAction.request.URL];
            return ;
        }
