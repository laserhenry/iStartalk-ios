...
- (void)viewDidLoad {
    ...
    NSArray *temp = [[STKit sharedInstance] getSessionList];
    ...
    _myGroups = [NSMutableArray arrayWithArray:[[STKit sharedInstance] getMyGroupList]];
...
- (void)searchBar:(SearchBar *)SectionSearchBar textDidChange:(NSString *)searchText {
    ...
    if (searchText.length > 0) {
        [_searchResults addObjectsFromArray:[[STKit sharedInstance] searchUserListBySearchStr:searchText]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_searchResults.count + _searchGroupResults.count > 0) {
        if (indexPath.row >= _searchResults.count) {
            ...
            NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:jid];
...
- (void)redeemDidSelectContactWithJid:(NSString *)jid chatType:(ChatType)chatType {
    ...
    NSString *name = nil;
    if (chatType == ChatType_SingleChat) {
        infoDic = [[STKit sharedInstance] getUserInfoByUserId:jid];
    } else {
        infoDic = [[STKit sharedInstance] getGroupCardByGroupId:jid];
    }
...
//解析原始消息
- (NSDictionary *)getOriginMessageWithMsg:(STMsgModel *)msg {
    if (msg.messageId.length <= 0) {
        return nil;
    }
    QIMVerboseLog(@"msgId : %@", msg.messageId);
    NSDictionary *msgDic = [[STKit sharedInstance] getMsgDictByMsgId:msg.messageId];
    if (msgDic.count > 0) {
        ...
        if (msgRaw) {
            NSDictionary * originMsgDic = [[STKit sharedInstance] parseMessageByMsgRaw:msgRaw];
...
- (void)selectContactWithJid:(NSString *)jid{
    ...
    QIMChatVC *chatVC = (QIMChatVC *)[[STFastEntrance sharedInstance] getSingleChatVCByUserId:jid];
    /*
    ...
    */
    ...
    if (self.ExternalForward) {
        ...
        if (Uploading == YES) {
            ...
            STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_SingleChat msgType:self.message.messageType backinfo:nil];
            [[STKit sharedInstance] qim_uploadFileWithFilePath:IPLocalPath forMessage:newMsg];
        } else {
            STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_SingleChat msgType:self.message.messageType backinfo:nil];
            [[STKit sharedInstance] sendMessage:newMsg ToUserId:jid];
        }
    } else {
        if (self.message) {
            ...
            if (msgContent.length > 0 && msgExtendInfo.length > 0) {
               STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_SingleChat msgType:self.message.messageType backinfo:self.message.backupInfo];
                [[STKit sharedInstance] sendMessage:newMsg ToUserId:jid];
            } else {
                ...
                if (msgType == QIMMessageType_None && !self.ExternalForward) {
                    [[STKit sharedInstance] sendMessage:self.message.message WithInfo:self.message.extendInformation ToUserId:jid WithMsgType:self.message.messageType];
                } else {
                   STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_SingleChat msgType:msgType backinfo:backUpInfo];
                    [[STKit sharedInstance] sendMessage:newMsg ToUserId:jid];
                }
            }
        } else if (self.messageList.count){
            for (STMsgModel * msg in self.messageList) {
                ...
                QIMMessageType msgType = [[originMsg objectForKey:@"MsgType"] integerValue];
                if (msgType == QIMMessageType_None && !self.ExternalForward) {
                    [[STKit sharedInstance] sendMessage:msg.message WithInfo:msg.extendInformation ToUserId:jid WithMsgType:msg.messageType];
                } else {
                   STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_SingleChat msgType:msgType backinfo:backUpInfo];
                    [[STKit sharedInstance] sendMessage:newMsg ToUserId:jid];
                }
...
#warning 转发消息 可能出现丢字段的情况，需要从本地数据库取出原始消息拼接
    [[STKit sharedInstance] clearNotReadMsgByGroupId:jid];
    NSDictionary *infoDic = [[STKit sharedInstance] getGroupCardByGroupId:jid];
    QIMGroupChatVC *chatGroupVC = [[STFastEntrance sharedInstance] getGroupChatVCByGroupId:jid];
    /*
    ...
    if (self.ExternalForward) {
        ...
        BOOL Uploading = [[msgExtendInfo objectForKey:@"Uploading"] boolValue];
        if (Uploading == YES) {
            STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_GroupChat msgType:self.message.messageType backinfo:nil];
            NSString *IPLocalPath = [msgExtendInfo objectForKey:@"IPLocalPath"];
            [[STKit sharedInstance] qim_uploadFileWithFilePath:IPLocalPath forMessage:newMsg];
        } else {
            STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:self.message.message extenddInfo:self.message.extendInformation userId:jid userType:ChatType_GroupChat msgType:self.message.messageType backinfo:nil];
            STMsgModel *tempMsg = [[STKit sharedInstance] sendMessage:newMsg ToUserId:jid];
        ...
        if (self.message) {
            ...
            NSString *msgExtendInfo = self.message.extendInformation;
            if (msgContent.length > 0 && msgExtendInfo.length > 0) {
               STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_GroupChat msgType:self.message.messageType backinfo:self.message.backupInfo];
                [[STKit sharedInstance] sendMessage:newMsg ToUserId:jid];
            } else {
                NSDictionary *originMsg = [self getOriginMessageWithMsg:self.message];
                NSString *msgContent = [originMsg objectForKey:@"Content"];
                ...
                QIMMessageType msgType = [[originMsg objectForKey:@"MsgType"] integerValue];
                if (msgType == QIMMessageType_None && !self.ExternalForward) {
                    [[STKit sharedInstance] sendMessage:self.message.message WithInfo:self.message.extendInformation ToGroupId:jid WithMsgType:self.message.messageType];
                } else {
                   STMsgModel *newMsg = [[STKit sharedInstance] createMessageWithMsg:msgContent extenddInfo:msgExtendInfo userId:jid userType:ChatType_GroupChat msgType:msgType backinfo:backUpInfo];
                    [[STKit sharedInstance] sendMessage:newMsg ToUserId:jid];
                }
            }
        } else if (self.messageList.count){
