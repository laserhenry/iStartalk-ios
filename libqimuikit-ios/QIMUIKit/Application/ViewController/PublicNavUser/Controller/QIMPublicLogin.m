...
- (void)setupUI {
    ...
    if ([[STKit sharedInstance] qimNav_isToC]) {
...
- (void)registerNew:(id)sender {
    [STFastEntrance openWebViewForUrl:@"https://i.startalk.im/home/#/register" showNavBar:YES];
}
...
- (void)forgotPWD:(id)sender {
    ...
    [STFastEntrance openWebViewForUrl:[[STKit sharedInstance] qimNav_resetPwdUrl] showNavBar:YES];
}
...
- (void)autoLogin {
    ...
    if ([[lastUserName lowercaseString] isEqualToString:@"appstore"]) {
        NSDictionary *testQTalkNav = @{QIMNavNameKey:@"Startalk", QIMNavUrlKey:@"https://www.qtalk.app:8080/newapi/nck/qtalk_nav.qunar"};
        [[STKit sharedInstance] qimNav_updateNavigationConfigWithNavDict:testQTalkNav WithUserName:lastUserName Check:YES WithForcedUpdate:YES withCallBack:^(BOOL success) {
            if (success) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    [[STKit sharedInstance] updateLastTempUserToken:@"appstore"];
                    [[STKit sharedInstance] loginWithUserName:lastUserName WithPassWord:@"774713"];
                });
            }
        }];

    } else {
        NSString *token = [[STKit sharedInstance] getLastUserToken];
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] loginWithUserName:lastUserName WithPassWord:token];
        });
    }
}
...
#warning 报错即将登陆的用户名 并请求导航
        ...
        [[STKit sharedInstance] setUserObject:userName forKey:@"currentLoginUserName"];
        if ([[userName lowercaseString] isEqualToString:@"appstore"]) {
            NSDictionary *testQTalkNav = @{QIMNavNameKey:@"Startalk", QIMNavUrlKey:@"https://www.qtalk.app:8080/newapi/nck/qtalk_nav.qunar"};
            /*
            [[STKit sharedInstance] qimNav_updateNavigationConfigWithNavDict:testQTalkNav WithUserName:userName Check:YES WithForcedUpdate:YES withCallBack:^(BOOL success) {
                if (success) {
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        [[STKit sharedInstance] loginWithUserName:userName WithPassWord:@"774713"];
            ...
            */
            ...
            [[STKit sharedInstance] qimNav_updateNavigationConfigWithNavDict:testQTalkNav WithUserName:userName Check:YES WithForcedUpdate:YES withCallBack:^(BOOL success) {
                ...
                [[STKit sharedInstance] getNewUserTokenWithUserName:userName WithPassword:pwd withCallback:^(NSDictionary *result) {
                    if (result) {
                        ...
                        if (ret && errcode == 0) {
                            ...
                            if (newUserName.length && newToken.length) {
                                [[STKit sharedInstance] updateLastTempUserToken:newToken];
                                [[STKit sharedInstance] loginWithUserName:newUserName WithPassWord:newToken];
                            }
                        } else {
            ...
            if ([[STKit sharedInstance] qimNav_LoginType] == QTLoginTypeNewPwd) {
                ...
                __weak id weakSelf = self;
                [[STKit sharedInstance] qimNav_updateNavigationConfigWithNavDict:publicQTalkNav WithUserName:userName Check:YES WithForcedUpdate:YES withCallBack:^(BOOL success) {
                    NSString *pwd = self.userPwdTextField.text;
                    [[STKit sharedInstance] getNewUserTokenWithUserName:userName WithPassword:pwd withCallback:^(NSDictionary *result) {
                        if (result) {
                            BOOL ret = [[result objectForKey:@"ret"] boolValue];
                ...
                [[STKit sharedInstance] qimNav_updateNavigationConfigWithNavDict:publicQTalkNav WithUserName:userName Check:YES WithForcedUpdate:YES withCallBack:^(BOOL success) {
                    NSString *pwd = self.userPwdTextField.text;
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                        [[STKit sharedInstance] updateLastTempUserToken:pwd];
                        [[STKit sharedInstance] loginWithUserName:userName WithPassWord:pwd];
                    });
                }];
            }
        }
...
//        [weakSelf stopLoginAnimation];
        ...
        [STFastEntrance openWebViewWithHtmlStr:netHelperString showNavBar:YES];
    }];
...
- (void)loginNotify:(NSNotification *)notify {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([notify.object boolValue]) {
            ...
            [[STKit sharedInstance] setUserObject:[infoDictionary objectForKey:@"CFBundleVersion"] forKey:@"QTalkApplicationLastVersion"];
...
- (void)onSaveWith:(NSString *)navAddressText navUrl:(NSString *)navUrl{
    ...
    if (navAddressText.length > 0) {
        ...
        [[STKit sharedInstance] setUserObject:userWillsaveNavDict forKey:@"QC_UserWillSaveNavDict"];
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] qimNav_updateNavigationConfigWithCheck:YES withCallBack:^(BOOL success) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
                    if (success) {
                        [[STKit sharedInstance] setUserObject:userWillsaveNavDict forKey:@"QC_CurrentNavDict"];
                        [self setupUI];
                    } else {
...
- (void)registerNewUserBtnClicked:(UIButton *)btn{
    [STFastEntrance openWebViewForUrl:[[STKit sharedInstance] qimNav_webAppUrl] showNavBar:YES];
}
