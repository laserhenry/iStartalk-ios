...
//            icon.image = [[QIMKit sharedInstance] getGroupImageFromLocalByGroupId:self.jid];
            ...
            QRCodeGenerator *qr = [[QRCodeGenerator alloc] initWithString:[NSString stringWithFormat:@"qtalk://group?id=%@",self.jid]];
                qr.size = CGSizeMake(256.0f, 256.0f);
            QRCodeImageView.image = [qr getImage];
        ...
        case QRCodeType_UserQR:
        {
            ...
            nameLabel.text = self.name;
            icon.image = [[QIMImageManager sharedInstance] getUserHeaderImageByUserId:[[STKit sharedInstance] getLastJid]];
            ...
            QRCodeGenerator *qr = [[QRCodeGenerator alloc] initWithString:[NSString stringWithFormat:@"qtalk://user?id=%@",self.jid]];
                qr.size = CGSizeMake(256.0f, 256.0f);
            QRCodeImageView.image = [qr getImage];
        ...
        case QRCodeType_RobotQR:
        {
            ...
            nameLabel.text = self.name;
            NSDictionary *cardDic = [[STKit sharedInstance] getPublicNumberCardByJid:[NSString stringWithFormat:@"%@@%@",self.jid,[[STKit sharedInstance] getDomain]]];
            NSString *headerSrc = [cardDic objectForKey:@"HeaderSrc"];
            icon.image = [[STKit sharedInstance] getPublicNumberHeaderImageByFileName:headerSrc];
            ...
            QRCodeGenerator *qr = [[QRCodeGenerator alloc] initWithString:[NSString stringWithFormat:url]];
                qr.size = CGSizeMake(256.0f, 256.0f);
            QRCodeImageView.image = [qr getImage];
        ...
        case QRCodeType_ClientNav: {
            ...
            //QRCodeImageView.image = [QRCodeGenerator qrImageForString:url imageSize:QRCodeImageView.bounds.size.width];
            QRCodeGenerator *qr = [[QRCodeGenerator alloc] initWithString:[NSString stringWithFormat:@"%@",self.jid]];
                qr.size = CGSizeMake(256.0f, 256.0f);
            QRCodeImageView.image = [qr getImage];
            [self.navigationItem setTitle:[NSString stringWithFormat:@"%@的二维码",self.name]];
        }
