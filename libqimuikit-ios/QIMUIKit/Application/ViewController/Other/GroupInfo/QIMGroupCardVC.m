...
- (void)updateUI{
    ...
    if ([[STKit sharedInstance] isGroupOwner:self.groupId]) {
        //群设置暂时屏蔽
...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        NSArray *members = [[STKit sharedInstance] getGroupMembersByGroupId:[self groupId]];
//        [[QIMKit sharedInstance] bulkInsertGroupMember:members WithGroupId:[self groupId]];
        _atGroupIn = [[STKit sharedInstance] isGroupMemberByGroupId:self.groupId];
        _groupCardDic = [[STKit sharedInstance] getGroupCardByGroupId:self.groupId];
        dispatch_async(dispatch_get_main_queue(), ^{
            [self updateUI];
        });
    });
...
- (void)reloadMember {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        _oldMembers = [NSMutableArray arrayWithArray:[[STKit sharedInstance] getGroupMembersByGroupId:self.groupId]];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [[STKit sharedInstance] updateGroupCardByGroupId:self.groupId];
    [self reloadMember];
    [_tableView reloadData];
...
- (void)setUpTableViewHeader {
    ...
    [headView qim_setImageWithURL:[[STKit sharedInstance] getGroupBigHeaderImageUrlWithGroupId:self.groupId]];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (alertView.tag == kKickMemberAlertViewTag) {
        if (buttonIndex == 1) {
            ...
            [[STKit sharedInstance] removeGroupMemberWithName:name WithJid:jid ForGroupId:groupId];
            [self reloadMember];
        }
    } else if (alertView.tag == kQuitGroupAlertViewTag){
        if (buttonIndex == 1) {
            BOOL result = [[STKit sharedInstance] quitGroupId:self.groupId];
            if (result) {
                [[STKit sharedInstance] removeStickWithCombineJid:[NSString stringWithFormat:@"%@<>%@", self.groupId, self.groupId] WithChatType:ChatType_GroupChat withCallback:nil];
                [self.navigationController setNavigationBarHidden:NO animated:YES];
...
- (void)onDoneButtonClick:(UIButton *)sender {
    if (_atGroupIn) {
        ...
        _nickName = [[STKit sharedInstance] getMyNickName];
        [[STKit sharedInstance] joinGroupId:self.groupId ByName:_nickName isInitiative:YES];
    }
}
...
- (void)onJoinSuccessNotify:(NSNotification *)notify {
    ...
    if ([_groupId isEqualToString:notify.object]) { 
        [[STKit sharedInstance] openGroupSessionByGroupId:self.groupId ByName:self.groupName];
        [STFastEntrance openGroupChatVCByGroupId:self.groupId];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([value isEqualToString:@"GroupName"]) {
        ...
        BOOL remind = [[STKit sharedInstance] groupPushState:self.groupId];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([value isEqualToString:@"QRCode"]) {
        [STFastEntrance showQRCodeWithQRId:self.groupId withType:QRCodeType_GroupQR];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    [[STKit sharedInstance] updatePushState:self.groupId withOn:state withCallback:nil];
    [_tableView reloadData];
}
