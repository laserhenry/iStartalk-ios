...
- (void)getMyMemberType{
    for (NSDictionary * memDic in self.items) {
        ...
        if ([name isEqualToString:[[STKit sharedInstance] getMyNickName]]) {
...
- (void)groupmemberChangedHandle:(id)notify
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        self.items = [NSMutableArray arrayWithArray:[[STKit sharedInstance] getGroupMembersByGroupId:self.groupID]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *remarkName = [[STKit sharedInstance] getUserMarkupNameWithUserId:userId];
...
{
    return [NSBundle qim_localizedStringForKey:@"Remove"];
}

- (void)delMember:(NSDictionary *)memDic
{
    UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:@"警告！" message:[NSString stringWithFormat:@"您即将将 %@ 踢出群组",memDic[@"name"]] delegate:self cancelButtonTitle:[NSBundle qim_localizedStringForKey:@"Cancel"] otherButtonTitles:[NSBundle qim_localizedStringForKey:@"common_ok"], nil];
    [alertView show];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        BOOL success = [[STKit sharedInstance] removeGroupMemberWithName:name WithJid:jid ForGroupId:self.groupID];
        if (success) {
            ...
            [[STKit sharedInstance] bulkInsertGroupMember:self.items WithGroupId:self.groupID];
