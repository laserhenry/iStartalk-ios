...
- (void)viewDidLoad {
    ...
    self.navConfigs = [[STKit sharedInstance] qimNav_localNavConfigs];
    [self initUI];
    if (![[STKit sharedInstance] userObjectForKey:@"QC_CurrentNavDict"]) {
        
        [[STKit sharedInstance] setUserObject:self.navConfigs[0] forKey:@"QC_CurrentNavDict"];
...
- (void)onSaveWithNavUrl:(NSString *)navUrl WithNavDict:(NSDictionary *)navUrlDict needSaveAllDict:(BOOL)needSaveAllDict {
    if (navUrl.length > 0) {
        [[STKit sharedInstance] setUserObject:navUrlDict forKey:@"QC_UserWillSaveNavDict"];
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[STKit sharedInstance] qimNav_updateNavigationConfigWithCheck:YES withCallBack:^(BOOL success) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (success) {
                        [[STKit sharedInstance] setUserObject:navUrlDict forKey:@"QC_CurrentNavDict"];
                        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)onSave{
    ...
    if (self.tempNavDict.count) {
        ...
        __block NSDictionary *navUrlDict = [[STKit sharedInstance] userObjectForKey:@"QC_CurrentNavDict"];
        NSString *navUrl;
        if (navUrlDict) {
...
- (void)onFeedBack {
    ...
    [[STLocalLog sharedInstance] submitFeedBackWithContent:nil withUserInitiative:NO];
#endif
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (dict) {
        ...
        cell.navInfo = dict;
        if ([dict isEqualToDictionary:[[STKit sharedInstance] userObjectForKey:@"QC_CurrentNavDict"]]) {
...
- (void)deleteNavWithNavCell:(id)cell {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[STKit sharedInstance] setUserObject:self.navConfigs forKey:@"QC_NavAllDicts"];
    });
}
...
- (void)showNavQRWithNavCell:(id)cell {
    ...
    [STFastEntrance showQRCodeWithQRId:navUrl withType:QRCodeType_ClientNav];
}
