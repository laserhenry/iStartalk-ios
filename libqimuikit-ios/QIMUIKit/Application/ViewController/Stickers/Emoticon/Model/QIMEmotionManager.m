...
- (instancetype)init {
    ...
    if (self) {
        ...
        dispatch_async(_emotionManagerQueue, ^{
            NSNumber *emotionUpdateKey = [[STKit sharedInstance] userObjectForKey:kEmotionUpdateKey];
            if (emotionUpdateKey == nil) {
                [[STKit sharedInstance] setUserObject:@(YES) forKey:kEmotionUpdateKey];
            }

            emotionUpdateKey = [[STKit sharedInstance] userObjectForKey:kEmotionUpdateKey];
            if ([emotionUpdateKey boolValue] == YES) {
                [[STKit sharedInstance] removeUserObjectForKey:@"EmojiOne"];
                // 判断cache文件夹
            ...
            BOOL reloadfromUrl = [[[STKit sharedInstance] userObjectForKey:@"emotion_check"] boolValue];
            if (reloadfromUrl) {
                [self getAndCheckEmotionsInfoFromHttp];
            }

            if (emotionUpdateKey.boolValue) {
                [[STKit sharedInstance] setUserObject:@(NO) forKey:kEmotionUpdateKey];
            }
...
- (void)expandNormalZipFileNamed:(NSString *)fileName {
    ...
    if (![[STKit sharedInstance] userObjectForKey:fileName] || ![[NSFileManager defaultManager] fileExistsAtPath:[outputPath stringByAppendingString:[[[STKit sharedInstance] userObjectForKey:fileName] objectForKey:@"xmlFilePath"]] isDirectory:nil]) {
        ...
        [[STKit sharedInstance] setUserObject:[NSDictionary dictionaryWithObjectsAndKeys:xmlPath, @"xmlFilePath", [NSString stringWithFormat:@"/%@", [[zip getZipFileContents] firstObject]], @"imagePreFix", nil] forKey:fileName];
    } else {
        NSString *xmlPath = [outputPath stringByAppendingString:[[[STKit sharedInstance] userObjectForKey:fileName] objectForKey:@"xmlFilePath"]];
...
//            QIMVerboseLog(@"expandNormalZipFileNamed FileName : %@ ===  %@", fileName, [[QIMKit sharedInstance] userObjectForKey:fileName]);
            self.currentPrefixPath = [[NSString stringWithFormat:@"%@", fileName] stringByAppendingPathComponent:[[[STKit sharedInstance] userObjectForKey:fileName] objectForKey:@"imagePreFix"]];
...
//            "name": "牛驼表情",
//            "file": "https://qim.qunar.com/resources/niutuoEmotions.emt",
//            "desc": "扩展表情包,下载后可显示牛驼表情",
//            "thumb": "https://qim.qunar.com/resources/niutuoThumb.gif",
//            "file_size": 881251,
    ...
    if (pkId && ![_emotionDispPkIdList containsObject:pkId]) {
        ...
        dispatch_async(concurrentQueue, ^{
            __block NSString *url = [NSString stringWithFormat:@"%@/file/v2/emo/d/z/%@?name=%@&u=%@&k=%@",
                                                               [[STKit sharedInstance] qimNav_InnerFileHttpHost],
                                                               pkId,
                                                               fileName,
...
- (void)getEmotionImageFromHttpForPkId:(NSString *)pkId shortcut:(NSString *)shortcut signKey:(NSString *)signKey {
    ...
    dispatch_async(_emotionManagerQueue, ^{
        ...
        [paramDic setQIMSafeObject:[STKit getLastUserName] forKey:@"u"];
        [paramDic setQIMSafeObject:[[STKit sharedInstance] myRemotelogginKey] forKey:@"k"];
        NSURL *url = nil;
        if (pkId.length) {
            url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/file/v2/emo/d/e/%@/%@/%@?u=%@&k=%@",
                                                                  ...
                                                                  @"org",
                                                                  [[STKit getLastUserName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],
                                                                  [[STKit sharedInstance] myRemotelogginKey]]];//org或者fixed
        } else {
            //兼容老版本
