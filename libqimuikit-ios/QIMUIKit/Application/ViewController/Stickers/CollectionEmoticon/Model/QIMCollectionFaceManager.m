...
- (instancetype)init {
    ...
    if (self) {
        ...
        _collectionFaceList = [NSMutableArray arrayWithArray:[[STKit sharedInstance] getClientConfigValueArrayWithType:QIMClientConfigTypeKCollectionCacheKey]];
    }
    return self;
}
...
- (NSArray *)getCollectionFaceList {
    ...
    dispatch_block_t block = ^{
        ...
        if ([_collectionFaceList count] <= 0) {
            [_collectionFaceList addObjectsFromArray:[[STKit sharedInstance] getClientConfigValueArrayWithType:QIMClientConfigTypeKCollectionCacheKey]];
        }
    };
...
- (void)updateCollectionEmojiWithEmojiUrl:(NSString *)emojiUrl {
    dispatch_block_t block = ^{
        NSString *emojiMd5 = [[STKit sharedInstance] md5fromUrl:emojiUrl];
        [[STKit sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKCollectionCacheKey WithSubKey:emojiMd5 WithConfigValue:emojiUrl WithDel:NO withCallback:nil];
    };
...
- (void) replaceCollectionInfoWithIndex:(NSInteger )index NewInfo:(NSDictionary *)newInfo {
    ...
    if (index < 0 || index > [_collectionFaceList count]) {
        ...
        [[STKit sharedInstance] setUserObject:_collectionFaceList forKey:kCollectionCacheKey];
...
- (void)fixErrorData {
    ...
    if (fixed) {
        ...
        dispatch_block_t block = ^{
            ...
            [[STKit sharedInstance] setUserObject:nil forKey:kCollectionCacheKey];
        };
...
- (void) delCollectionFaceArr:(NSArray *)delArr
{
    for (id item in delArr) {
        ...
        if ([item isKindOfClass:[NSString class]]) {
            httpUrl = item;
            NSString *imageName = [[STKit sharedInstance] getFileNameFromUrl:httpUrl];
    ...
    dispatch_block_t block = ^{
        ...
        for (NSString *item in array) {
            ...
            if (!httpUrl.length) {
                ...
                NSString *configSubKey = [[STKit sharedInstance] md5fromUrl:httpUrl];
        ...
        if (canUpdateArr.count) {
            ...
            [[STKit sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKCollectionCacheKey BatchProcessConfigInfo:canUpdateArr WithDel:del withCallback:nil];
        }
    };
...
//            [[QIMKit sharedInstance] updateRemoteClientConfigBatchProcessConfigInfo:infoDicStr WithDel:YES];
            [[STKit sharedInstance] updateRemoteClientConfigWithType:QIMClientConfigTypeKCollectionCacheKey BatchProcessConfigInfo:infoDicStr WithDel:YES withCallback:nil];
        }
    };
