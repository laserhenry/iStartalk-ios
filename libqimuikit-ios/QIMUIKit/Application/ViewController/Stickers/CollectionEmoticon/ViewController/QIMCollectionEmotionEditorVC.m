...
@interface QIMCollectionEmotionEditorVC () <UICollectionViewDelegate, UICollectionViewDataSource, UINavigationBarDelegate, QIMCollectionEmotionEditorViewDelegate, STImgPickerControllerDelegate, QIMDragCellCollectionViewDelegate, QIMDragCellCollectionViewDataSource, UIAlertViewDelegate>
{
...
- (void)collectionEmotionEditorCell:(QIMCollectionEmotionEditorViewCell *)cell didClickedItemAtIndex:(NSInteger)index selected:(BOOL)selected {
    ...
    if (![item isEqualToString:kImageFacePageViewAddFlagName]) {
        ...
        STImgPickerController *imagePickerController = [[STImgPickerController alloc] init];
...
-(void)qtImagePickerController:(STImgPickerController *)picker didFinishPickingAssets:(NSArray *)assets ToOriginal:(BOOL)flag
{
    for (ALAsset * asset in assets) {
        ...
        //mark temp
        [[STKit sharedInstance] qim_uploadImageWithImageData:imageData WithMsgId:nil WithMsgType:30 WithPathExtension:@"png" withCallBack:^(NSString *httpPermUrl) {
...
-(void)qtImagePickerController:(STImgPickerController *)picker didFinishPickingImage:(UIImage *)image
{
    ...
    //mark temp
    [[STKit sharedInstance] qim_uploadImageWithImageData:imageData WithMsgId:nil WithMsgType:30 WithPathExtension:@"jpg" withCallBack:^(NSString *imageURL) {
