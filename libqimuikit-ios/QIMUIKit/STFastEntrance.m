...
@interface STFastEntrance () <MFMailComposeViewControllerDelegate>

@end

@interface STFastEntrance () <QIMMWPhotoBrowserDelegate>
...
@implementation STFastEntrance

static STFastEntrance *_sharedInstance = nil;

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[STFastEntrance alloc] init];
    });
    return _sharedInstance;
}

+ (instancetype)sharedInstanceWithRootNav:(UINavigationController *)nav rootVc:(UIViewController *)rootVc {
    STFastEntrance *instance = [STFastEntrance sharedInstance];
...
- (void)launchMainControllerWithWindow:(UIWindow *)window {
    ...
    if ([STKit getQIMProjectType] == QIMProjectTypeQChat) {
        if ([[STKit sharedInstance] qimNav_Debug] == 1) {
        ...
        NSString *userName = [STKit getLastUserName];
        NSString *userToken = [[STKit sharedInstance] getLastUserToken];
        if (userName.length > 0 && userToken.length > 0 && [[STKit sharedInstance] getIsIpad] == NO) {
            STMainVC *mainVC = [STMainVC sharedInstanceWithSkipLogin:YES];
            ...
            if (userName && userToken && [STKit getQIMProjectType] == QIMProjectTypeQTalk) {
...
- (void)launchMainAdvertWindow {
    ...
    [[STAppWindowMgr sharedInstance] setAdvertWindow:advertWindow];
    NSTimeInterval nowAdTime = [NSDate timeIntervalSinceReferenceDate];
    [[STKit sharedInstance] setUserObject:@(nowAdTime) forKey:@"lastAdShowTime"];
}

+ (void)showMainVc {
    if ([STMainVC checkMainVC] == NO || [STMainVC getMainVCReShow] == YES) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([[STKit sharedInstance] getIsIpad] == YES) {
                /* mark by newipad
                STMainVC *mainVC = [STMainVC sharedInstanceWithSkipLogin:NO];
                QIMNavController *navVC = [[QIMNavController alloc] initWithRootViewController:mainVC];
...
- (void)sendMailWithRootVc:(UIViewController *)rootVc ByUserId:(NSString *)userId {
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([MFMailComposeViewController canSendMail]) {
            ...
            [controller setToRecipients:@[[NSString stringWithFormat:@"%@@%@", [[userId componentsSeparatedByString:@"@"] firstObject], [[STKit sharedInstance] qimNav_Email]]]];
            [controller setSubject:[NSString stringWithFormat:@"From %@", [[STKit sharedInstance] getMyNickName]]];
            [controller setMessageBody:[NSString stringWithFormat:@"\r\r\r\r\r\r\r\r\r\r\r From Iphone %@.", [STKit getQIMProjectTitleName]] isHTML:NO];
            if (rootVc) {
...
- (UIViewController *)getUserChatInfoByUserId:(NSString *)userId {
    ...
    if ([[STKit sharedInstance] getIsIpad]) {
        ...
        if ([[STKit sharedInstance] qimNav_RNUserCardView]) {
...
- (UIViewController *)getQIMGroupCardVCByGroupId:(NSString *)groupId {
    ...
    if ([[STKit sharedInstance] qimNav_RNGroupCardView]) {
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
            navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
        }
        ...
        UIViewController *vc = nil;
        if ([RunC respondsToSelector:sel]) {
            QIMGroupIdentity groupIdentity = [[STKit sharedInstance] GroupIdentityForUser:[[STKit sharedInstance] getLastJid] byGroup:groupId];
...
+ (void)openQIMGroupCardVCByGroupId:(NSString *)groupId {
    ...
    if ([[STKit sharedInstance] qimNav_RNGroupCardView]) {
        ...
        BOOL isGroupOwner = [[STKit sharedInstance] isGroupOwner:groupId];
        Class RunC = NSClassFromString(@"QimRNBModule");
        SEL sel = NSSelectorFromString(@"openQIMRNVCWithParam:");
        if ([RunC respondsToSelector:sel]) {
            QIMGroupIdentity groupIdentity = [[STKit sharedInstance] GroupIdentityForUser:[[STKit sharedInstance] getLastJid] byGroup:groupId];
...
+ (void)openFastChatVCByXmppId:(NSString *)userId WithRealJid:(NSString *)realJid WithChatType:(NSInteger)chatType WithFastMsgTimeStamp:(long long)fastMsgTime {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIViewController *chatVc = [[STFastEntrance sharedInstance] getFastChatVCByXmppId:userId WithRealJid:realJid WithChatType:chatType WithFastMsgTimeStamp:fastMsgTime];
...
+ (void)openConsultChatByChatType:(ChatType)chatType UserId:(NSString *)userId WithVirtualId:(NSString *)virtualId {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *consultChatVc = [[STFastEntrance sharedInstance] getConsultChatByChatType:chatType UserId:userId WithVirtualId:virtualId];
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
            navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
        }
...
+ (void)openConsultServerChatByChatType:(ChatType)chatType WithVirtualId:(NSString *)virtualId WithRealJid:(NSString *)realjid {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *consultChatVc = [[STFastEntrance sharedInstance] getConsultServerChatByChatType:chatType WithVirtualId:virtualId WithRealJid:realjid];
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
            navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
        }
...
- (UIViewController *)getSingleChatVCByUserId:(NSString *)userId withFastTime:(long long)fastTime withRemoteSearch:(BOOL)flag {
    ...
    if (userInfo == nil) {
        [[STKit sharedInstance] updateUserCard:userId withCache:YES];
        userInfo = [[STKit sharedInstance] getUserInfoByUserId:userId];
    }
    NSString *name = [[STKit sharedInstance] getUserMarkupNameWithUserId:userId];
    if (name.length <= 0) {
        name = [userId componentsSeparatedByString:@"@"].firstObject;
    }
    ChatType chatType = [[STKit sharedInstance] openChatSessionByUserId:userId];
...
+ (void)openSingleChatVCByUserId:(NSString *)userId withFastTime:(long long)fastTime withRemoteSearch:(BOOL)flag {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *chatVC = [[STFastEntrance sharedInstance] getSingleChatVCByUserId:userId withFastTime:fastTime withRemoteSearch:flag];
...
+ (void)openSingleChatVCByUserId:(NSString *)userId {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *chatVC = [[STFastEntrance sharedInstance] getSingleChatVCByUserId:userId];
        [[NSNotificationCenter defaultCenter] postNotificationName:kNotifySelectTab object:@(0)];
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
            navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
...
+ (void)openGroupChatVCByGroupId:(NSString *)groupId withFastTime:(long long)fastTime withRemoteSearch:(BOOL)flag {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *chatGroupVC = [[STFastEntrance sharedInstance] getGroupChatVCByGroupId:groupId withFastTime:fastTime withRemoteSearch:flag];
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
            navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
        }
...
+ (void)openGroupChatVCByGroupId:(NSString *)groupId {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *chatGroupVC = [[STFastEntrance sharedInstance] getGroupChatVCByGroupId:groupId];
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
            navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
        }
...
- (UIViewController *)getHeaderLineVCByJid:(NSString *)jid {
    if ([jid hasPrefix:@"FriendNotify"]) {
        ...
        webView.url = [[STKit sharedInstance] qimNav_QcGrabOrder];
        return webView;
    } else if ([jid hasPrefix:@"rbt-zhongbao"]) {
        QIMWebView *webView = [[QIMWebView alloc] init];
        webView.needAuth = YES;
        webView.navBarHidden = YES;
        [[STKit sharedInstance] clearNotReadMsgByJid:jid];
        webView.url = [[STKit sharedInstance] qimNav_QcOrderManager];
        return webView;
    } else {
...
+ (void)openHeaderLineVCByJid:(NSString *)jid {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIViewController *chatSystemVC = [[STFastEntrance sharedInstance] getHeaderLineVCByJid:jid];
...
+ (void)openQRCodeVC {
    dispatch_async(dispatch_get_main_queue(), ^{
        QIMZBarViewController *vc = [[QIMZBarViewController alloc] initWithBlock:^(NSString *str, BOOL isScceed) {
            if (isScceed) {
                [STJumpURLHandle decodeQCodeStr:str];
            }
        }];
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
...
- (UIViewController *)getRobotChatVC:(NSString *)robotJid {
    NSDictionary *cardDic = [[STKit sharedInstance] getPublicNumberCardByJId:robotJid];
    if (cardDic.count > 0) {
...
- (UIViewController *)getRobotCard:(NSString *)robotJid {
    ...
    if ([[STKit sharedInstance] getPublicNumberCardByJid:robotJid]) {
...
- (UIViewController *)getMyRedPack {
    ...
    if (myRedpackageUrl.length > 0) {
        return [[STFastEntrance sharedInstance] getWebViewForUrl:myRedpackageUrl showNavBar:YES];
    }
    return nil;
}
...
- (UIViewController *)getMyRedPackageBalance {
    ...
    if (balacnceUrl.length > 0) {
        return [[STFastEntrance sharedInstance] getWebViewForUrl:balacnceUrl showNavBar:YES];
    }
    return nil;
}
...
+ (void)openQIMRNWithScheme:(NSString *)scheme withChatId:(NSString *)chatId withRealJid:(NSString *)realJid withChatType:(ChatType)chatType {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [properties setQIMSafeObject:[[STKit sharedInstance] getLastJid] forKey:@"from"];
        ...
        SEL sel = NSSelectorFromString(@"openQIMRNVCWithParam:");
        if ([RunC respondsToSelector:sel]) {
            NSDictionary *param = @{@"navVC": navVC, @"hiddenNav": @(YES), @"module": @"Merchant", @"properties": @{@"Screen": @"Seats", @"from": [[STKit sharedInstance] getLastJid], @"to": chatId, @"customerName": realJid}};
            [RunC performSelector:sel withObject:param];
        }
#endif
    });
...
- (UIViewController *)getQRCodeWithQRId:(NSString *)qrId withType:(QRCodeType)qrcodeType {
    ...
    switch (qrcodeType) {
        case QRCodeType_UserQR: {
            NSDictionary *userInfo = [[STKit sharedInstance] getUserInfoByUserId:qrId];
            qrName = [userInfo objectForKey:@"Name"];
        }
            break;
        case QRCodeType_GroupQR: {
            NSDictionary *groupVCard = [[STKit sharedInstance] getGroupCardByGroupId:qrId];
            qrName = [groupVCard objectForKey:@"Name"];
        }
            break;
        case QRCodeType_RobotQR: {
            NSDictionary *robotVCard = [[STKit sharedInstance] getPublicNumberCardByJid:qrId];
...
+ (void)showQRCodeWithQRId:(NSString *)qrId withType:(QRCodeType)qrcodeType {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *qrVC = [[STFastEntrance sharedInstance] getQRCodeWithQRId:qrId withType:qrcodeType];
        UINavigationController *navVC = [[UIApplication sharedApplication] visibleNavigationController];
        if (!navVC) {
            navVC = [[STFastEntrance sharedInstance] getQIMFastEntranceRootNav];
        }
...
//        [[QIMKit sharedInstance] sendNoPush];
        ...
        [[STKit sharedInstance] clearDataBase];
        [[STKit sharedInstance] clearLogginUser];
        [[STKit sharedInstance] quitLogin];
        [[STKit sharedInstance] setNeedTryRelogin:NO];
        [[STKit sharedInstance] clearUserToken];
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([[STKit sharedInstance] getIsIpad] && [STKit getQIMProjectType] != QIMProjectTypeQChat) {
#if __has_include("QIMIPadWindowManager.h")
...
+ (void)signOut {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        [[STKit sharedInstance] sendPushTokenWithMyToken:nil WithDeleteFlag:YES withCallback:^(BOOL result) {
...
- (void)browseBigHeader:(NSDictionary *)param {
    ...
    if (imageUrl.length > 0) {
        ...
        NSString *headerUrl = [[STKit sharedInstance] getUserHeaderSrcByUserId:self.browerImageUserId];
        if (![headerUrl qim_hasPrefixHttpHeader]) {
            headerUrl = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], headerUrl];
        }
...
#pragma mark - 查看大图
    if (self.browerImageUrlList.count > 0) {
        ...
        if (![imageUrl qim_hasPrefixHttpHeader]) {
            imageUrl = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], imageUrl];
        }
        ...
        if (![self.browerImageUrl qim_hasPrefixHttpHeader]) {
            self.browerImageUrl = [NSString stringWithFormat:@"%@/%@", [[STKit sharedInstance] qimNav_InnerFileHttpHost], self.browerImageUrl];
        }
...
- (void)openQIMFilePreviewVCWithParam:(NSDictionary *)param {
    ...
    STMsgModel *fileMsg = [[STMsgModel alloc] init];
    ...
    STFilePreviewVC *filePreviewVc = [[STFilePreviewVC alloc] init];
