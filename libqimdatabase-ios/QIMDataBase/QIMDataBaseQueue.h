...
/** To perform queries and updates on multiple threads, you'll want to use `QIMDataBaseQueue`.
 
 Using a single instance of `<QIMDataBase>` from multiple threads at once is a bad idea.  It has always been OK to make a `<QIMDataBase>` object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.
 
 Instead, use `QIMDataBaseQueue`. Here's how to use it:
 
 First, make your queue.
 
 QIMDataBaseQueue *queue = [QIMDataBaseQueue databaseQueueWithPath:aPath];
 
 Then use it like so:
 
 [queue inDatabase:^(QIMDataBase*db) {
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 
 DataReader *rs = [db executeQuery:@"select * from foo"];
 while ([rs next]) {
 //…
 }
 }];
 ...
 [queue inTransaction:^(QIMDataBase*db, BOOL *rollback) {
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
 [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 ...
 *rollback = YES;
 ...
 `QIMDataBaseQueue` will run the blocks on a serialized queue (hence the name of the class).  So if you call `QIMDataBaseQueue`'s methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.
 
 ### See also
 
 - `<QIMDataBase>`
 
 @warning Do not instantiate a single `<QIMDataBase>` object and use it across multiple threads. Use `QIMDataBaseQueue` instead.
 
 @warning The calls to `QIMDataBaseQueue`'s methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.
