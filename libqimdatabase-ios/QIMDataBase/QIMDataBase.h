...
#define QIMDBRetain(__v) ([__v retain]);
#define QIMDBReturnRetained QIMDBRetain

#define QIMDBRelease(__v) ([__v release]);

#define QIMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
// -fobjc-arc
#define QIMDBAutorelease(__v)
#define QIMDBReturnAutoreleased(__v) (__v)

#define QIMDBRetain(__v)
#define QIMDBReturnRetained(__v) (__v)

#define QIMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
#if OS_OBJECT_USE_OBJC
#define QIMDBDispatchQueueRelease(__v)
#else
#define QIMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#endif
#endif
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - [QIMDB on GitHub](https://github.com/ccgus/QIMDB) including introductory documentation
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) QIMDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<DataReader>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[DataReader next]>`) from one record to the other.
 ...
 @return A `<DataReader>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> DataReader!`
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 DataReader *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one QIMDB instance and should not be used for other purposes.
...
@interface QIMDBStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
