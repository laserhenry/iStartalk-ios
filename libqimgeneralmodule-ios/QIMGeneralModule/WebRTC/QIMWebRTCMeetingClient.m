...
- (void)joinRoomByMessage:(NSDictionary *)message {
    ...
    if (message) {
        ...
        _createRoom = NO;
        NSDictionary * dic =  [[STKit sharedInstance] getGroupCardByGroupId:roomId];
        if (dic && dic.count > 0) {
            roomName = dic[@"Name"];
        }
...
//        self.rtcMeetingView.headerImage = [[QIMKit sharedInstance] getGroupImageFromLocalByGroupId:self.groupId];
        if ([[STKit sharedInstance] getCurrentServerTime] - startTime > 24 * 60 * 60 * 1000) {
...
- (void)joinRoomById:(NSString *)roomId WithRoomName:(NSString *)roomName {
    ...
    self.rtcMeetingView.headerImage = [STKit defaultGroupHeaderImage];
    self.rtcMeetingView.nickName = roomName;
    [self.rtcMeetingView show];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([peerConnection isEqual:self.localPeerConnection]) {
            if (self.localPeerConnection.remoteDescription) {
                [self.rtcMeetingView.socketClient sendICECandidateWithEndpointName:[[STKit sharedInstance] getLastJid] WithCandidate:candidate.sdp WithSdpMLineIndex:(int) candidate.sdpMLineIndex WithSdpMid:candidate.sdpMid complete:^(BOOL success) {
                    QIMVerboseLog(@"success : %d", success);
                }];
            } else {
                NSString *name = [[STKit sharedInstance] getLastJid];
...
- (void)webRTCSocketClientDidConnected:(QIMWebRTCSocketClient *)client {
    ...
    [mySelf.rtcMeetingView.socketClient joinRoom:mySelf.roomId WithTopic:mySelf.roomName WithNickName:[[STKit sharedInstance] getLastJid] complete:^(NSDictionary *resultDic) {
        ...
        if (result) {
            ...
            if (_createRoom) {
                ...
                [messageDic setObject:mySelf.rtcMeetingView.roomId ? mySelf.rtcMeetingView.roomId : [[STKit sharedInstance] getLastJid] forKey:@"roomName"];
                ...
                [messageDic setObject:@([[STKit sharedInstance] getCurrentServerTime]) forKey:@"createTime"];
                [messageDic setObject:[[STKit sharedInstance] getLastJid] forKey:@"creator"];
                [messageDic setObject:@([[STKit sharedInstance] getCurrentServerTime]) forKey:@"startTime"];
                ...
                STMsgModel *msg = [[STKit sharedInstance] sendMessage:@"[当前客户端不支持音视频]" WithInfo:extendInfo ToGroupId:mySelf.groupId WithMsgType:QIMMessageTypeWebRtcMsgTypeVideoGroup];
                dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [mySelf.localPeerConnection setRemoteDescription:remoteSdp completionHandler:^(NSError *_Nullable error) {
                            if (error) {
                                ...
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    ...
                                    NSString *myUserName = [[STKit sharedInstance] getLastJid];
...
- (void)addIceCandidateWithUserName:(NSString *)userName WithCandidate:(NSString *)candidate WithSdpMLineIndex:(int)sdpMLineIndex WithSdpMid:(NSString *)sdpMid {
    ...
    if ([userName isEqualToString:[[STKit sharedInstance] getLastJid]]) {
        if ([self.localPeerConnection remoteDescription]) {
            [self.localPeerConnection addIceCandidate:cand];
        } else {
            QIMVerboseLog(@"");
            NSMutableArray *list = [self.peerConnectionCanDic objectForKey:[[STKit sharedInstance] getLastJid]];
            if (list == nil) {
                list = [NSMutableArray array];
                [self.peerConnectionCanDic setObject:list forKey:[[STKit sharedInstance] getLastJid]];
            }
            [list addObject:cand];
        }
