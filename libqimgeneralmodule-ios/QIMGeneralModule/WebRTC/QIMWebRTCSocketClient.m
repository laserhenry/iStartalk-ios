...
- (instancetype)init {
    ...
    if (self) {
        ...
        _rtcSocketHost = [[STKit sharedInstance] qimNav_WssHost];
        _httpRtcSocketHost = [[STKit sharedInstance] qimNav_VideoApiHost];
...
//http://150.242.184.16:8080/room/
    NSString *remoteKey = [[STKit sharedInstance] thirdpartKeywithValue];
...
- (void)sendMessage:(NSMutableDictionary *)params msgId:(int)msgId complete:(Callback)complete {
    if (params) {
        [params setObject:[[STKit sharedInstance] thirdpartKeywithValue] forKey:@"ckey"];
...
- (void)joinRoom:(NSString *)roomName WithTopic:(NSString *)topic WithNickName:(NSString *)nickName complete:(void (^)(NSDictionary *))complete {
    ...
    [params setObject:@([[STKit sharedInstance] getCurrentServerTime]) forKey:@"startTime"];
